...
oj.TreeDndContext.prototype.handleDnDOptionChange = function (newval) {
  ...
  if ((reorder !== dnd.reorder) || (dragFromEnabled !== dnd.dragFromEnabled)) {
    if ((!dnd.reorder) && (!dnd.dragFromEnabled)) {
      this.component._getContainerList().find('li')
...
oj.TreeDndContext.prototype._drag = function (e) {
  ...
  if (dnd.dragDragCallback) { // check if app callback in drag options?
...
oj.TreeDndContext.prototype._dragOver = function (e) {
  ...
  if (elemId === dnd.nodeDraggedId) { // ignore drag over the dragged node itself
    this._dndHideMarker();
    return undefined;
  }
...
oj.TreeDndContext.prototype._dndShowMarker = function () {
  ...
  if (r === 'first') { // if to be placed as the first child of
  ...
  if (r) {
    ...
    vars.r.removeClass(cons._OJ_VALID_DROP)
           .removeClass(cons._OJ_VALID_DROP);
  }
...
oj.TreeDndContext.prototype._clearDropClasses = function ($var) {
  ...
  if ($var) {
    $var.removeClass(cons._OJ_VALID_DROP)
...
(function () {
  ...
  */
  ...
  var OJ_DISC = 'oj-tree-icon oj-tree-disclosure-icon oj-component-icon oj-clickable-icon-nocontext oj-default';
  ...
  oj.__registerWidget('oj.ojTree', $.oj.baseComponent,
    {
     ...
     /*---------------*/
      ...
      collapse: function (node, skipAnim) {
        ...
        if (!node.length || node === -1 || !node.hasClass(TreeUtils._OJ_EXPANDED) ||
                            this._data.core.locked || node.hasClass(TreeUtils._OJ_DISABLED)) {
          return false;
        }
...
//      }
        ...
        this._addBusyState("Tree (id='" + this._elemId + "') : node id='" + node.attr('id') +
                           "' is animating.");
      ...
      _compareSelectionValues: function (value1, value2) {
        ...
        if (len !== len2) {
          ...
          for (var i = 0; i < len; i++) {
            ...
            if (!bInList) { // if not in list, values are different
              bDiff = true;
              break;
            }
          }
...
//        this._lock(newval) ;
      ...
      _prepare_move: function (o, r, pos, cb, isCb) {
        ...
        if (!isCb && this._data.core.prepMoveBlk.o &&
        ...
        if (!/^(before|after)$/.test(p.p) &&
            p.op && p.np && p.op[0] === p.np[0] && p.o.index() < p.cp) {
          p.cp += 1;
        }
      ...
      _moveNode: function (obj, ref, position, isCopy, isPrepared, skipCheck) {
        ...
        if ((obj.hasClass && obj.hasClass(TreeUtils._OJ_DISABLED)) ||
            this._data.core.locked) {
          return undefined;
        }

        if (!isPrepared) {
          return this._prepare_move(obj, ref, position,
                                    function (p) {
                                      ...
                                      this._moveNode(p, false, false,
                                                     isCopy, true, skipCheck);
                                    });
        }
...
//         }
          ...
          // Add a busy state for the animation.  The busy state resolver will be invoked
          // when the animation is completed
          this._addBusyState("Tree (id='" + this._elemId + "') : node id='" +
      ...
      _select: function (node, check, e) {
        ...
        if ((!e) || (e.type !== 'touchend')) { // if via api, ensure touchEvent flag is off
          ui.touchEvent = false;
        }
        ...
        if (check) {
          ...
          switch (!0) {
            case (isRange):
              ...
              node = node[node.index() < ui.lastSelected.index() ?
                          'nextUntil' : 'prevUntil']('.oj-tree-last-selected').addBack();
            ...
            case (!isSelected && isMultiple):
              if ((selectMode === -1) ||
                  (ui.selected.length + 1 <= selectMode)) {
                proceed = true;
              }
              break;
            default:
...
//   },
       ...
       */
      // eslint-disable-next-line no-unused-vars
      _load_node: function (obj, successCallback, errorCallback) { // Dummy function overriden by data methods
       ...
       */
      // eslint-disable-next-line no-unused-vars
      _isLoaded: function (obj) { // Dummy function overriden by data methods
        return true;
      },
...
//      this._emitEvent(null, "reselect", true);
      ...
      _saveSelected: function () {
        ...
        ui.selected.each(function () {
          if (this.id) {
            ui.toSelect.push('#' + this.id.toString()
                             .replace(/^#/, '')
                             .replace(/\\\//g, '/')
                             .replace(/\//g, '\\/')
                             .replace(/\\\./g, '.')
        ...
        switch (!0) {
          ...
          case (!data && !!ajax) || (!!data && !!ajax && obj && obj !== -1):
            ...
            successFunc = function (d, status, x) {
              ...
              } else if (obj === -1 || !obj) { // parse returned nothing
            ...
            this._addBusyState("Tree (id='" + this._elemId + "') : node id='" + nodeId +
                               "' is lazyloading.");
...
//             }
          ...
          a.each(function () {
            ...
            if (aAttr.length ||
                ...
                (t.children('ins').get(0).className && t.children('ins').get(0).className
                 .replace(/oj-tree[^ ]*|$/ig, '').length)) {
              ...
              if (t.children('ins').get(0).className.replace(/oj-tree[^ ]*|$/ig, '')
                  .replace(/^\s+$/ig, '').length) {
                sAttr = t.children('ins').get(0).className
                  .replace(/oj-tree[^ ]*|$/ig, '')
                  .replace(/ oj-[^ ]*/ig, '')
                  .replace(/\s+$/ig, ' ')
                  .replace(/^ /, '')
              ...
              if (t.children('ins').get(0).style.backgroundImage.length) {
                sAttr = t.children('ins').get(0).style.backgroundImage
                  .replace('url(', '')
                  .replace(')', '');
                if (sAttr.length) {
                  if (!tmp3) {
...
*/
        ...
        switch (!0) {
          ...
          case (!!data && !ajax) || (!!data && !!ajax && (!obj || obj === -1)):
            if (!obj || obj === -1) {
              ...
              this._$container_ul.find('li.oj-tree-leaf ins:first-child')
                .removeClass(OJ_DISC)
            ...
            successFunc = function (d, status, x) {
              ...
              if (d) {
                ...
                if (obj === -1 || !obj) {
                  ...
                  self._$container_ul.find('li.oj-tree-leaf ins:first-child')
                    .removeClass(OJ_DISC)
                    .removeClass(OJT_NICON)
                    .addClass(OJT_ICON);
                  ...
                  obj.find('li.oj-tree-leaf ins:first-child')
                    .removeClass(OJ_DISC)
                    .removeClass(OJT_NICON)
                    .addClass(OJT_ICON);
                  parent = obj;
      ...
      _save_opened: function () {
        ...
        this._$container_ul.find('li.oj-expanded').each(function () {
          if (this.id) {
            _this._data.core.toExpand.push('#' + this.id.toString()
                                           .replace(/^#/, '')
                                           .replace(/\\\//g, '/')
                                           .replace(/\//g, '\\/')
                                           .replace(/\\\./g, '.')
...
//      this._initCrrm() ;          // not in V1
      ...
      _emitEvent: function (data, evname, bInternal) {
        ...
        if ((this._data.core.locked === true) && (evname !== 'unlock')
                                              && (evname !== 'isLocked')
                                              && (evname !== 'lock')) {
          return undefined;
        }
    ...
    /**
      ...
      */
      _initUI: function () {
        ...
        if (this._isTouch) { // for tap select/deselect
          this._$container
             .delegate('.oj-tree-list a', 'touchend.ojtree', $.proxy(function (event) {
...
//               }
        ...
        */
          ...
          .bind('ojmove', function (event, ui) {
            ...
            if ((data.ot !== data.rt) && (!copy)) { // different trees and a move op?
            ...
            $.each(types, function (i, tp) {
              ...
              if (tp.icon.image || tp.icon.position) {
                if (i === 'default') {
                  ...
                  iconsCss += '} ';
                  iconsCss += '.oj-tree-' + _this._getIndex() + ' .oj-tree-list li[' +
                    attr + '="oj-tree-deftype"].oj-tree-type > a ins.oj-tree-node-icon { ';
                } else if (tp.icon.image) {
                  iconsCss += '.oj-tree-' + _this._getIndex() + ' .oj-tree-list li[' +
                    attr + '="' + i + '"].oj-tree-type > a > ins.oj-tree-node-icon { ';
                }
...
//     }
      ...
      _fix_scroll: function (obj) {
        ...
        if (c.scrollHeight > c.offsetHeight) {
          ...
          if (t + this._data.core.li_height +
              (c.scrollWidth > c.offsetWidth ? scrollbarWidth : 0) >
              c.offsetHeight) {
            c.scrollTop += ((t - c.offsetHeight) +
                            this._data.core.li_height + 1 +
                            (c.scrollWidth > c.offsetWidth ? scrollbarWidth : 0));
          }
        }
      },
...
//    },
       ...
       *  @private
       */
      _unsetFocus: function () { // undocumented per Design Review
/*    See setFocus()
        if (this._isFocused()) {
...
//   },
      ...
      _keyHandler: {
        ...
        'ctrl+space': function () { // toggle the select state
          if (this._data.ui.hovered) {
            var ev = $.Event('click');
            ev.ctrlKey = true;
        ...
        'shift+space': function () { // select to the hovered node
          if (this._data.ui.hovered) {
            var ev = $.Event('click');
            ev.shiftKey = true;
            this._data.ui.hovered.children('a:eq(0)').trigger(ev);

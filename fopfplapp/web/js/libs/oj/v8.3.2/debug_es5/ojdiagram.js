...
oj.ConversionDiagramDataSource.prototype.getData = function (parentData) {
  if (parentData) {
    ...
    if (childData === undefined && this.childDataCallback) {
      ...
      return Promise.resolve(childNodes).then(function (values) {
  ...
  return Promise.all([nodes, links]).then(function (values) {
    return Promise.resolve({
      nodes: values[0],
      links: values[1]
    });
...
oj.__registerWidget('oj.ojDiagram', $.oj.dvtBaseComponent, {
  ...
  _ProcessOptions: function _ProcessOptions() {
    ...
    this.options._logger = Logger; // convert deprecated node renderers into new format
    ...
    if (this.options.renderer || this._TemplateHandler.getTemplates().nodeContentTemplate || this.options.linkContent || this._TemplateHandler.getTemplates().linkContentTemplate) {
    ...
    if (this.options.nodes) {
      this.options.nodeProperties = this.options.nodeProperties ? this.options.nodeProperties : function (data) {
        return data;
      };
      this.options.linkProperties = this.options.linkProperties ? this.options.linkProperties : function (data) {
        return data;
      };
      this.options.data = new oj.ConversionDiagramDataSource({
  ...
  _IsDraggable: function _IsDraggable() {
    ...
    return dragObj.nodes && Object.keys(dragObj.nodes).length > 0 || dragObj.ports && Object.keys(dragObj.ports).length > 0;
  },
  //* * @inheritdoc */
  _GetComponentRendererOptions: function _GetComponentRendererOptions() {
    return [{
      ...
      path: 'nodeContent/renderer',
      slot: 'nodeContentTemplate'
    }, {
      path: 'nodeContent/focusRenderer',
      slot: 'nodeContentTemplate'
    }, {
      path: 'nodeContent/hoverRenderer',
      slot: 'nodeContentTemplate'
    }, {
      path: 'nodeContent/selectionRenderer',
      slot: 'nodeContentTemplate'
    }, {
      path: 'nodeContent/zoomRenderer',
      slot: 'nodeContentTemplate'
    }, {
      path: 'linkContent/renderer',
      slot: 'linkContentTemplate'
    }, {
      path: 'linkContent/focusRenderer',
      slot: 'linkContentTemplate'
    }, {
      path: 'linkContent/hoverRenderer',
      slot: 'linkContentTemplate'
    }, {
      path: 'linkContent/selectionRenderer',
      slot: 'linkContentTemplate'
    }];
  },
  //* * @inheritdoc */
  ...
  _getContextHandler: function _getContextHandler() {
    ...
    var contextHandlerFunc = function contextHandlerFunc(type, parentElement, rootElement, childContent, dataContext, state, previousState, linkPoints) {
      var context = {
  ...
  _GetChildStyleClasses: function _GetChildStyleClasses() {
    ...
    styleClasses['oj-diagram-node oj-hover'] = [{
      path: 'styleDefaults/nodeDefaults/hoverOuterColor',
      property: 'border-top-color'
    }, {
      path: 'styleDefaults/nodeDefaults/hoverInnerColor',
    ...
    styleClasses['oj-diagram-link'] = [{
      ...
      path: 'styleDefaults/linkDefaults/_hitDetectionOffset',
      property: 'padding'
    }, {
      path: 'styleDefaults/promotedLink/_hitDetectionOffset',
      property: 'padding'
    }];
    ...
    styleClasses['oj-diagram-link oj-hover'] = [{
      path: 'styleDefaults/linkDefaults/hoverOuterColor',
      property: 'border-top-color'
    }, {
      path: 'styleDefaults/linkDefaults/hoverInnerColor',
      property: 'border-bottom-color'
    }];
    styleClasses['oj-diagram-overview'] = [{
      ...
      path: 'styleDefaults/_overviewStyles/overview/padding',
      property: 'padding'
    }];
    styleClasses['oj-diagram-overview-content'] = [{
      path: 'styleDefaults/_overviewStyles/overviewContent/padding',
      property: 'padding'
    }];
    styleClasses['oj-diagram-overview-viewport'] = [{
      path: 'styleDefaults/_overviewStyles/viewport/borderColor',
      property: 'border-color'
    }, {
  ...
  _WrapInlineTemplateRenderer: function _WrapInlineTemplateRenderer(origRenderer, templateName, option) {
    var templateDataSet = this._TemplateHandler.getDataSet(templateName); // get the index of the interval that the zoom value is in.


    var getInterval = function getInterval(zoom, thresholds) {
    ...
    var hasZoomThresholdChange = function hasZoomThresholdChange(state, prevState, thresholds) {
    ...
    if (option === 'zoomRenderer') {
      if (templateDataSet.ojZoomThresholds) {
        try {
          ...
          return function (context) {
            if (hasZoomThresholdChange(context.state.zoom, context.previousState.zoom, thresholds)) {
              return origRenderer(context);
            }
   ...
   * @memberof oj.ojDiagram
   */
  collapse: function collapse(nodeId, vetoable) {
    var result = this._trigger('beforeCollapse', null, {
      nodeId: nodeId
    });
   ...
   * @memberof oj.ojDiagram
   */
  expand: function expand(nodeId, vetoable) {
    var result = this._trigger('beforeExpand', null, {
      nodeId: nodeId
    });
   ...
   */
  getNode: function getNode(nodeIndex) {
    var auto = this._component.getAutomation();
   ...
   */
  getLink: function getLink(linkIndex) {
    var auto = this._component.getAutomation();
   ...
   */
  getPromotedLink: function getPromotedLink(startNodeIndex, endNodeIndex) {
    var auto = this._component.getAutomation();
...
(function () {
  ...
  var nodeShapeParseFunc = DvtAttributeUtils.shapeParseFunction({
    'icon.shape': true
  });

...
function MessageViewModel(context) {
  ...
  this.handleKeydown = this._handleKeydown.bind(this); // Store a reference to the properties for any later use
...
MessageViewModel.prototype._propertyChanged = function (detail) {
  ...
  if (subPropertyChanged(detail, 'message', 'category') || subPropertyChanged(detail, 'displayOptions', 'category')) {
...
MessageViewModel.prototype._registerSwipeHandler = function () {
  if (oj.DomUtils.isTouchSupported() && this._computeCloseAffordance() === 'defaults') {
    ...
    var swipeDirections = oj.DomUtils.getReadingDirection() === 'rtl' ? 'swipeleft swipeup' : 'swiperight swipeup';
    this._hammerSwipe = messageContainerElement.ojHammer(options).on(swipeDirections, function (event) {
      event.preventDefault();
...
MessageViewModel.prototype._closeMessage = function (event) {
  var messageContainerElement = document.getElementById(this._messagesContainerId); // Possible that the oj-message element may be disconnected/removed while close is in progress,
  ...
  if (messageContainerElement) {
    ...
    var mediator = this._operationMediator; // When messages are inlined, the parent oj-messages takes care of open animation, but close
    ...
    var options = this._getAnimateOptionDefaults(action); // eslint-disable-next-line no-undef


    AnimationUtils.startAnimation(messageContainerElement, action, options, this._composite).then(function () {
      ...
      if (event) {
        Object.defineProperty(closeEvent, '_originalEvent', {
          value: event,
          writable: false
        });
      }
...
MessageViewModel.prototype._formatTimestamp = function () {
  ...
  if (!oj.StringUtils.isEmptyOrUndefined(message.timestamp)) {
    ...
    var resolve = busyContext.addBusyState(options); // Our NLS rules restrict that the numerals in the date string should be in latin digits
    ...
    converterPromise.then(function (converter) {
      try {
        var formattedTimestamp = converter.format(message.timestamp); // fine to update the observable directly with resolved value, it will also be updated
        //  in propertyChanged() if timestamp was to change

        this.formattedTimestamp(formattedTimestamp);
      } catch (e) {
        // expect oj.ConverterError if supplied value is not valid
        Logger.info(["JET oj-message id='", MessageViewModel._toSelector(this._composite), "': failed to parse or format the supplied value to message.timestamp='", message.timestamp, "'."].join(''));
      } finally {
        resolve();
      }
    }.bind(this));
...
MessageViewModel.prototype._getConverterPromise = function (timestamp) {
  ...
  if (!dateTimeConverterLoadPromise) {
    Logger.warning(["JET oj-message id='", MessageViewModel._toSelector(this._composite), "': failed to parse message.timestamp='", timestamp, "' because require() is not available"].join(''));
  }
...
MessageViewModel.prototype._isDateToday = function (isoDate) {
  ...
  return todayDate.getUTCFullYear() === suppliedDate.getUTCFullYear() && todayDate.getUTCMonth() === suppliedDate.getUTCMonth() && todayDate.getUTCDate() === suppliedDate.getUTCDate();
...
MessageViewModel.prototype._computeCategory = function () {
  ...
  var translatedCategory = translations && translations.categories ? translations.categories[severity] : '';
...
MessageViewModel.prototype._openMessage = function () {
  var messageContainerElement = document.getElementById(this._messagesContainerId); // Possible that the oj-message element may be disconnected/removed while open is in progress,
  //  guard against this case where messageContainerElement is removed
  ...
  if (messageContainerElement) {
    ...
    if (this._isInlinedChildOfOjMessages()) {
      ...
      this._operationMediator = new OperationMediator(this._composite, 'open'); // notify the oj-messages to show the messages container so initial animation
      // will be seen.
...
MessageViewModel.prototype._isInlinedChildOfOjMessages = function () {
  var messagesAncestor = this._findMessagesAncestor(); // If messages property is specified on oj-messages, the inlined oj-message children in its
...
MessageViewModel.prototype._prepareMessageAtOpen = function () {
  ...
  if (computedSound !== 'none') {
    ...
    this._initAudioContext(); // 1. for inlined children case, we'd play sound a bit before the message actually is opened by
  ...
  this._registerSwipeHandler(); // 2. for inlined children case, we'd be short of few seconds since we dont know when the parent
...
MessageViewModel.prototype._playSound = function (sound) {
  ...
  if (sound !== 'defaults') {
    ...
    audio.src = sound; // Handle any error due to possible invalid URL.

    audio.addEventListener('error', function () {
      Logger.info(["JET oj-message id='", MessageViewModel._toSelector(this._composite), "': failed to load or play media file for URL in property message.sound='", sound, "'."].join(''));
    }.bind(this));
    var promise = audio.play();

    if (promise !== undefined) {
      promise.then(function () {// All is well with autoplay
        ...
        //  loaded.
        Logger.info(["JET oj-message id='", MessageViewModel._toSelector(this._composite), "': failed to load or play specified sound: '", sound, "': error details: '", error].join(''));
      }.bind(this));
    }
  } else if (window.audioContext === undefined) {
    // User agent does not support web audio API
    Logger.info(["JET oj-message id='", MessageViewModel._toSelector(this._composite), "': failed to load or play default sound for message because user agent does\n", "not support Web Audio API'"].join(''));
  } else {
    // Default gain value will be 100% of speaker volume which is fine
    var gainNode = window.audioContext.createGain(); // Default destination is system speakers

    gainNode.connect(window.audioContext.destination); // Use default oscillator node type 'sine' and frequency '440 Hz', suffices for simple beep

    var oscillatorNode = window.audioContext.createOscillator();
    oscillatorNode.connect(gainNode);
    oscillatorNode.start(); // Play for 10 ms
...
OperationMediator.prototype._init = function () {
  ...
  this._element.addEventListener(eventType, this._callback); // Add a busy state for the pending operation.  The busy state resolver will
...
OperationMediator.prototype._getBusyStateDescription = function (element, operation) {
  return [element.nodeName, " identified by '", MessageViewModel._toSelector(element), "' is busy animating on the '", operation, "' operation."].join('');
};
/**
...
OperationMediator.prototype._deliverResolved = function (operation) {
  ...
  this._resolvedQueue = []; // eslint-disable-next-line no-param-reassign
...
OperationMediator.prototype.isOperationPending = function (operation, callback) {
  ...
  if (operation === pendingOperation) {
    ...
    Logger.info(["An oj-message id='", MessageViewModel._toSelector(this._element), "' invoked a '", operation, "' operation while pending animation of the same type of operation. ", 'The second request will be ignored.'].join(''));
    isPending = true;
  } else if (pendingOperation !== 'none') {
    Logger.info(["An oj-message id='", MessageViewModel._toSelector(this._element), "' invoked a '", operation, "' operation while pending animation of a '", pendingOperation, "' operation. The second request will be invoked after the pending ", 'operation completes.'].join('')); // Queue the operation after the pending operation has completed.

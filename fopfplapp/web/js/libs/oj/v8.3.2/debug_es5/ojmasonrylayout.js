...
function MasonryLayoutCommon(elem, rtl, automationEnabled, selectors, styles, callbackInfo) {
  ...
  this._layoutOnEndFunc = callbackInfo.layoutOnEndFunc; // FIX : need to know when layout cycle is starting in order
  // to preserve focus across layout DOM order changes

  this._layoutCycleOnStartFunc = callbackInfo.layoutCycleOnStartFunc;
  this._layoutCycleOnEndFunc = callbackInfo.layoutCycleOnEndFunc; // FIX : need to be able to sort the tiles into their original
  ...
  this._addBusyStateFunc = callbackInfo.addBusyState; // create a non-absolutely positioned div to define the size of the
...
MasonryLayoutCommon.prototype.setup = function (init, reorder) {
  ...
  if (init) {
    ...
    ret = !!this._layout(); // FIX : reorder the tile DOM elements to match the visual layout order

    this._reorderTilesForLayout();
  } else {
    // FIX : add busy state before animating a layout change
    this._busyStateResolveFunc = this._addBusyStateFunc('performing layout (setup)'); // FIX : notify the peer that a layout cycle is starting so that
    // it can save state if it wants

    this._layoutCycleOnStartFunc(); // if this is a refresh, add the transition class BEFORE doing the next layout
...
MasonryLayoutCommon.prototype.destroy = function () {
  ...
  var elem = this._elem; // remove layout positions from the children
...
MasonryLayoutCommon.prototype.resizeTile = function (selector, sizeStyleClass) {
  var elem = this._elem;
  var infolet = elem.querySelector(selector); // add the transition class immediately, but defer the layout call using a
...
MasonryLayoutCommon.prototype.insertTileDomElem = function (tileDomElem, index) {
  var arChildren = this._getTileChildren(); // FIX : need to sort the tiles into their original DOM order
...
MasonryLayoutCommon.prototype.showTile = function (selector) {
  ...
  if (infolet) {
    ...
    this._showingInfolets = true; // don't queue another layout if we're already doing layout and in a phase
    // before the SHOW phase

    var mlcClass = MasonryLayoutCommon;

    if (this._layoutPhase !== mlcClass._PHASE_HIDE && this._layoutPhase !== mlcClass._PHASE_LAYOUT) {
      this._queueRelayout();
    } else {
...
MasonryLayoutCommon.prototype.resizeNotify = function () {
  ...
  if (!this._resizingInfolet && !this._hidingInfolets && !this._showingInfolets) {
    // FIX : add busy state before animating a layout change
    this._busyStateResolveFunc = this._addBusyStateFunc('performing layout (resizeNotify)'); // FIX : notify the peer that a layout cycle is starting so that
    // it can save state if it wants
...
MasonryLayoutCommon.prototype.isAnimationEnabled = function () {
  ...
  if (!this._cachedAnimationEnabled) {
    if (this._automationEnabled) {
      ...
      var agentVersion = mlcClass._agentTypeAndVersion[1]; // check whether the agent supports CSS transitions

      this._animationEnabled = mlcClass._isMinimumAgentMet(agentType, agentVersion, ['gecko', 16, 'trident', 6, 'webkit', 533.1]);
    }
...
MasonryLayoutCommon.prototype.isInLayoutCycle = function () {
  ...
  // we're moving tiles
  return this._layoutPhase != null || this._arMovedInfolets != null && this._arMovedInfolets.length > 0;
};
/**
...
MasonryLayoutCommon.prototype.finishLayoutCycle = function () {
  ...
  this._temporarilyDisableAnimation = true; // remove transition classes for animation

  this._removeStyleClassFromTiles(this._transitionShowFromStyleClass); // remove transition listeners
  ...
  if (this._hideTilesAnimationStarted) {
    ...
    this._showTiles();
  } else if (this._layoutPhase === mlcClass._PHASE_LAYOUT || this._arMovedInfolets != null && this._arMovedInfolets.length > 0) {
    // FIX : when drag and drop reordering, the layoutPhase doesn't
    // get set because we're only running layout itself, so also check whether
    // we're moving tiles
...
MasonryLayoutCommon._getElemSize = function (elem) {
  ...
  var extraWidth = mlcClass._getCSSLengthAsInt(computedStyle.marginLeft) + mlcClass._getCSSLengthAsInt(computedStyle.marginRight);

  var extraHeight = mlcClass._getCSSLengthAsInt(computedStyle.marginTop) + mlcClass._getCSSLengthAsInt(computedStyle.marginBottom);

  return {
    w: elem.offsetWidth + extraWidth,
    h: elem.offsetHeight + extraHeight
  };
};
/**
...
MasonryLayoutCommon._getAgentTypeAndVersion = function (givenUserAgentString) {
  ...
  if (userAgent.indexOf('msie') !== -1 || userAgent.indexOf('trident') !== -1) {
    ...
    agentType = 'webkit'; // 536.26.17 = Mac Desktop Safari 6.0.2
    ...
    agentType = 'gecko'; // rv:5 = Firefox 5
    // rv:2 = Firefox 4
    // rv:1.9 = Firefox 3
    // rv:1.8.1 = Firefox 2
    // rv:1.8 = Firefox 1.5
...
MasonryLayoutCommon.prototype._transitionLayout = function () {
  ...
  var newMovedInfolets = this._layout(); // an infolet should be considered to "move" if it's resized, even if it doesn't change position,
...
MasonryLayoutCommon.prototype._layout = function () {
  ...
  var children = this._getTileChildren(); // FIX : need to sort the tiles into their original DOM order
  // before layout so that layout always starts from the intended order of the
  // children, not from the current layout order


  this._sortTilesOriginalOrderFunc(children); // always recalculate cell size in case the app has specified media queries
  ...
  var arNewPositions = [];
  var arCols = [];
  var rtl = this._rtl; // FIX : need to take container border and padding into account for layout

  var mlcClass = MasonryLayoutCommon;

  var insets = mlcClass._getElemInsets(elem); // FIX : keep track of maxColSpan to define width of layout


  var maxColSpan = 0; // FIX : keep track of tile positions so that tiles can later be
  ...
  for (i = 0; i < children.length; i++) {
    child = children[i]; // if we've saved cell size on the infolet, we need to use it because
    ...
    cellSize = this._cellSize; // keep track of which cells are occupied
    ...
    if (!this._occupancyMap) {
      ...
      // padding and borders
      var effectiveWidth = elem.offsetWidth - insets.paddingLeft - insets.paddingRight - insets.borderLeftWidth - insets.borderRightWidth;
      this._cols = Math.max(Math.floor(effectiveWidth / cellSize.w), 1);

      this._initOccupancyMap(this._cols, this._rows); // FIX : initialize maxColSpan to define width of layout based
    ...
    for (var r = 0; r < this._rows; r++) {
      for (var c = 0; c < this._cols; c++) {
        if (this._fits(c, r, childSpan)) {
          ...
          next = true; // FIX : keep track of tile positions so that tiles can later be
          // sorted into layout order

          arTilePositions.push({
            startCol: c,
            startRow: r,
            tile: child
          });
  ...
  if (cellSize) {
    ...
    if (style) {
      ...
      var height = this._rows * cellSize.h; // get the current sizeDiv width and height
  ...
  for (i = 0; i < children.length; i++) {
    ...
    oldPosition = arOldPositions[i]; // if this is a new tile that masonryLayout didn't know about, then
    ...
    if (oldPosition.top !== '') {
      ...
      // they're so small
      // FIX : in RTL, position tiles using "right" instead of "left"
      if (rtl && parseInt(newPosition.right, 10) !== parseInt(oldPosition.right, 10) || !rtl && parseInt(newPosition.left, 10) !== parseInt(oldPosition.left, 10) || parseInt(newPosition.top, 10) !== parseInt(oldPosition.top, 10)) {
...
MasonryLayoutCommon.prototype._reorderTilesForLayout = function () {
  ...
  for (var i = 0; i < children.length; i++) {
    ...
    var posTile = pos.tile; // if the tiles are different, need to rearrange the DOM

    if (child !== posTile) {
      // notify the element that it's been detached from the DOM BEFORE actually
      // detaching it so that components can save state
      this._subtreeDetachedFunc(posTile); // insert the tile from the layout order into the DOM at this index
      ...
      this._subtreeAttachedFunc(posTile); // rearrange the array of children to match the DOM reorder above
...
MasonryLayoutCommon.prototype._calcPosition = function (child, col, row, childSpan, cellSize, insets) {
  var colSpan = childSpan.colSpan;
  var rowSpan = childSpan.rowSpan;
  var occupancyMap = this._occupancyMap;
  ...
  style.top = newTop + 'px'; // FIX : in RTL, position tiles using "right" instead of "left"
...
MasonryLayoutCommon.prototype._handleTransitionEnd = function (target) {
  ...
  if (doneTransitioning) {
    ...
    this._showingInfolets = false; // FIX : reorder the tile DOM elements to match the visual layout order

    this._reorderTilesForLayout();

    this._layoutOnEndFunc(); // update sizeDiv after the layout transition is complete
...
MasonryLayoutCommon.prototype._calcCellSize = function (child, childSpan) {
  ...
  return {
    w: childSize.w / childSpan.colSpan,
    h: childSize.h / childSpan.rowSpan
  };
};
/**
  ...
  if (!this._arInfoletsToHide) {
    if (this._arFireHideOnEnd) {
      ...
      for (i = 0; i < arFireHideOnEnd.length; i++) {
        infolet = arFireHideOnEnd[i]; // remove layout position from the child
    ...
    this._transitionStart(false); // update size style classes AFTER starting transition, but BEFORE doing layout
    ...
    if (this._arInfoletsToResize) {
      ...
      for (i = 0; i < arInfoletsToResize.length; i += 2) {
        ...
        var oldSizeStyleClass = this._getSizeStyleClassNameFunc(infolet); // if we add a _transitionResizeFromStyleClass in the future, we'll have
        ...
        if (this.isAnimationEnabled()) {
          ...
          // the transition runs
          infolet._afrCellSize = this._cellSize; // This must be called before the style class change in order to work
          // consistently.
...
MasonryLayoutCommon.prototype._handleShowTransitionEnd = function (target) {
  ...
  if (!this._arInfoletsToShow) {
    this._layoutPhase = null; // FIX : notify the callback that the layout cycle is done
    // FIX : resolve busy state after laying out

    this._layoutCycleOnEndFunc(this._resolveBusyStateFunc); // FIX : if we got another layout request while we were in the layout cycle,
...
(function () {
  ...
  var _OJ_MASONRYLAYOUT_TILE_TRANSITION_SHOW_FROM_CLASS = 'oj-masonrylayout-tile-transition-show-from'; // log warning message when "disabled" property set

  var _WARNING_DISABLED_OPTION = "JET MasonryLayout: 'disabled' property not supported"; // Context Menu: menu item ids
  ...
  oj.__registerWidget('oj.ojMasonryLayout', $.oj.baseComponent, {
    ...
    _ComponentCreate: function _ComponentCreate() {
      ...
      elem.addClass('oj-masonrylayout oj-component'); // log warning message when "disabled" property set
    ...
    refresh: function refresh() {
      ...
      this._super(); // if RTL has changed, just destroy and recreate the MasonryLayoutCommon
    ...
    _NotifyShown: function _NotifyShown() {
      this._super(); // perform a deferred layout
    ...
    _NotifyContextMenuGesture: function _NotifyContextMenuGesture(menu, event, eventType) {
      ...
      this._prepareContextMenuBeforeOpen(event); // FIX : only allow reordering if context menu was
     ...
     * @private
     */
    _setup: function _setup(isInit) {
      // Private, not an override (not in base class).
      var self = this; // if masonryLayout is detached or hidden, we can't layout correctly, so
      ...
      if (isInit) {
        ...
        if (!this._mlCommon) {
          ...
          this._applyTileStyleClass(); // save the original order of the tiles, because the DOM order may
          ...
          this._mlCommon = new MasonryLayoutCommon(elem[0], this._bRTL, Config.getAutomationMode() === 'enabled', selectors, styles, callbackInfo);
        }
        ...
        this._applyTileStyleClass(); // FIX : tear down and setup the reorder handles for the
        ...
        this._setupReorderHandlesForElem(children); // make sure that any new tiles have an assigned index in the
      ...
      if (isInit) {
        ...
        oj.DomUtils.addResizeListener(elem[0], this._handleResizeFunc, _RESIZE_LISTENER_COLLAPSE_EVENT_TIMEOUT);
      }
    },
    ...
    _destroy: function _destroy() {
      ...
      this._handleResizeFunc = null; // restore tiles to their original order and delete stored indices
     ...
     */
    resizeTile: function resizeTile(selector, sizeStyleClass) {
      var mlCommon = this._mlCommon; // FIX : if we're already in a layout cycle, finish it immediately
      ...
      var jqElem = $(selector);
      var elem = jqElem[0]; // Fix  - throw error if the selector is invalid

      if (!elem) {
        throw new Error('JET MasonryLayout: Unable to resize tile because ' + 'no tile found for selector: ' + selector);
      }

      var prevSizeStyleClass = _getSizeStyleClassName(elem); // FIX : if the style class isn't changing, don't do anything;
      ...
      if (sizeStyleClass === prevSizeStyleClass) {
        throw new Error('JET MasonryLayout: Unable to resize child ' + selector + ' to style class ' + sizeStyleClass + ' because ' + sizeStyleClass + ' is already applied.');
     ...
     */
    insertTile: function insertTile(selector, index) {
      var mlCommon = this._mlCommon; // FIX : if we're already in a layout cycle, finish it immediately
      ...
      if (!elem) {
        throw new Error('JET MasonryLayout: Unable to insert tile because ' + 'no tile found for selector: ' + selector);
      } // add the oj-masonrylayout-tile style class to the inserted tile


      jqElem.addClass(_OJ_MASONRYLAYOUT_TILE); // fire beforeInsert event
      ...
      if (bSuccess !== false) {
        elem._jetDataTileInsertIndex = index; // only need to initially hide inserts if they will be animated later,
        ...
        style.top = '-1px'; // FIX : in RTL, position tiles using "right" instead of "left"
        ...
        mlCommon.insertTileDomElem(elem, index); // notify the element that it's been attached to the DOM
     ...
     */
    removeTile: function removeTile(selector) {
      var mlCommon = this._mlCommon; // FIX : if we're already in a layout cycle, finish it immediately
      ...
      var infolet = jqInfolet[0]; // Fix  - throw error if the selector is invalid

      if (!infolet) {
        throw new Error('JET MasonryLayout: Unable to remove tile because ' + 'no tile found for selector: ' + selector);
      ...
      if (oj.FocusUtils.containsFocus(infolet)) {
        ...
        var index = children.indexOf(infolet); // save the previous infolet, because this one is being deleted
     ...
     * @private
     */
    _showTileOnEnd: function _showTileOnEnd(elem) {
      var jqElem = $(elem);
      var index = elem._jetDataTileInsertIndex; // eslint-disable-next-line no-param-reassign

      delete elem._jetDataTileInsertIndex; // FIX : setup the reorder handle for the inserted tile
     ...
     * @private
     */
    _hideTileOnEnd: function _hideTileOnEnd(elem) {
      var jqElem = $(elem); // FIX : tear down the reorder handle for the removed tile
      ...
      jqElem.removeClass(_OJ_MASONRYLAYOUT_TILE); // notify the element that it's been detached from the DOM BEFORE actually
      // detaching it so that components can save state

      Components.subtreeDetached(elem);
      var parentNode = elem.parentNode;
      parentNode.removeChild(elem); // remove the tile from the original order

      this._removeTileOriginalOrder(elem); // fire remove event
     ...
     * @private
     */
    _startAnimation: function _startAnimation(elem, action) {
      var defaults = (ThemeUtils.parseJSONFromFontFamily('oj-masonrylayout-option-defaults') || {}).animation;
      var animation = defaults ? defaults[action] : null; // eslint-disable-next-line no-undef
     ...
     * @private
     */
    _layoutOnEnd: function _layoutOnEnd() {
      var i;
      ...
      if (this._arResizingTiles) {
        ...
        for (i = 0; i < arResizingTiles.length; i += 3) {
          ...
          var sizeStyleClass = arResizingTiles[i + 2]; // fire resize event
     ...
     * @private
     */
    _layoutCycleOnStart: function _layoutCycleOnStart() {
      this._inLayoutCycle = true;
      this._layoutStartActiveDomElem = null; // store events to fire when the layout cycle ends

      if (!this._eventsToFire) {
        this._eventsToFire = [];
      }

      var activeDomElem = document.activeElement; // if the currently focused element is a child of the  masonryLayout,
     ...
     * @private
     */
    _layoutCycleOnEnd: function _layoutCycleOnEnd(resolveBusyState) {
      this._inLayoutCycle = false;
      var elem = this.element[0]; // if we saved the focus element before layout, try to restore focus now

      if (this._layoutStartActiveDomElem) {
        var layoutStartActiveDomElem = this._layoutStartActiveDomElem;
        this._layoutStartActiveDomElem = null; // after deleting a tile, focus the tile that took its position in the layout
        ...
        if (this._deletingTileWithFocusPrev) {
          ...
          if (tile && oj.DomUtils.isAncestor(elem, tile)) {
            ...
            var index = children.indexOf(tile); // because we saved the id of the prev tile, try to restore focus
     ...
     * @private
     */
    _destroyMLCommon: function _destroyMLCommon() {
      var mlCommon = this._mlCommon;
     ...
     * @private
     */
    _canCalculateSizes: function _canCalculateSizes() {
     ...
     * @private
     */
    _getTileElements: function _getTileElements(excludeDropSite) {
      var elem = this.element;
     ...
     * @private
     */
    _saveTilesOriginalOrder: function _saveTilesOriginalOrder() {
      var arTiles = this._getTileElements();

      if (arTiles) {
        for (var i = 0; i < arTiles.length; i++) {
          var tile = arTiles[i]; // only assign original index to tile if it hasn't already been assigned,
     ...
     * @private
     */
    _checkTilesOriginalOrder: function _checkTilesOriginalOrder() {
      var arTiles = this._getTileElements();

      if (arTiles) {
        for (var i = 0; i < arTiles.length; i++) {
          var tile = arTiles[i]; // insert the tile at its current index
     ...
     * @private
     */
    _restoreTilesOriginalOrder: function _restoreTilesOriginalOrder() {
      var children = this._getTileElements();
      ...
      for (var i = 0; i < children.length; i++) {
        var child = children[i];
        var sortedChild = sortedChildren[i]; // if the tiles are different, need to rearrange the DOM

        if (child !== sortedChild) {
          // notify the element that it's been detached from the DOM BEFORE actually
          // detaching it so that components can save state
          Components.subtreeDetached(sortedChild); // insert the tile from the layout order into the DOM at this index

          var parentNode = child.parentNode;
          parentNode.insertBefore(sortedChild, child); // @HTMLUpdateOK
          // notify the element that it's been attached to the DOM

          Components.subtreeAttached(sortedChild); // rearrange the array of children to match the DOM reorder above
     ...
     * @private
     */
    _insertTileOriginalOrder: function _insertTileOriginalOrder(insertedTile, index) {
      var arTiles = this._getTileElements();
      ...
      if (arTiles) {
        for (var i = 0; i < arTiles.length; i++) {
          var tile = arTiles[i]; // increment the index of each shifted tile
     ...
     * @private
     */
    _removeTileOriginalOrder: function _removeTileOriginalOrder(removedTile) {
      if (removedTile._jetDataMasonryOriginalOrder) {
        var arTiles = this._getTileElements();

        if (arTiles) {
          for (var i = 0; i < arTiles.length; i++) {
            var tile = arTiles[i]; // decrement the index of each shifted tile

            if (tile._jetDataMasonryOriginalOrder && tile._jetDataMasonryOriginalOrder > removedTile._jetDataMasonryOriginalOrder) {
              tile._jetDataMasonryOriginalOrder -= 1;
            }
          }
     ...
     * @private
     */
    _applyTileStyleClass: function _applyTileStyleClass() {
      var elem = this.element;
      var children = elem.children();
    ...
    _initMenu: function _initMenu(newVal) {
      ...
      var menu = this._GetContextMenu(); // check for contextmenu attribute on the root element
      ...
      if ($m.length > 0) {
        ...
        dm.$container = $m; // have a context menu
    ...
    _applyMenu: function _applyMenu() {
      ...
      var $menuContainer = this._menu.$container;
      var self = this; // Check if the menu container tag name is 'OJ-MENU' for custom element context menu
     ...
     * @private
     */
    _prepareContextMenuBeforeOpen: function _prepareContextMenuBeforeOpen(e) {
      var elem = this.element; // FIX : only allow reordering if context menu was
      ...
      var bDisable; // set menu items disabled state
      ...
      if (this._menu.usermenu) {
        ...
        if (elemCut) {
          ...
          bDisable = false; // disable "cut" if this tile has already been cut
        ...
        if (elemPasteBefore) {
          ...
          bDisable = false; // disable "pasteBefore" if this is the tile that was cut or if the cut
        ...
        if (elemPasteAfter) {
          ...
          bDisable = false; // disable "pasteAfter" if this is the tile that was cut or if the cut
     ...
     * @private
     */
    _buildContextMenuLabel: function _buildContextMenuLabel(command, useOjOption) {
      // convert to the translation key convention
      var transKey = _MENU_TRANSLATION_MAP[command];
      var textNode = document.createTextNode(this.getTranslatedString(transKey)); // for custom elements, no <a> tag is required
     ...
     * @private
     */
    _menuCut: function _menuCut(obj) {
      if (obj) {
     ...
     * @private
     */
    _menuPaste: function _menuPaste(obj, pasteBefore) {
     ...
     * @private
     */
    _doPaste: function _doPaste(cutTile, pasteTile, pasteBefore) {
      var fromIndex = cutTile._jetDataMasonryOriginalOrder - 1;
      var jqCutTile = $(cutTile); // fire beforeReorder event
      ...
      if (bSuccess !== false) {
        ...
        var mlCommon = this._mlCommon;
        mlCommon.setup(true); // fire reorder event
     ...
     * @private
     */
    _getTileIndex: function _getTileIndex(tile) {
      // index should be with respect to "real" children, not including the dropsite,
      // so exclude the dropsite from the children array
      var children = this._getTileElements(true); // need to sort the children because the index is with respect to the original order
     ...
     * @private
     */
    _setupReorderHandles: function _setupReorderHandles() {
      var elem = this.element;
      var children = elem.children();

      this._setupReorderHandlesForElem(children);

      elem.on('dragstart' + this.reorderHandleEventNamespace, this._handleDragStartFunc).on('dragenter' + this.reorderHandleEventNamespace, this._handleDragEnterFunc).on('dragover' + this.reorderHandleEventNamespace, this._handleDragOverFunc).on('dragleave' + this.reorderHandleEventNamespace, this._handleDragLeaveFunc).on('dragend' + this.reorderHandleEventNamespace, this._handleDragEndFunc).on('drop' + this.reorderHandleEventNamespace, this._handleDropFunc);
    },
     ...
     * @private
     */
    _setupReorderHandlesForElem: function _setupReorderHandlesForElem(jqElem) {
     ...
     * @private
     */
    _tearDownReorderHandles: function _tearDownReorderHandles() {
      var elem = this.element;
      var children = elem.children(); // tear down reorder handles on the children of the root elem in case
     ...
     * @private
     */
    _tearDownReorderHandlesForElem: function _tearDownReorderHandlesForElem(jqElem) {
      var options = this.options; // tear down for tiles themselves

      var tiles = jqElem.filter(options.reorderHandle);
      tiles.removeAttr(_DRAGGABLE).removeClass(_OJ_DRAGGABLE); // tear down for descendants of tiles
    ...
    _handleDragStart: function _handleDragStart(event) {
      var options = this.options;
      var target; // FIX : if drag didn't start on reorder handle, don't
      // reorder tile
      ...
      if (!this._bDragging) {
        ...
        if (tile) {
          ...
          tile._jetDataMasonryDragSourceIndex = index; // fire beforeReorder event
    ...
    _handleDragEnter: function _handleDragEnter(event) {
      ...
      if (relatedTarget) {
        ...
        enteringMasonryLayout = elemUnderPoint && (elemUnderPoint === elem || oj.DomUtils.isAncestor(elem, elemUnderPoint));
      }
    ...
    _handleDragOver: function _handleDragOver(event) {
      ...
      this._dragMove(originalEvent.pageX, originalEvent.clientX, originalEvent.clientY); // necessary to allow a drop
     ...
     * @private
     */
    _handleDragLeave: function _handleDragLeave(event) {
      // FIX : only process drag and drop event if masonryLayout
      // started the drag
      if (!this._bDragging) {
        return;
      ...
      if (relatedTarget) {
        ...
        var elemUnderPoint = document.elementFromPoint(originalEvent.clientX, originalEvent.clientY);
        leavingMasonryLayout = elemUnderPoint && elemUnderPoint !== elem && !oj.DomUtils.isAncestor(elem, elemUnderPoint);
      }
     ...
     * @private
     */
    _clearDragStartHideTileTimeout: function _clearDragStartHideTileTimeout() {
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _handleDragEnd: function _handleDragEnd(event) {
      // FIX : if the drag and drop was a very short duration,
      // the hide tile timeout may not have fired yet, so clear it now
      this._clearDragStartHideTileTimeout(); // FIX : only process drag and drop event if masonryLayout
      // started the drag
      ...
      if (!this._bDropping) {
        ...
        if (draggedTile && this._dropSite) {
          ...
          var elem = this.element[0]; // tile was not moved outside
    ...
    _handleDrop: function _handleDrop(event) {
      ...
      this._drop(this._draggedTile, originalEvent.pageX, originalEvent.pageY); // stop the browser from redirecting
     ...
     * @private
     */
    _dragStart: function _dragStart(tile, pageX, pageY, dataTransfer) {
      ...
      style.top = tileStyle.top; // FIX : in RTL, position tiles using "right" instead of "left"
      ...
      this._dragOffset = dragOffset; // add the oj-drag class BEFORE creating the drag feedback image

      $(tile).addClass('oj-drag'); // eslint-disable-next-line no-param-reassign

      dataTransfer.effectAllowed = 'move';
      dataTransfer.setData('text/html', tile.outerHTML); // @HTMLUpdateOK

      dataTransfer.setDragImage(tile, dragOffset.left, dragOffset.top); // hide the original tile in a timeout AFTER creating the drag feedback image
      ...
      this._dragStartHideTileTimeout = setTimeout(function () {
        ...
        self._busyStateResolveFunc = null; // notify the element that it's been hidden
     ...
     * @private
     */
    _dragMove: function _dragMove(pageX, clientX, clientY) {
      this._bMouseMoved = true; // FIX : if the dragged tile hasn't been hidden yet, don't
      ...
      if (!this._bDragMoveTransition) {
        ...
        var tileUnderPoint = _findContainingTile(elemUnderPoint, elem); // FIX : only allow reordering if tile has reorder
        // handle
        ...
        if (tileUnderPoint) {
          ...
          var options = this.options;
          var reorderHandle = options.reorderHandle; // FIX : the tile has a reorder handle if: 1) the tile
        ...
        if (tileUnderPoint == null || tileUnderPoint === this._dropSite || tileUnderPoint === this._draggedTile) {
          this._prevTileUnderPoint = null;
          this._prevCellUnderPoint = null;
        } else if (this._prevTileUnderPoint && this._prevCellUnderPoint && this._prevTileUnderPoint !== tileUnderPoint) {
          var cellUnderPoint = this._getCellPosition(clientX, clientY);

          if (cellUnderPoint && cellUnderPoint.row === this._prevCellUnderPoint.row && cellUnderPoint.col === this._prevCellUnderPoint.col) {
            bSkipReorder = true;
          }
        }

        if (tileUnderPoint && !bSkipReorder && tileUnderPoint !== this._dropSite && tileUnderPoint !== this._draggedTile && tileHasReorderHandle) {
          ...
          var newNextSibling = _getNextElement(this._dropSite); // only need to layout if the dropSite has actually changed order


          if (oldNextSibling !== newNextSibling) {
            // only record the start of a drag move transition if the layout
            // actually changed
            var mlCommon = this._mlCommon; // save the tileUnderPoint and cellUnderPoint of the current layout
            ...
            this._bDragMoveTransition = mlCommon.setup(false, true); // if the tile has moved, save tmpTileUnderPoint and tmpCellUnderPoint
     ...
     * @private
     */
    _handleDragMoveTransitionEnd: function _handleDragMoveTransitionEnd() {
      this._bDragMoveTransition = false;
    },
     ...
     * @private
     */
    _drop: function _drop(tile, pageX, pageY) {
      this._bDropping = true;
      var elem = this.element[0];
      var dropSite = this._dropSite;
      this._dropSite = null;
      this._prevTileUnderPoint = null;
      this._prevCellUnderPoint = null; // notify the element that it's been detached from the DOM BEFORE actually
      // detaching it so that components can save state

      Components.subtreeDetached(tile); // FIX : calling DOM replaceChild() seems to break in IE11 on Win7, so instead call
      // equivalent insertBefore() and removeChild()

      elem.insertBefore(tile, dropSite); // @HTMLUpdateOK

      elem.removeChild(dropSite); // notify the element that it's been attached to the DOM

      Components.subtreeAttached(tile); // eslint-disable-next-line no-param-reassign

      tile._jetDataMasonryOriginalOrder = dropSite._jetDataMasonryOriginalOrder;
      var style = tile.style;
      style.display = ''; // notify the element that it's been shown

      Components.subtreeShown(tile); // immediately transform the page absolute coords to ones relative to the
      // ojmasonrylayout

      var offset = _getRelativePosition(elem);

      var dragOffset = this._dragOffset;
      style.top = pageY - dragOffset.top - offset.top + _PX; // FIX : in RTL, position tiles using "right" instead of "left"
     ...
     * @private
     */
    _handleDragEndTransitionEnd: function _handleDragEndTransitionEnd() {
      this._bDragEndTransition = false;
      this._bDragging = false;
      this._bDropping = false;
      this._bMouseMoved = false;
      ...
      var toIndex = this._getTileIndex(tile); // fire reorder event
     ...
     * @private
     */
    _getCellPosition: function _getCellPosition(clientX, clientY) {
      var mlCommon = this._mlCommon;

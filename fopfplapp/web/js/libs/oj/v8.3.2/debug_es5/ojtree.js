...
oj.TreeDndContext.prototype.handleDnDOptionChange = function (newval) {
  ...
  if (reorder !== dnd.reorder || dragFromEnabled !== dnd.dragFromEnabled) {
    if (!dnd.reorder && !dnd.dragFromEnabled) {
      this.component._getContainerList().find('li').removeClass(cons._OJ_DRAGGABLE).removeClass(cons._OJ_DRAG).removeClass(cons._OJ_VALID_DROP).removeAttr('draggable');
    }
  }
...
oj.TreeDndContext.prototype.initDnDOpts = function () {
  ...
  var ot;
  var cb;
  var n; // Handle tree reorder via DnD
  ...
  dnd.pureReorder = dnd.reorder && !dnd.dragFromEnabled; // if (dnd.reorder || dnd.dropToEnabled) {
...
oj.TreeDndContext.prototype.initDnD = function () {
  ...
  if (!oj.TreeDndContext._dragMarker) {
    ...
    $("<div class='oj-tree-drop-marker'><span class='oj-tree-drop-ptr oj-component-icon'>&#160;</span></div>").css('pointer-events', 'none') // prevent mouse events on marker, so that
  ...
  if (dnd.reorder || dnd.dragFromEnabled) {
    ...
    $(div).on('dragend', dnd.fnDragEnd); // If here after tree is constructed, it is because of a dnd option change

    if ($ul.length) {
      $ul.find('li').addClass(cons._OJ_DRAGGABLE).addClass(cons._OJ_DRAG).addClass(cons._OJ_VALID_DROP).attr('draggable', 'true');
    }
  }
...
oj.TreeDndContext.prototype._dragStart = function (e) {
  ...
  var ret;
  var dnd = this._dnd;
  var vars = dnd.vars; // this._trace(this._elemId + " _dragStart(" + $node.attr("id") + ") - dragged=" + dnd.nodeDraggedId +
  //            "  nodeLastEnter=" +
  ...
  if (!dnd.reorder && !dnd.dragFromEnabled || $node.hasClass(TreeUtils._OJ_DISABLED) || this.component._isTreeDisabled()) {
  ...
  $nodes = this.component.isSelected($node) ? this.component._getNode(null, true) : $nodes = $node; // Set app defined dataTypes to the node data
  ...
  this._setDragImage(dt, $nodes); //  Call app callback in drag options


  if (dnd.dragStartCallback) {
    var adata = this._getDraggedItemArray($nodes);

    ret = dnd.dragStartCallback(e, {
      item: adata
    });
...
oj.TreeDndContext.prototype._dragEnter = function (e) {
  ...
  var bClean = false;
  var ret; // Check if entering the internal placeholder node (created after
  ...
  if (!dnd.foreign && !dnd.reorder || dnd.foreign && this._isDtType(dt, cons._DND_INTERNAL_DT_REORDER)) {
    this._dndClean();
  ...
  if (dnd.foreign) {
    bForeignTree = this._isDtType(dt, cons._DND_INTERNAL_DT); // this._trace("_dragEnter - " + (bForeignTree? "Foreign Tree" :
  ...
  vars.r.removeClass(cons._OJ_INVALID_DROP).addClass(cons._OJ_VALID_DROP).addClass(cons._OJ_ACTIVE_DROP); //  Call dragEnter callback in the drop options if defined
...
oj.TreeDndContext.prototype._dragOver = function (e) {
  ...
  this._dndPlacement(e, $node); // this._dndDrag(e) ;
...
oj.TreeDndContext.prototype._dragLeave = function (e) {
  ...
  var bNode = $node && $node.length > 0 && $node.hasClass('oj-tree-node');
  var dt = e.originalEvent.dataTransfer; // this._trace(this._elemId + " _dragLeave(" + e.target.tagName + ", " + (bNode? $node.attr("id"): "nonode") +
...
oj.TreeDndContext.prototype._dndEnter = function () {
  var dnd = this._dnd;
  var vars = dnd.vars; // this._trace("_dndEnter vars.o = " + vars.o + " vars.r = " +  vars.r) ;

  dnd.prepared = false; // no prepMoveBlk object yet

  var ret = this._dndPrepare(); // var ms  = s["openTimeout"] ;     // not publicly exposed
...
oj.TreeDndContext.prototype._dndFinishReorder = function (e) {
  var dnd = this._dnd;
  var vars = dnd.vars; // For a pure reorder on the same tree, we can just move the nodes.

  if (dnd.dragStarted && dnd.reorder && _typeof(vars.o) === 'object' && vars.o.length > 0) {
    this.component._moveNode(vars.o, vars.r, vars.last_pos, oj.DomUtils.isMetaKeyPressed(e)); // Don't want dragEnd() to do any node(s) removal since they have just been
    // moved  by tree
...
oj.TreeDndContext.prototype._dndClean = function () {
  ...
  dnd.off = false;
  dnd.prepared = false;
  dnd.openTimerId = false; // dnd.i1             = false;
  // dnd.i2             = false;
...
oj.TreeDndContext.prototype._dndCleanAll = function () {
  var dnd = this._dnd;
  var $div; //  Clean out the old dnd settings
...
oj.TreeDndContext.prototype._dndShowMarker = function () {
  ...
  var r = false; // this._trace("dndShowMarker entry  vars.o = " + vars.o + "  vars.r = " + vars.r + " dnd.place = " + dnd.place, 1) ;
  ...
  if (dnd.place === 'after' && dnd.after || dnd.place === 'inside' && dnd.inside || dnd.place === 'before' && dnd.before || dnd.place === 'first') {
    r = dnd.place;
  }
...
oj.TreeDndContext.prototype._moveMarkerUI = function ($obj, styles) {
  // eslint-disable-next-line no-param-reassign
  styles.left += 'px'; // eslint-disable-next-line no-param-reassign
...
oj.TreeDndContext.prototype._dndStopDrag = function () {
  var dnd = this._dnd;
  var vars = dnd.vars;
  var cons = oj.TreeDndContext; //      if (vars.sli)  {
  //        clearInterval(vars.sli);
  //      }
  //      if (vars.sti)   {
  //        clearInterval(vars.sti);
...
oj.TreeDndContext.prototype._setDragImage = function (dt, $nodes) {
  var len = $nodes.length; // If only one node being dragged, use the default drag image
...
oj.TreeDndContext.prototype._clearDropClasses = function ($var) {
  ...
  if ($var) {
    $var.removeClass(cons._OJ_VALID_DROP).removeClass(cons._OJ_INVALID_DROP).removeClass(cons._OJ_ACTIVE_DROP);
  }
};
/**
...
oj.TreeDndContext.prototype._reset = function () {
  ...
  _dnd.cof = false;
  _dnd.cw = false;
  _dnd.ch = false; // _dnd.i1         = false ;      // not currently used
...
/**
  ...
  *  @constructor
  */
function TreeUtils() {} // css class names
...
(function () {
  ...
  */
  ...
  var OJT_LOADING = 'oj-tree-loading'; // var OJT_LAST_SELECTED = 'oj-tree-last-selected';

  var OJT_TITLE = 'oj-tree-title';
  var OJT_TYPE = 'oj-tree-type';
  var OJT_INACTIVE = 'oj-tree-inactive'; // ojTree disclosure class names

  var OJ_DISC = 'oj-tree-icon oj-tree-disclosure-icon oj-component-icon oj-clickable-icon-nocontext oj-default'; //  WAI-ARIA
  ...
  var WA_ACTIVEDESCENDANT = 'aria-activedescendant'; // var WA_MULTISELECTABLE = 'aria-multiselectable';

  var WA_LABELLEDBY = 'aria-labelledby'; //  Data names

  var OJT_CHILDREN = 'oj-tree-children'; //  Data source in use
  ...
  var DS_HTML = 4;
  var DS_NONE = 0;
  var DS_ERROR = -1; // Context Menu item id's
  ...
  var TRANSKEY_LOADING = 'stateLoading'; // var TRANSKEY_NEWNODE = 'labelNewNode';
  // var TRANSKEY_MULTISEL = 'labelMultiSelection';

  var TRANSKEY_NODATA = 'labelNoData';
  var USER_UL_ID_PREFIX = 'oj-tree-existing-markup-'; //  Attr's we ignore when creating a list of user attr's
  ...
  oj.__registerWidget('oj.ojTree', $.oj.baseComponent, {
     ...
     * @memberof oj.ojTree
     */
    collapse: function collapse(node, skipAnim) {
      skipAnim = skipAnim || false; // Get css value $treeAnimationDuration.  This is needed because if the value is zero
      ...
      if (!node.length || node === -1 || !node.hasClass(TreeUtils._OJ_EXPANDED) || this._data.core.locked || node.hasClass(TreeUtils._OJ_DISABLED)) {
        return false;
      }
     ...
     *  @private
     */
    _slide: function _slide(node, bSlideUp) {
      var $ul;
      var hNow;
      var hFinal;
      $ul = $(node.children('ul')); // For the windows safari platform only we have a problem setting "transitionend" events,
     ...
     * @private
     */
    _getElemHeight: function _getElemHeight(el) {
      ...
      var h = el.offsetHeight || el.scrollHeight; // Restore DOM
    ...
    _transitionEndHandler: function _transitionEndHandler(e) {
      var $ul = $(e.target);
      var node = $ul.closest('li'); // For the windows safari platform only we have a problem with "transitionend" events,
     ...
     *  @private
     */
    _transitionEnd: function _transitionEnd($ul, node) {
      var op;
    ...
    /** Collapses a node and all its descendants.  Triggers a "collapseall" event.
      ...
      * @memberof oj.ojTree
    */
    collapseAll: function collapseAll(node, anim) {
      var origTarg = node || -1;
     ...
     * @memberof oj.ojTree
     */
    expand: function expand(node, skipAnim) {
     ...
     * @memberof oj.ojTree
     */
    expanded: function expanded(nodes, skipAnim) {
      var _this = this;
     ...
     * @memberof oj.ojTree
     */
    expandAll: function expandAll(node, anim) {
     ...
     * @memberof oj.ojTree
     */
    toggleExpand: function toggleExpand(node, skipAnim) {
      node = this._getNode(node);
     ...
     * @memberof oj.ojTree
     */
    deselect: function deselect(node) {
     ...
     * @memberof oj.ojTree
     */
    create: function create(refnode, position, data) {
      var node;
      var i;
      var ar = $.isArray(data);
      ...
      if (position !== 'before') {
        ...
        for (i = data.length - 1; i >= 0; i--) {
          ...
          ar.unshift(node[0]); // if parseJson() applied the select class, select the node
     ...
     * @memberof oj.ojTree
     */
    rename: function rename(node, text) {
     ...
     * @memberof oj.ojTree
     */
    getPath: function getPath(node, idMode) {
      var p = [];
     ...
     *  @memberof oj.ojTree
     */
    move: function move(node, refnode, position, iscopy) {
     ...
     *  @memberof oj.ojTree
     */
    setType: function setType(node, str) {
    ...
    */
    getContextByNode: function getContextByNode(node) {
      ...
      if (bNode || bTree) {
        return {
          ...
          item: bNode ? node[0] : null,
          // <li>
          node: bNode ? node : false,
          // jQuery wrapped <li>
     ...
     * @memberof oj.ojTree
     */
    getPrevSibling: function getPrevSibling(node) {
     ...
     * @memberof oj.ojTree
     */
    scrollIntoView: function scrollIntoView(obj, alignTo, setActive) {
      var node;
    ...
    _setOption: function _setOption(key, newval, flags) {
      ...
      if (key === 'selection') {
        ...
        newval = this._getSelected(); // optionChange event will be fired when super is called below.
        ...
        this._handleDisabledChanged(newval); // end "disabled"
     ...
     *  @private
     */
    _compareSelectionValues: function _compareSelectionValues(value1, value2) {
      var bDiff = false;
      var bInList = false;
     ...
     *  @private
     */
    _resolveBusyContext: function _resolveBusyContext() {
      if (this._animationResolve) {
     ...
     *  @private
     */
    _isOnBusyStack: function _isOnBusyStack(op, id) {
     ...
     *  @private
     */
    _removeFromBusyStack: function _removeFromBusyStack(op, id) {
     ...
     *  @private
     */
    _resolveIfBusyStackEmpty: function _resolveIfBusyStackEmpty() {
     ...
     *  @private
     */
    _clearTree: function _clearTree() {
      ...
      this._$container.unbind('.oj-tree').undelegate('.oj-tree').removeData('oj-tree-instance-id').find("[class^='oj-tree']").addBack().attr('class', function () {
      ...
      $(document).unbind('.oj-tree-' + n).undelegate('.oj-tree-' + n); // Remove the constructed tree from the DOM.
     ...
     *  @private
     */
    _getNode: function _getNode(obj, allowMultiple) {
     ...
     *  @private
     */
    _getPrev: function _getPrev(obj, strict) {
     ...
     *  @private
     */
    _getNext: function _getNext(obj, strict) {
      obj = this._getNode(obj);
     ...
     *  @private
     */
    _getParent: function _getParent(obj) {
      obj = this._getNode(obj);
     ...
     *  @private
     */
    _getChildren: function _getChildren(obj) {
      obj = this._getNode(obj);
     ...
     *  @private
     */
    _getNodeElem: function _getNodeElem(node) {
     ...
     *  @private
     */
    _reference: function _reference(node, o) {
    ...
    /**
      ...
      *  @private
      */
    _applyDefaults: function _applyDefaults(to, from) {
     ...
     *  @private
     */
    _handleDisabledChanged: function _handleDisabledChanged(newval) {
      var curState;
     ...
     *  @private
     */
    _treeDisable: function _treeDisable(lstate) {
      lstate = lstate || false;
      ...
      if (lstate) {
        ...
        this._$container_ul.removeClass(TreeUtils._OJ_DISABLED).css('opacity', this._data.ui.opacity);
      }
    },
     ...
     *  @private
     */
    _isTreeDisabled: function _isTreeDisabled() {
     ...
     *  @private
     */
    _handleSelectionOptionChange: function _handleSelectionOptionChange(newSels) {
      var sels = newSels.slice(0); // working copy

      var cur = []; // xfer valid selection args to this

      var _this = this; // Convert the new array to jQuery wrapped node elements (thereby changing any
     ...
     *  @private
     */
    _prepare_move: function _prepare_move(o, r, pos, cb, isCb) {
      var p = {}; // new prepMoveBlk object

      r = r === -1 || !r ? -1 : this._getNode(r); // Get tree component's involved
      ...
      if (!isCb && this._data.core.prepMoveBlk.o && this._data.core.prepMoveBlk.o[0] === p.o[0] && this._data.core.prepMoveBlk.r[0] === p.r[0] && this._data.core.prepMoveBlk.p === p.p) {
      ...
      this._data.core.prepMoveBlk = p; // this._emitEvent(this._data.core.prepMoveBlk, "prepare_move", true);
     ...
     *  @private
     */
    _checkMove: function _checkMove() {
     ...
     *  @private
     */
    _renameNode: function _renameNode(node, text) {
      node = this._getNode(node);
     ...
     *   @private
     */
    _moveNode: function _moveNode(obj, ref, position, isCopy, isPrepared, skipCheck) {
      if (!isPrepared) {
        obj = this._getNode(obj);
      ...
      try {
        ...
        if (!obj.op.find('> ul > li').length) {
          obj.op.removeClass('oj-expanded oj-collapsed').removeAttr(WA_EXPANDED).addClass(OJT_LEAF).children('ul').remove();
        }
     ...
     *  @private
     */
    _getMoveBlk: function _getMoveBlk() {
     ...
     *  @private
     */
    _setType: function _setType(node, str) {
     ...
     *  @private
     */
    _canTypedNodeMove: function _canTypedNodeMove($node, func) {
      var ret = true;
      ...
      if (types) {
        ...
        if (types) {
          ...
          if ((types[type] && typeof types[type][func] !== 'undefined' || types.default && typeof types.default[func] !== 'undefined') && this._check(func, $node) === false) {
            ret = false;
          }
        }
      }
    ...
    _check: function _check(rule, obj, opts) {
      ...
      var ty = this._getType(obj); // var d = 0;
     ...
     * @private
     */
    _cleanNode: function _cleanNode(obj) {
      var dndClasses;
      var draggableAttr;
     ...
     *  @private
     */
    _createNode: function _createNode(obj, position, js, callback, isLoaded) {
      obj = obj || -1;
      obj = this._getNode(obj);
      ...
      this.__rollback(); //  Parse the JSON
     ...
     *  @private
     */
    _expand: function _expand(obj, callback, skipAnim) {
      obj = this._getNode(obj);
      ...
      if (!this._isLoaded(obj)) {
        obj.children('a').addClass(OJT_LOADING); // Load the node. This does not return if an Ajax request is made and ends in failure.
     ...
     *  @private
     */
    _expandAll: function _expandAll(obj, animate, originalObj) {
      var origTarg = obj || -1;
     ...
     *  @private
     */
    _select: function _select(node, check, e) {
      ...
      var prevSelections = this.options.selection.slice(0); //  Establish whether the select should happen, and deselect where necessary.
      ...
      var isRange = selRangeMod !== false && e && e[selRangeMod + 'Key'] && ui.lastSelected && this._data.ui.lastSelected[0] !== node[0] && ui.lastSelected.parent()[0] === node.parent()[0];
      var proceed = true;
      var t = this;

      if (check) {
        if (disSelChildren && isMultiple && (node.parentsUntil('.oj-tree', 'li').children('a.oj-selected').length || node.children('ul').find('a.oj-selected:eq(0)').length)) {
          return false;
        }
        ...
        switch (!0) {
          case isRange:
            ...
            node = node[node.index() < ui.lastSelected.index() ? 'nextUntil' : 'prevUntil']('.oj-tree-last-selected').addBack();
          ...
          case ui.touchEvent && selectMode === -1:
            // touch and multi-select mode
     ...
     *  @private
     */
    _deselectAll: function _deselectAll(context) {
      if (this._data.core.locked) {
        return;
      }

      var ret = context ? $(context).find('a.oj-selected').parent() : this._$container.find('a.oj-selected').parent();
      ret.not('.oj-disabled');
     ...
     * @private
     */
    _setSelected: function _setSelected(nodes, e) {
      if (this._data.core.locked) {
        return;
      }
     ...
     *  @private
     */
    _handleNodeTapClick: function _handleNodeTapClick(event) {
      event.preventDefault();
      event.currentTarget.blur();

      if (!$(event.currentTarget).hasClass(OJT_LOADING)) {
        this._setFocus(); //  Handle case where tree does not have focus and a node has been selected.
     ...
     *  @private
     */
    _disclosureHover: function _disclosureHover(elem, bHover) {
      elem = $(elem);
    ...
    _dehover: function _dehover() {
      ...
      this._$container_ul.removeAttr(WA_ACTIVEDESCENDANT); //       if (this._data.ui.hovered[0] === p[0]) {
     ...
     *  @private
     */
    _refresh: function _refresh(node) {
     ...
     *  @private
     */
    _refresh_core: function _refresh_core(node) {
      var origTarg = node || -1;
     ...
     *  @private
     */
    _refresh_ui: function _refresh_ui(obj) {
      this._saveSelected();
     ...
     *  @private
     */
    _getSelected: function _getSelected(context) {
     ...
     *  @private
     */
    _set_text: function _set_text(obj, val) {
      obj = this._getNode(obj);
    ...
    /**
       ...
       *  @private
       */
    _loadNodes: function _loadNodes() {
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _load_node: function _load_node(obj, successCallback, errorCallback) {
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _isLoaded: function _isLoaded(obj) {
      // Dummy function overriden by data methods
      return true;
    },
     ...
     *  @private
     */
    _load_node_DS: function _load_node_DS(obj, successCallback, errorCallback) {
      var _this = this;

      this._load_node_tree(obj, function () {
        _this._emitEvent({
          obj: _this._getNode(obj)
        }, 'load_node', true);
     ...
     *   @private
     */
    _isLoaded_DS: function _isLoaded_DS(obj) {
      // var s = this.options["data"]["json"];
      obj = this._getNode(obj); // return (obj === -1 || !obj ) || (!s["ajax"] && !this._data.ds.progressiveRender && !$.isFunction(s["data"]))

      return obj === -1 || !obj || obj.is('.oj-expanded, .oj-tree-leaf') || obj.children('ul').children('li').length > 0;
    },
     ...
     *  @private
     */
    _refresh_DS: function _refresh_DS(obj) {
      obj = this._getNode(obj); // var s = this.options["data"]["json"];
     ...
     *  @private
     */
    _load_node_J: function _load_node_J(obj, successCallback, errorCallback) {
      var _this = this;

      this._loadNodeJson(obj, // node or -1 for tree
      function () {
        // success func
        _this._emitEvent({
          obj: _this._getNode(obj)
     ...
     *   @private
     */
    _isLoaded_J: function _isLoaded_J(obj) {
      var s = this.options.data;
      obj = this._getNode(obj);
      return obj === -1 || !obj || !s.ajax && !this._data.ds.progressiveRender && !$.isFunction(s.data) || obj.is('.oj-expanded, .oj-tree-leaf') || obj.children('ul').children('li').length > 0;
    },
     ...
     *   @private
     */
    _load_node_H: function _load_node_H(obj, successCallback, errorCallback) {
      var _this = this;

      this._loadNodeHtml(obj, function () {
        // success func
        _this._emitEvent({
          obj: _this._getNode(obj)
        }, 'load_node', true);
     ...
     *  @private
     */
    _isLoaded_H: function _isLoaded_H(obj) {
      var s = this.options.data;
      var data = null;
      var ajax = null;
      ...
      obj = this._getNode(obj);
      return obj === -1 || !obj || !ajax && !$.isFunction(data) || obj.is('.oj-expanded, .oj-tree-leaf') || obj.children('ul').children('li').length > 0;
    },
     ...
     * @private
     */
    _saveSelected: function _saveSelected() {
      var ui = this._data.ui;
      ui.toSelect = [];
      ui.selected.each(function () {
        if (this.id) {
    ...
    */
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _load_node_tree: function _load_node_tree(obj, successCallback, errorCallback) {
     ...
     */
    _JsonDSToJson: function _JsonDSToJson(parentKey, node) {
      var arJson = [];
      var ds = this._tds;
      var range = {};
      var rslt = {
     ...
     *  @private
     */
    _refresh_json: function _refresh_json(obj) {
      obj = this._getNode(obj);
     ...
     *  @private
     */
    _loadNodeJson: function _loadNodeJson(obj, successCallback, errorCallback) {
      ...
      if (obj && obj !== -1 && (this._data.ds.progressiveRender || this._data.ds.progressiveUnload) && !obj.is('.oj-expanded, .oj-tree-leaf') && obj.children('ul').children('li').length === 0 && obj.data(OJT_CHILDREN)) {
      ...
      switch (!0) {
        ...
        case !data && !!ajax || !!data && !!ajax && obj && obj !== -1:
          // AJAX error function
          errorFunc = function errorFunc(x, status, e) {
          ...
          successFunc = function successFunc(d, status, x) {
            ...
            self._removeFromBusyStack('j', this.id); // Reget the options without our updated ajax changes to avoid forever loop
          ...
          this._addBusyState("Tree (id='" + this._elemId + "') : node id='" + nodeId + "' is lazyloading."); // console.log("ajax JSON load (" + nodeId + ") . . .  busyStack=" + this._arrayToStr(this._busyStack)) ;
     ...
     *  @private
     */
    _parseJson: function _parseJson(js, obj, isRecurse) {
      var d = false;
        ...
        $.each(js.data, function (i, m) {
          ...
          if (m.icon) {
            if (m.icon.indexOf('/') === -1) {
              ...
              tmp2.children('ins').css('background', "url('" + m.icon + "') center center no-repeat");
            }
          }
        });
     ...
     *   @private
     */
    __getJson: function __getJson(obj, liAttr, aAttr, isCallback) {
      var result = [];
      var s = this.options;
      ...
      obj.each(function () {
        ...
        $.each(liAttr, function (i, v) {
          ...
          if (tmp2 && tmp2.length && tmp2.replace(/oj-tree[^ ]*/ig, '').length) {
            tmp2 = (' ' + tmp2).replace(/ oj-tree[^ ]*/ig, '').replace(/ oj-[^ ]*/ig, '').replace(/\s+$/ig, ' ').replace(/^ /, '').replace(/ $/, '');
        ...
        a.each(function () {
          ...
          if (aAttr.length || t.children('ins').get(0).style.backgroundImage.length || t.children('ins').get(0).className && t.children('ins').get(0).className.replace(/oj-tree[^ ]*|$/ig, '').length) {
            title = _this.getText(t);
            $.each(aAttr, function (k, z) {
              sAttr = (' ' + (t.attr(z) || '')).replace(/ oj-tree[^ ]*/ig, '').replace(/ oj-[^ ]*/ig, '').replace(/\s+$/ig, ' ').replace(/^ /, '').replace(/ $/, '');
            ...
            if (t.children('ins').get(0).className.replace(/oj-tree[^ ]*|$/ig, '').replace(/^\s+$/ig, '').length) {
              sAttr = t.children('ins').get(0).className.replace(/oj-tree[^ ]*|$/ig, '').replace(/ oj-[^ ]*/ig, '').replace(/\s+$/ig, ' ').replace(/^ /, '').replace(/ $/, '');
            ...
            if (t.children('ins').get(0).style.backgroundImage.length) {
              sAttr = t.children('ins').get(0).style.backgroundImage.replace('url(', '').replace(')', '');
     ...
     */
    _noteUserAttrs: function _noteUserAttrs($node) {
     ...
     *  @private
     */
    _correct_state: function _correct_state(obj) {
      obj = this._getNode(obj);
      ...
      obj.removeClass('oj-collapsed oj-expanded').removeAttr(WA_EXPANDED).addClass(OJT_LEAF).children('ul').remove();
    ...
    */
     ...
     *   @private
     */
    _loadNodeHtml: function _loadNodeHtml(obj, successCallback, errorCallback) {
      var s = this._getOptions().data; // work on a copy of the options to avoid
      ...
      switch (!0) {
        ...
        case !data && !ajax:
          if (!obj || obj === -1) {
            this._$container_ul.empty().append(this._data.html.cloneMarkup) // @HTMLUpdateOK
            ...
            .end().filter('a').children('ins:first-child').not('.oj-tree-node-icon').addClass(OJT_NICON).addClass(OJT_ICON);

            this._$container_ul.find('li').children('ins:first-child').addClass(OJ_DISC); //  No point in checking for empty parent <ul> because we don't have a
        ...
        case !!data && !ajax || !!data && !!ajax && (!obj || obj === -1):
          if (!obj || obj === -1) {
            ...
            this._$container_ul.find('li.oj-tree-leaf ins:first-child').removeClass(OJ_DISC).removeClass(OJT_NICON).addClass(OJT_ICON); //  Add the <a> text <span> for hover/click styling
        ...
        case !data && !!ajax || !!data && !!ajax && obj && obj !== -1:
          obj = this._getNode(obj); // AJAX error function
          ...
          successFunc = function successFunc(d, status, x) {
            ...
            if (d) {
              ...
              if (obj === -1 || !obj) {
                self._$container_ul.empty().append(d.children()) // @HTMLUpdateOK
                .find('li, a').filter(function () {
                  return !self.firstChild || !self.firstChild.tagName || self.firstChild.tagName !== 'INS';
                }).prepend("<ins class='oj-tree-icon'>&#160;</ins>") // @HTMLUpdateOK
                .end().filter('a').children('ins:first-child').not('.oj-tree-node-icon').addClass(OJT_NICON).addClass(OJT_ICON);

                self._$container_ul.find('li.oj-tree-leaf ins:first-child').removeClass(OJ_DISC).removeClass(OJT_NICON).addClass(OJT_ICON);
                ...
                .children('ul').find('li, a').filter(function () {
                  return !self.firstChild || !self.firstChild.tagName || self.firstChild.tagName !== 'INS';
                }).prepend("<ins class='oj-tree-icon'>&#160;</ins>") // @HTMLUpdateOK
                .end().filter('a').children('ins:first-child').not('.oj-tree-node-icon').addClass(OJT_NICON).addClass(OJT_ICON);
                obj.removeData('oj-tree-is-loading');
                obj.find('li.oj-tree-leaf ins:first-child').removeClass(OJ_DISC).removeClass(OJT_NICON).addClass(OJT_ICON);
                parent = obj;
              } //  Look for parents with empty children <ul> list (lazy loading),
              //  and add the closed class to make it a parent.


              self._handleHtmlParentNoChildren(parent); //  Add the <a> text <span> for hover/click styling


              self._insertHtmlTextSpan(parent); //  Escape all Ajax loaded node text for security
          ...
          this._addBusyState("Tree (id='" + this._elemId + "') : node id='" + nodeId + "' is lazyloading."); // console.log("ajax HTML load (" + nodeId + ") . . . busyStack=" + this._arrayToStr(this._busyStack));
     ...
     *   @private
     */
    _handleHtmlParentNoChildren: function _handleHtmlParentNoChildren(parent) {
      var lazy = parent.find(parent.is('ul') ? 'li ul' : 'ul').filter(function () {
        return !this.firstChild || this.childNodes.length === 0 || this.childNodes.length === 1 && this.firstChild.nodeType === 3;
      });
      $.each(lazy, function () {
     ...
     *   @private
     */
    _removeEmptyUL: function _removeEmptyUL(parent) {
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _loadHtmlString: function _loadHtmlString(s, obj, successCallback, errorCallback) {
      var parent;
      ...
      if (s && s !== '' && s.toString && s.toString().replace(/^[\s\n]+$/, '') !== '') {
        ...
        if (obj === -1 || !obj) {
          this._$container_ul.empty().append(s.children()) // @HTMLUpdateOK
          .find('li, a').filter(function () {
            return !this.firstChild || !this.firstChild.tagName || this.firstChild.tagName !== 'INS';
          }).prepend("<ins class='oj-tree-icon oj-tree-disclosure-icon'>&#160;</ins>") // @HTMLUpdateOK
          .end().filter('a').children('ins:first-child').not('.oj-tree-node-icon').addClass(OJT_NICON).addClass(OJT_ICON);

          this._$container_ul.find('li.oj-tree-leaf ins:first-child').removeClass(OJ_DISC).removeClass(OJT_NICON).addClass(OJT_ICON);

          parent = this._$container_ul; //  No point in checking for empty parent <ul> because we don't have a
          ...
          .end().filter('a').children('ins:first-child').not('.oj-tree-node-icon').addClass('oj-tree-node-icon').addClass('oj-tree-icon');
          obj.removeData('oj-tree-is-loading');
          parent = obj; //  If the "default" node type has been defined, add the def type to
     ...
     *  @private
     */
    _insertHtmlTextSpan: function _insertHtmlTextSpan(elem) {
     ...
     *  @private
     */
    _addDefType: function _addDefType(obj) {
      if (this._data.types.defType) {
        var s = this.options.types;
     ...
     *  @private
     */
    _save_opened: function _save_opened() {
      var _this = this;
     ...
     *  @private
     */
    _reload_nodes: function _reload_nodes(bIsCallback) {
      var _this = this;
      ...
      if (this._data.core.toExpand.length) {
        this._data.core.toLoad = this._data.core.toLoad.concat(this._data.core.toExpand); // Remove duplicates
     ...
     *   @private
     */
    setTheme: function setTheme(themeName, themeUrl) {
      if (!themeName) {
        return false;
      }
    ...
    */
     ...
     *   @private
     */
    _hideIcons: function _hideIcons() {
     ...
     *   @private
     */
    _enableKeys: function _enableKeys() {
    ...
    _initTree: function _initTree() {
      ...
      this._initUIOpts(); //      this._initCrrmOpts() ;      // not in V1
      ...
      this._initDataSource(); //      this._initCrrm() ;          // not in V1
     ...
     *  @private
     */
    _emitEvent: function _emitEvent(data, evname, bInternal) {
      if (!evname || $.type(evname) !== 'string') {
        return undefined;
      }

      if (this._data.core.locked === true && evname !== 'unlock' && evname !== 'isLocked' && evname !== 'lock') {
        return undefined;
     ...
     *  @private
     */
    _fireOptionChangeEvent: function _fireOptionChangeEvent(key, prevVal, newVal, origEvent) {
      if (key === 'selection') {
        if (newVal == null) {
    ...
    _start: function _start() {
      ...
      this._data.core.selectMode === -1 ? " aria-multiselectable='true'" : '') + ' ' + WA_LABELLEDBY + "='" + this._elemId + "'" + "><li class='oj-tree-last oj-tree-leaf'><ins class='oj-tree-icon'>&#160;</ins><a class='oj-tree-loading' href='#'><ins class='oj-tree-icon'>&#160;</ins>" + this._getString(TRANSKEY_LOADING) + '</a></li></ul>');
      ...
      if (this._isTouch) {
        ...
        this._$container.delegate('.oj-tree-list ins.oj-tree-disclosure-icon', 'touchend.ojtree', function (event) {
      ...
      this._$container.delegate('.oj-tree-list ins.oj-tree-disclosure-icon', 'click.ojtree', function (event) {
        var trgt = $(event.target); // if(trgt.is("ins") && event.pageY - trgt.offset().top < this.data.core.li_height) { this.toggle_node(trgt); }

        this.toggleExpand(trgt);
      }.bind(this)).delegate('.oj-tree-list ins.oj-tree-disclosure-icon', 'mousedown', function (event) {
        ...
        var trgt = $(event.target);
        trgt.removeClass('oj-default').removeClass(TreeUtils._OJ_HOVER).addClass(TreeUtils._OJ_SELECTED);
      }.bind(this)).delegate('.oj-tree-list ins.oj-tree-disclosure-icon', 'mouseup', function (event) {
        var trgt = $(event.target);
      ...
      }.bind(this)).bind('dblclick.ojtree', function () {
        var sel;
      ...
      this._$container_ul.focus(function () {
        ...
        var n; // Tree has gained focus, highlight a node as current.
        ...
        if (n) {
          this.hover(n);
          this._data.ui.lastHovered = null; // want color change for selected inactive to selected
    ...
    _initUI: function _initUI() {
      ...
      if (this._isTouch) {
        ...
        this._$container.delegate('.oj-tree-list a', 'touchend.ojtree', $.proxy(function (event) {
      ...
      this._$container.delegate('.oj-tree-list a', 'click.ojtree', function (event) {
        this._data.ui.touchEvent = false;

        this._handleNodeTapClick(event);
      }.bind(this)).delegate('.oj-tree-list a', 'mouseenter.ojtree', function (event) {
        if (!$(event.currentTarget).hasClass(OJT_LOADING)) {
          this.hover(event.target);
        }
      }.bind(this)).delegate('.oj-tree-list a', 'mouseleave.ojtree', function (event) {
        if (!$(event.currentTarget).hasClass(OJT_LOADING)) {
          this.dehover(event.target);
        }
      }.bind(this)).delegate('.oj-tree-list .oj-tree-disclosure-icon', 'mouseenter.ojtree', function (event) {
        if (!$(event.currentTarget).hasClass(OJT_LOADING)) {
          this._disclosureHover(event.target, true);
        }
      }.bind(this)).delegate('.oj-tree-list .oj-tree-disclosure-icon', 'mouseleave.ojtree', function (event) {
        if (!$(event.currentTarget).hasClass(OJT_LOADING)) {
          this._disclosureHover(event.target, false);
        }
      }.bind(this)).bind('_treereopen', function () {
        this._reselect();
      }.bind(this)).bind('_treeget_rollback', function () {
        this.dehover();
      ...
      */
      .bind('ojcollapse', function (event, ui) {
      ...
      }.bind(this)).bind('ojremove', function (event, ui) {
        // node delete
        ...
        clk.each(function () {
          _this.deselect(this); // Remove lastHovered if it refers to the node removed
      ...
      }.bind(this)).bind('ojmove', function (event, ui) {
        var data = ui.data;
        var copy = data.cy;
     ...
     *  @private
     */
    _initTreeData: function _initTreeData() {
     ...
     *  @private
     */
    _initJsonData: function _initJsonData() {
     ...
     *  @private
     */
    _initHtmlData: function _initHtmlData() {
     ...
     *  @private
     */
    _processExistingMarkup: function _processExistingMarkup() {
      ...
      if (!this._data.html.markup_ul) {
        ...
        this._data.html.markup_div = $("<div id='" + USER_UL_ID_PREFIX + this._getIndex() + "' style='display:none'>").append(this._data.html.markup_ul); // @HTMLUpdateOK
     ...
     *  @private
     */
    _transformElemIds: function _transformElemIds(markup) {
     ...
     *  @private
     */
    _unTransformElemIds: function _unTransformElemIds(markup) {
      var $nodes = $(markup).find('li');
      $nodes.each(function (i, node) {
     ...
     *  @private
     */
    _initThemes: function _initThemes() {
      /*
     ...
     *  @private
     */
    _initTypes: function _initTypes() {
      var s = this.options.types;
        ...
        $.each(types, function (i, tp) {
          ...
          if (tp.icon.image || tp.icon.position) {
            if (i === 'default') {
              ...
              iconsCss += '.oj-tree-' + _this._getIndex() + ' .oj-tree-list li.oj-tree-type a > .oj-tree-node-icon { ';
              iconsCss += _this._addTypeCss(tp, iconsCss);
              iconsCss += '} ';
              iconsCss += '.oj-tree-' + _this._getIndex() + ' .oj-tree-list li[' + attr + '="oj-tree-deftype"].oj-tree-type > a ins.oj-tree-node-icon { ';
            } else if (tp.icon.image) {
              iconsCss += '.oj-tree-' + _this._getIndex() + ' .oj-tree-list li[' + attr + '="' + i + '"].oj-tree-type > a > ins.oj-tree-node-icon { ';
            }
      ...
      }, this)).bind('ojbefore', function (e, data) {
        var func = data.func;
        var item = data.item;
     ...
     *  @private
     */
    _addTypeCss: function _addTypeCss(tp) {
      var css = '';
     ...
     *  @private
     */
    _getDndContext: function _getDndContext() {
      if (!this._TreeDndContext) {
    ...
    */
    ...
    _initMenu: function _initMenu(newVal) {
      var menu; // Temp code for M6 to clear .  Once the new methods
    ...
    _NotifyContextMenuGesture: function _NotifyContextMenuGesture(menu, event, eventType) {
      ...
      if (this._data.menu.usermenu && (this._data.menu.$elemPaste || this._data.menu.$elemPasteBefore || this._data.menu.$elemPasteAfter)) {
     ...
     *  @private
     */
    _initCoreOpts: function _initCoreOpts() {
     ...
     *  @private
     */
    _initUIOpts: function _initUIOpts() {},

    /**
     *  Process data source options
     *  @private
     */
    _initDSOpts: function _initDSOpts(bInit) {
      var s = this.options.data;
      this._data.ds.type = DS_NONE;
      this._data.html.useExistingMarkup = false;
      this._data.html.cloneMarkup = false;
      this._loadFailureIds = {}; // _data.html.markup_ul, _data.html.markup and  _data.html.markup_div
     ...
     *  @private
     */
    _initTreeDSOpts: function _initTreeDSOpts() {},

    /**
     *  Process json_data options
     *  @private
     */
    _initJsonOpts: function _initJsonOpts() {},

    /**
     *  Process html_data options
     *  @private
     */
    _initHtmlOpts: function _initHtmlOpts() {},
     ...
     *  @private
     */
    _initMenuOpts: function _initMenuOpts() {},
    //   /**
     ...
     *  @private
     */
    _initTypeOpts: function _initTypeOpts() {
      var o = this.options.types; //     For V1, there are no default options that we publish
      //     if (typeof o === "object") {
      ...
      if (_typeof(o) === 'object') {
        this._applyDefaults(o, {
          attr: this._data.types.defaults.attr
        });
      }
    },
     ...
     *  @private
     */
    _initExpandedOpts: function _initExpandedOpts() {
    ...
    _initData: function _initData() {
      ...
      var data = this._data; //  Core

      data.core = {
        initLoaded: [],
        selectMode: 1,
        //  0 = no selection, 1, 2 ..., or -1 for unlimited
        load_open: false,
        li_height: 0,
        toExpand: false,
        toLoad: false,
        prepMoveBlk: {},
        // for the _moveNode function
        suppressSelectEvent: false,
        // used for batching up optionChange events
      ...
      data.ui = {
        selected: $(),
        // selected node jquery list
        lastSelected: false,
        hovered: null,
        // currently hovered
        lastHovered: null,
        // last hovered before blur
        disclosureClick: false,
        // used for 1st time disclosure click
        toSelect: null,
        // initial selection in options:selection
        opacity: 1,
        // used by disable/_lock()
        spacebar: false,
        // true if select caused by keybd (toggles)
        focused: false,
        // tree has focus
        animDurDiv: null,
        // div used to get animation duration
      ...
      data.types.defaults = {
        ...
        maxChildren: -1,
        // defines max number of root nodes
        ...
        useData: false,
        // whether to use $.data     TDO
        attr: 'type',
        // attr name in <li> where type is stored
        types: {
          // a list of types
          default: {
            // the default type
            maxChildren: -1,
            maxDepth: -1,
            validChildren: 'all' // Bound functions - you can bind any other function here
    ...
    _fix_scroll: function _fix_scroll(obj) {
      ...
      if (c.scrollHeight > c.offsetHeight) {
        ...
        if (t + this._data.core.li_height + (c.scrollWidth > c.offsetWidth ? scrollbarWidth : 0) > c.offsetHeight) {
          c.scrollTop += t - c.offsetHeight + this._data.core.li_height + 1 + (c.scrollWidth > c.offsetWidth ? scrollbarWidth : 0);
        }
      }
    },
     ...
     *  @private
     */
    _unsetFocus: function _unsetFocus() {// undocumented per Design Review
     ...
     *  @private
     */
    _getContainerList: function _getContainerList() {
    ...
    _keyHandler: {
      ...
      'shift+home': function shiftHome(event) {
        ...
        if (hover) {
          ...
          var nodes = this._$container_ul.find('li:visible'); // Batch up the optionChange/selection events so that the final result is
      ...
      'shift+pgdn': function shiftPgdn(event) {
        ...
        if (hover) {
          ...
          var l = this._$container_ul.find('li:visible'); // Will batch up the optionChange/selection events so that the final result is fired in
     ...
     *   @private
     */
    _applyMenu: function _applyMenu() {
      if (!this._data.menu.usermenu) {
        return;
      ...
      $menuContainer.on('ojselect', this._handleContextMenuSelect.bind(this)); // If there are any ojTree built in menu item ids, construct the menu items
     ...
     * @private
     */
    _buildContextMenuListItem: function _buildContextMenuListItem(cmd) {
     ...
     * @private
     */
    _buildContextMenuLabel: function _buildContextMenuLabel(cmd) {
     ...
     *  @private
     */
    _isLoadFailureId: function _isLoadFailureId(id) {
      id = id === -1 ? '-1' : id;
     ...
     *  @private
     */
    _noteLoadFailureId: function _noteLoadFailureId(id, add) {
      id = id === -1 ? '-1' : id;
     ...
     *  @private
     */
    _crrm_cut: function _crrm_cut(obj) {
     ...
     *  @private
     */
    _crrm_copy: function _crrm_copy(obj) {
     ...
     *  @private
     */
    _crrm_paste: function _crrm_paste(obj, menuId) {
      obj = this._getNode(obj);
      ...
      var position = 'inside';
      if (menuId === 'ojtreepasteafter') position = 'after';else if (menuId === 'ojtreepastebefore') position = 'before'; // Handle cut nodes
     ...
     *  @private
     */
    _crrm_move_node: function _crrm_move_node(obj, ref, position, isCopy, isPrepared, skipCheck) {
     ...
     *  @private
     */
    _crrm_remove: function _crrm_remove(obj) {
      obj = this._getNode(obj, true); //      var p    = this._getParent(obj),
      //          prev = this._getPrev(obj);

      this.__rollback();

      this.remove(obj); //         if (obj !== false) {
     ...
     *  @private
     */
    _crrm_rename: function _crrm_rename(obj) {
      // var f = this._emitEvent;
      obj = this._getNode(obj);

      this.__rollback(); // eslint-disable-next-line no-unused-vars


      this._crrm_showInput(obj, function (_obj, newName, oldName) {//  Emit rename event
        //                                    f.call(this, {
        //                                                   "obj"       : _obj,
        //                                                   "title"     : newName,
        //                                                   "prevTitle" : oldName
     ...
     *  @private
     */
    _crrm_showInput: function _crrm_showInput(obj, callback) {
      obj = this._getNode(obj);
      var rtl = this._isRtl; // w   = this.options["crrm"]["inputWidthLimit"],       // applyDefaults() not done for V1
     ...
     *  @private
     */
    _crrm_create: function _crrm_create(obj, position, js, callback, skipRename) {
      var _this = this;
     ...
     *  @private
     */
    _applyEmptyText: function _applyEmptyText() {
     ...
     *  @private
     */
    _processSubId: function _processSubId(locator) {
      // Parent node
     ...
     *  @private
     */
    _getSubIdFromNodeElem: function _getSubIdFromNodeElem(elem) {
     ...
     *  @private
     */
    _isOptExpandAll: function _isOptExpandAll() {
     ...
     *  @private
     */
    _getNodeHeight: function _getNodeHeight() {
     ...
     *  @private
     */
    _getAnimDuration: function _getAnimDuration() {
      var ret = 0;
     ...
     *  @private
     */
    _varCopy: function _varCopy(obj, s) {
      var o = {};
      o[s] = obj[s];

...
(function (dvt) {
  ...
  dvt.ThematicMap.prototype._createLayers = function () {
    ...
    this._linkLayers.setAriaRole('region', true); // BUG: JET-31433 - IN THEMATIC MAP: INLINE TEMPLATES DEMO, THE MARKER STAYS HIGHLIGHTED
    ...
    if (dvt.Agent.isTouchDevice()) {
      ...
      this._touchEventLayer.setStyle({
        display: 'none'
      });
    }
  };
  ...
  dvt.ThematicMap.prototype.PreRender = function () {
    dvt.ThematicMap.superclass.PreRender.call(this); // Set map changed flag before parsing and rendering

    this._bBaseMapChanged = false; // 3 cases we need to handle
    // 1. Initial render
    // 2. New base map
    ...
    if (!this.IsResize() && this._pzcContainer) {
      ...
      this._layers = [];
      this._zooming = true;
      this._panning = true; // save a copy of the old pzc for animation
      ...
      this._bListenersRemoved = false; // clear data tips from previous event handlers
  ...
  dvt.ThematicMap.prototype.Render = function () {
    dvt.ThematicMap.superclass.Render.call(this); // Create a new container and render the component into it
    ...
    if (!this._areaLayer) return; // Animation Support
    // Stop any animation in progress

    this.StopAnimation(true);
    var bounds = new dvt.Rectangle(0, 0, this.getWidth(), this.getHeight()); // 2 types of animations can occur where #2 needs to animate out the old data
    ...
    if (this.Animation) {
      this.EventManager.hideTooltip(); // Disable event listeners temporarily

      this.EventManager.removeListeners(this);
      this._bListenersRemoved = true; // Start the animation
    ...
    this._pzcContainer = pzcContainer; // For mashup case, the component who calls this last will get their keyboard handler set on the wrapping div
  ...
  dvt.ThematicMap.prototype._render = function (pzcContainer, cpContainer) {
    this._updateAriaForLayers(); // Add all containers


    cpContainer.addChild(this._areaLayers);
    cpContainer.addChild(this._dataAreaLayers);
    if (this.isMarkerZoomBehaviorFixed()) pzcContainer.addChild(this._dataPointLayers);else cpContainer.addChild(this._dataPointLayers);
    ...
    this._setInitialKeyboardFocus(); // Do not set min and max zoom before calling zoom to fit on map
    ...
    this._pzc.setPanningEnabled(true); // Zoom to fit before initial render animations so animations will look correct
  ...
  dvt.ThematicMap.prototype.updateLayer = function (dataLayerOptions, parentLayer, isAreaDataLayer) {
    ...
    this._bRendered = false;
    this.StopAnimation(true); // Parse new data layer

    var parser = new DvtThematicMapJsonParser(this);
    parser.ParseDataLayers([dataLayerOptions], this.getLayer(parentLayer), this._areaLayer.getLayerName(), isAreaDataLayer);
    this._bRendered = true; // Reset initially focused object with updated data items

    this._setInitialKeyboardFocus(); // Set component keyboard listener last for mashup case


    this.getCtx().setKeyboardFocusArray([this]); // reset zoom limits since we could now have an isolated area after data layer update
  ...
  dvt.ThematicMap.prototype._transformContainers = function (pzcMatrix) {
    ...
    mat = mat.translate(pzcMatrix.getTx(), pzcMatrix.getTy()); // this._dataPointLayers zoom transforms handled by markers to avoid scaling marker filter effects
  ...
  dvt.ThematicMap.prototype._updateAnimator = function (event, bRecenterObjs) {
    ...
    if (animator) {
      ...
      if (this.isMarkerZoomBehaviorFixed()) animator.addProp(dvt.Animator.TYPE_MATRIX, this._dataPointLayers, this._dataPointLayers.getMatrix, this._dataPointLayers.setMatrix, transMat);
  ...
  dvt.ThematicMap.prototype.HandleZoomEvent = function (event) {
    ...
    switch (subtype) {
      ...
      case 'zoomed':
        ...
        if (zoom != null) {
          var pzcMatrix = this._pzc.getContentPane().getMatrix(); // null out animator for Flash. Temp fix until  is done.
  ...
  dvt.ThematicMap.prototype._zoomData = function () {
    ...
    var animation;
    if (!dvt.Agent.isEnvironmentTest()) animation = new dvt.Animator(this.getCtx(), .3);
    if (bounds.w > 0 && bounds.h > 0) this._pzc.zoomToFit(animation, bounds);else this._pzc.zoomToFit(animation, this._areaLayer.getLayerDim());
  ...
  dvt.ThematicMap.prototype._setCenteredObjsMatrix = function (matrix) {
    this._currentAnimMatrix = matrix; // update centered markers and images
  ...
  dvt.ThematicMap.prototype._handleSelectionEvent = function (event) {
    if (this._clickedDataLayerId) {
      ...
      event['clientId'] = this._clickedDataLayerId; // Save fit to region object

      if (this._clickedObject && !(this._clickedObject instanceof dvt.SimpleMarker || this._clickedObject instanceof dvt.ImageMarker)) this._zoomToFitObject = this._clickedObject;
    } else {
  ...
  dvt.ThematicMap.prototype.OnAnimationEnd = function () {
    ...
    this._renderCompleted(); // After the initial render animations we should perform any additional zooms


    if (this._initialZooming) this._zoomData(); // Initial Highlighting

    if (this.Options['highlightedCategories'] && this.Options['highlightedCategories'].length > 0) this.highlight(this.Options['highlightedCategories']); // Restore event listeners
  ...
  dvt.ThematicMap.prototype.highlight = function (categories) {
    ...
    dvt.CategoryRolloverHandler.highlight(categories, this.getNavigableAreas().concat(this.getNavigableMarkers()).concat(this.getNavigableLinks()), this.Options['highlightMatch'] == 'any');
  };
  /**
  ...
  dvt.ThematicMap.prototype.handleTouchEnd = function () {
    ...
    if (this._touchEventContentMarkerRef && this._touchEventContentMarkerRef.handleTouchEnd) {
      ...
      this._touchEventContentMarkerRef.handleTouchEnd(); // Null out the reference as it need not be called again for this marker
  ...
  dvt.ThematicMap.prototype._getAreaFromDataLayer = function (areaName, dataLayer) {
    ...
    for (var j = 0; j < dataObjs.length; j++) {
      if (dataObjs[j].getLocation() === areaName) return dataObjs[j];
    }
  ...
  var DvtThematicMapDefaults = function DvtThematicMapDefaults(context) {
    this.Init({
  ...
  DvtThematicMapDefaults.prototype.calcAreaLayerOptions = function (userOptions, isCustomElement) {
    ...
    if (isCustomElement) defaultObj = dvt.JsonUtils.merge({
      'labelDisplay': 'off'
    }, defaultObj);
  ...
  var DvtThematicMapAutomation = function DvtThematicMapAutomation(dvtComponent) {
  ...
  DvtThematicMapAutomation.prototype.GetSubIdForDomElement = function (displayable) {
    ...
    if (logicalObj && (logicalObj instanceof DvtMapAreaPeer || logicalObj instanceof DvtMapObjPeer)) return this._getSubId(logicalObj);
    return null;
  };
  /**
   * Valid subIds inlcude:
  ...
  DvtThematicMapAutomation.prototype.getDomElementForSubId = function (subId) {
    if (subId == dvt.Automation.TOOLTIP_SUBID) return this.GetTooltipElement(this._tmap);
      ...
      for (var id in dataLayers) {
        ...
        if (displayable instanceof dvt.Path) {
          ...
          for (var k = 0; k < links.length; k++) {
            if (links[k] === dataObject) return this._getDataLayerId(id) + ':' + 'link[' + k + ']';
          }
        } else {
          var markers = dataLayers[id].getMarkerObjects();

          for (var k = 0; k < markers.length; k++) {
            if (markers[k] === dataObject) return this._getDataLayerId(id) + ':' + 'marker[' + k + ']';
          }
        }
      }
    }
  ...
  var DvtSelectablePath = function DvtSelectablePath(context, bSupportsVectorEffects) {
  ...
  DvtSelectablePath.prototype.Init = function (context, bSupportsVectorEffects) {
    ...
    this.Zoom = 1; //IE10, Flash/XML toolkit do not support vector-effects=non-scaling-stroke so we still need to set stroke width based on zoom
  ...
  DvtSelectablePath.prototype.CreateSelectedHoverStrokes = function () {
    if (!this.SelectedHoverInnerStroke) {
      this.SelectedHoverInnerStroke = new dvt.Stroke(this.HoverInnerStroke.getColor(), this.HoverInnerStroke.getAlpha(), DvtSelectablePath.HOVER_STROKE_WIDTH, this._bSupportsVectorEffects);
    }

    if (!this.SelectedHoverOuterStroke) {
      this.SelectedHoverOuterStroke = new dvt.Stroke(this.SelectedOuterStroke.getColor(), this.SelectedOuterStroke.getAlpha(), DvtSelectablePath.SELECTED_HOVER_OUTER_STROKE_WIDTH, this._bSupportsVectorEffects);
    }
  };
  /**
   * @override
  ...
  var DvtCustomDataItem = function DvtCustomDataItem(context, dataItem, styles) {
  ...
  DvtCustomDataItem.prototype.Init = function (context, dataItem, styles) {
    ...
    if (dataItem instanceof dvt.BaseComponent) {
      ...
      if (Array.isArray(dataItem)) {
        dataItem.forEach(function (node) {
          this.getElem().appendChild(node);
        }.bind(this)); // @HTMLUpdateOK
      } else {
    ...
    this._boundingRect.setHoverStroke(his, hos).setSelectedStroke(sis, sos).setSelectedHoverStroke(shis, shos); // Bounding rect stroke alignment is set to OUTER.
  ...
  DvtCustomDataItem.prototype.setAriaProperty = function (property, value) {
    if (dvt.Agent.isTouchDevice()) this._boundingRect.setAriaProperty(property, value);else DvtCustomDataItem.superclass.setAriaProperty.call(this, property, value);
  };
  /**
   * For accessibility, a custom svg data item sets wai-aria information on its bounding rect for VoiceOver.
   * @override
  ...
  DvtCustomDataItem.prototype.setAriaRole = function (role) {
    if (dvt.Agent.isTouchDevice()) this._boundingRect.setAriaRole(role);else DvtCustomDataItem.superclass.setAriaRole.call(this, role);
  };
  /**
  ...
  DvtCustomDataItem.prototype.updateRootElement = function (rootContent, shouldRemoveOldContent) {
    ...
    if (this._dataItem && shouldRemoveOldContent) {
      if (this._dataItem instanceof dvt.BaseComponent) {
        ...
        this._dataItem.forEach(function (node) {
    ...
    if (rootContent instanceof dvt.BaseComponent) this.addChild(rootContent);else if (Array.isArray(rootContent)) rootContent.forEach(function (node) {
      this.getElem().appendChild(node);
    }.bind(this)); // @HTMLUpdateOK
  ...
  DvtBaseMapManager.getAreaLabelInfo = function (baseMapName, layerName) {
    ...
    var layer = DvtBaseMapManager._GLOBAL_MAPS[baseMapName][layerName];
    if (layer) return layer[DvtBaseMapManager.TYPE_LABELINFO];else return null;
  };
  /**
  ...
  DvtBaseMapManager.getAreaLabels = function (baseMapName, layerName) {
    ...
    var layer = DvtBaseMapManager._GLOBAL_MAPS[baseMapName][layerName];
    if (layer) return layer[DvtBaseMapManager.TYPE_LABELS];else return null;
  };
  /**
  ...
  DvtBaseMapManager.getLongAreaLabel = function (baseMapName, layerName, areaId) {
    ...
    if (labels && labels[areaId]) return labels[areaId][1];else return null;
  };
  /**
  ...
  DvtBaseMapManager.getAreaPaths = function (baseMapName, layerName, viewportW, viewportH, zoomFactor) {
    ...
    var dimAr = layer[DvtBaseMapManager.TYPE_DIM];
    if (dimAr) return DvtBaseMapManager._simplifyAreaPaths(paths, dimAr[2], dimAr[3], viewportW, viewportH, zoomFactor);
    return paths;
  };
  /**
  ...
  DvtBaseMapManager.registerBaseMapLayer = function (baseMapName, layerName, labelMetadata, pathMetadata, parentsRegionMetadata, labelInfoMetadata, index, dim) {
    ...
    if (!layerMetadata) {
      ...
      basemapMetadata[layerName] = layerMetadata; // custom area layers don't have indicies when registered
  ...
  DvtBaseMapManager.registerResourceBundle = function (baseMapName, layerName, labelMetadata) {
    ...
    var layerMetadata = basemapMetadata[layerName]; // Overwrite english labels with resource bundle language
  ...
  DvtBaseMapManager._processUnprocessedMaps = function () {
    ...
    if (unprocessedParentUpdates) {
      for (i = 0; i < unprocessedParentUpdates[0].length; i++) {
        args = unprocessedParentUpdates[0][i]; // update extending layer parent mapping
        ...
        if (basemapMetadata) {
          ...
          if (layerMetadata) {
            ...
            basemapDim = layerMetadata[DvtBaseMapManager.TYPE_DIM]; // get the index of the extended layer and update indicies for layers
  ...
  DvtBaseMapManager._simplifyAreaPaths = function (paths, basemapW, basemapH, viewportW, viewportH, zoomFactor) {
    ...
    if (zoomFactor > 0) {
      ...
      if (scale <= 1) return paths; // If scale = 10 that means 10 pixels in the map coordinate space = 1 pixel in the current viewport
  ...
  var DvtThematicMapCategoryWrapper = function DvtThematicMapCategoryWrapper(displayable, category) {
  ...
  var DvtMapLabel = function DvtMapLabel(context, label, labelInfo, labelDisplay, parentContainer, bSupportsVectorEffects) {
  ...
  DvtMapLabel.prototype.Init = function (context, label, labelInfo, labelDisplay, parentContainer, bSupportsVectorEffects) {
    DvtMapLabel.superclass.Init.call(this, context, label, 0, 0); //IE10, Flash/XML toolkit do not support vector-effects=non-scaling-stroke so we still need to set stroke width based on zoom
      ...
      if (state == -1) {
        ...
        if (this._leaderLines) {
          ...
          if (state > 0) {
            ...
            this._parentContainer.addChild(this._leaderLine); // when using leaderliners, change text to black
            ...
            this.alignCenter();
            this.alignMiddle(); // reset label color
  ...
  var DvtMapObjPeer = function DvtMapObjPeer(data, dataLayer, displayable, label, center, locationName) {
  ...
  DvtMapObjPeer.prototype.Init = function (data, dataLayer, displayable, label, center, locationName) {
    ...
    if (this.Displayable.setDataColor) this.Displayable.setDataColor(data['color']);
    this._label = label;
    this._center = center;
    this.Zoom = 1;
    ...
    if (this._view.getDisplayTooltips() == 'auto' && locationName) this._data['shortDesc'] = data['shortDesc'] ? locationName + ': ' + data['shortDesc'] : locationName; // WAI-ARIA
  ...
  DvtMapObjPeer.prototype.getSize = function () {
    if (this.Displayable.getWidth) return this.Displayable.getWidth() * this.Displayable.getHeight();
    return 0;
  };
  /**
  ...
  DvtMapObjPeer.prototype.getDatatip = function () {
    if (this._view.getDisplayTooltips() != 'none') {
      ...
      if (tooltipObj) return this._view.getCtx().getTooltipManager().getCustomTooltip(tooltipObj['renderer'], this.getDataContext()); // Custom Tooltip from ShortDesc
  ...
  DvtMapObjPeer.prototype.getAriaLabel = function () {
    var states = [];
    if (this.isSelectable()) states.push(this._view.getOptions().translations[this.isSelected() ? 'stateSelected' : 'stateUnselected']);
  ...
  DvtMapObjPeer.prototype.isSelectable = function () {
    if (!this.Displayable.isSelectable) return false;else if (this._data['selectable'] !== 'off') return this._dataLayer.isSelectable();else return false;
  };
  /**
   * Implemented for DvtSelectable
   * @override
  ...
  DvtMapObjPeer.prototype.setSelected = function (selected) {
    if (this.isSelectable()) {
      ...
      if (this._dataLayer.getOptions()['selectionRenderer']) this._callCustomRenderer(this._dataLayer.getOptions()['selectionRenderer'], this._getState(), prevState);else this.processDefaultSelectionEffect(selected);
      this.UpdateAriaLabel();
    }
  };
  /**
  ...
  DvtMapObjPeer.prototype.showHoverEffect = function () {
    ...
    if (this._dataLayer.getOptions()['hoverRenderer']) this._callCustomRenderer(this._dataLayer.getOptions()['hoverRenderer'], this._getState(), prevState);else this.processDefaultHoverEffect(true);
  };
  /**
   * Implemented for DvtSelectable
   * @override
  ...
  DvtMapObjPeer.prototype.hideHoverEffect = function () {
    if (this.IsHoverEffectShown()) {
      ...
      this._isShowingHoverEffect = false;
      if (this._dataLayer.getOptions()['hoverRenderer']) this._callCustomRenderer(this._dataLayer.getOptions()['hoverRenderer'], this._getState(), prevState);else this.processDefaultHoverEffect(false);
    }
  };
  /**
  ...
  DvtMapObjPeer.prototype.showKeyboardFocusEffect = function () {
    ...
    if (this._dataLayer.getOptions()['focusRenderer']) this._callCustomRenderer(this._dataLayer.getOptions()['focusRenderer'], this._getState(), prevState);else this.processDefaultFocusEffect(true);
  };
  /**
   * Implemented for DvtKeyboardNavigable.
   * @override
  ...
  DvtMapObjPeer.prototype.hideKeyboardFocusEffect = function () {
    if (this.isShowingKeyboardFocusEffect()) {
      ...
      this._isShowingKeyboardFocusEffect = false;
      if (this._dataLayer.getOptions()['focusRenderer']) this._callCustomRenderer(this._dataLayer.getOptions()['focusRenderer'], this._getState(), prevState);else this.processDefaultFocusEffect(false);
    }
  };
  /**
   * Implemented for DvtKeyboardNavigable.
  ...
  DvtMapObjPeer.prototype.positionLabel = function () {
    if (this._label) {
      ...
      if (position == 'top') {
        ...
        y = markerY + markerH / 6; // in this special case we need special alignment since standard baseline has to be higher than
  ...
  DvtMapObjPeer.prototype.__recenter = function () {
    ...
    if (width != null && height != null && this.Displayable.getParent()) {
      ...
      shapeY = this._center.y * this.Zoom - shapeY; // account for x/y if displayable doesn't have setters for x/y
  ...
  DvtMapObjPeer.prototype.animateUpdate = function (handler, oldObj) {
    ...
    var oldDisplayable = oldObj.getDisplayable(); // Color change
    ...
    if (this.Displayable.getFill) {
      ...
      if (endFill instanceof dvt.SolidFill && !endFill.equals(startFill)) {
        ...
        if (oldObj.getLabel() && this._label) {
          ...
          if (!endLabelFill.equals(this._label.getFill())) anim.getAnimator().addProp(dvt.Animator.TYPE_FILL, this._label, this._label.getFill, this._label.setFill, endLabelFill);
        }
    ...
    if (startRotation != endRotation) {
      ...
      if (diffRotation > Math.PI) startRotation -= Math.PI * 2;else if (diffRotation < -Math.PI) startRotation += Math.PI * 2;
  ...
  DvtMapObjPeer.prototype.processDefaultHoverEffect = function (hovered) {
    if (hovered) {
      ...
      if (this.Displayable.hideHoverEffect && !this.isShowingKeyboardFocusEffect()) this.Displayable.hideHoverEffect();
    }
  };
  /**
  ...
  DvtMapObjPeer.prototype._callCustomRenderer = function (renderer, state, prevState) {
    ...
    var newRootElem = renderer(context); //   - support null case on updates for custom elements
  ...
  DvtMapObjPeer.prototype._checkAndMoveContents = function (rootElem) {
    ...
    this._contentStoredInTouchEventContainer = rootElem; // Reset the _hasContentBoundToTouchEvent flag since the contents are moved to the touch events pane
    // and this node's content no longer is associated with active touch event

    this._hasContentBoundToTouchEvent = false; // return true to indicate the contents are moved
  ...
  var DvtMapAreaPeer = function DvtMapAreaPeer(data, dataLayer, displayable, label, locationName) {
  ...
  DvtMapAreaPeer.prototype.hideHoverEffect = function () {
    if (this.isSelected()) this._dataAreaLayer.addChild(this.Displayable);else this._dataAreaLayer.addChildAt(this.Displayable, 0);
  ...
  DvtMapAreaPeer.prototype.__recenter = function () {// no-op
  };
  /**
   * @license
   * Copyright (c) %FIRST_YEAR% %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  var DvtMapLinkPeer = function DvtMapLinkPeer(data, dataLayer, displayable, startPt, endPt, startMarker, endMarker) {
  ...
  var DvtMapArea = function DvtMapArea(context, view, dvtShape, areaId, areaName, bSupportsVectorEffects) {
  ...
  DvtMapArea.prototype.Init = function (context, view, dvtShape, areaId, areaName, bSupportsVectorEffects) {
    ...
    this._view = view; //IE10, Flash/XML toolkit do not support vector-effects=non-scaling-stroke so we still need to set stroke width based on zoom
  ...
  DvtMapArea.prototype.getDatatip = function () {
    if (this._view.getDisplayTooltips() != 'none') {
      ...
      if (tooltipObj) return this._view.getCtx().getTooltipManager().getCustomTooltip(tooltipObj['renderer'], this.getDataContext());
  ...
  DvtMapArea.prototype.HandleZoomEvent = function (pzcMatrix) {
    if (!this._bSupportsVectorEffects && this._shape && this._areaStrokeWidth) {
      ...
      var zoomStroke = new dvt.Stroke(copyStroke.getColor(), copyStroke.getAlpha(), this._areaStrokeWidth / pzcMatrix.getA(), copyStroke.isFixedWidth());
  ...
  var DvtMapLayer = function DvtMapLayer(tmap, layerName, eventHandler) {
  ...
  DvtMapLayer.prototype.PreDataLayerUpdate = function () {//subclasses to override
  };
  /**
  ...
  DvtMapLayer.prototype.PostDataLayerUpdate = function () {//subclasses to override
  };
  /**
  ...
  DvtMapLayer.prototype.updateDataLayer = function (dataLayer, pzcMatrix, topLayerName) {
    ...
    this.PzcMatrix = pzcMatrix; // Get old data layer

    this._oldDataLayer = this.getDataLayer(dataLayer.getClientId());
    this.addDataLayer(dataLayer);
    dataLayer.render(this.PzcMatrix); // create a zoom event so we can update the data objects with the current zoom
    ...
    if (this._oldDataLayer) {
      ...
      if (anim == 'auto') {
        ...
        var anim1 = dvt.BlackBoxAnimationHandler.getCombinedAnimation(this._tmap.getCtx(), anim, oldNonScaledContainers, newNonScaledContainers, bounds1, animDur);
        ...
        var anim2 = dvt.BlackBoxAnimationHandler.getCombinedAnimation(this._tmap.getCtx(), anim, oldScaledContainers, newScaledContainers, bounds2, animDur);
      ...
      this.PreDataLayerUpdate(); // If an animation was created, play it

      if (this._animation) {
        // Disable event listeners temporarily
        this.EventHandler.removeListeners(this._callbackObj); // Start the animation

        var thisRef = this;

        this._animation.setOnEnd(function () {
          thisRef.OnAnimationEnd(dataLayer);
        }, this);
  ...
  DvtMapLayer.prototype.OnAnimationEnd = function (dataLayer) {
    ...
    this._tmap.OnUpdateLayerEnd(); // Reset the animation stopped flag
    // Remove the animation reference


    this._animation = null; // Restore event listeners
  ...
  var DvtMapAreaLayer = function DvtMapAreaLayer(tmap, layerName, labelDisplay, labelType, eventHandler) {
  ...
  DvtMapAreaLayer.prototype.getShortAreaName = function (area) {
    if (this.AreaLabels && this.AreaLabels[area]) return this.AreaLabels[area][DvtMapAreaLayer._SHORT_NAME];
    return null;
  };

  DvtMapAreaLayer.prototype.getLongAreaName = function (area) {
    if (this.AreaLabels && this.AreaLabels[area]) return this.AreaLabels[area][DvtMapAreaLayer._LONG_NAME];
    return null;
  };
  ...
  DvtMapAreaLayer.prototype.setIsolatedArea = function (isolatedArea) {
    this._isolatedArea = isolatedArea; // reset the layer dimensions in case of data layer update
  ...
  DvtMapAreaLayer.prototype.getLayerDim = function () {
    if (!this._layerDim) {
      ...
      if (this._isolatedArea) {
        ...
        if (!basemap) this._layerDim = dvt.DisplayableUtils.getDimensionsForced(this._tmap.getCtx(), this.getAreaShape(this._isolatedArea));else this._layerDim = dvt.PathUtils.getDimensions(dvt.PathUtils.createPathArray(DvtBaseMapManager.getPathForArea(this._tmap.getMapName(), this.LayerName, this._isolatedArea)));
      } else {
        if (basemap && basemap != 'world' && basemap != 'worldRegions') this._layerDim = DvtBaseMapManager.getBaseMapDim(this._tmap.getMapName(), this.LayerName);
        ...
        if (!this._layerDim) {
          ...
          var dim = this.AreaContainer.getDimensions().getUnion(this._tmap.getDataAreaContainer().getDimensions()); // if we don't have cached dims and no objects have been rendered yet, dim will have 0 dimensions
      ...
      if (this._renderLabel[area]) {
        ...
        if (!label) {
          if (this._labelDisplay != 'off' && this.AreaLabels) {
            var labelText = this._labelType == 'short' ? this.AreaLabels[area][DvtMapAreaLayer._SHORT_NAME] : this.AreaLabels[area][DvtMapAreaLayer._LONG_NAME];

            if (labelText) {
              if (this._labelInfo && this._labelInfo[area]) label = new DvtMapLabel(this._tmap.getCtx(), labelText, this._labelInfo[area], this._labelDisplay, this.LabelContainer, this._tmap.supportsVectorEffects());else {
                var areaDim = dvt.DisplayableUtils.getDimensionsForced(this._tmap.getCtx(), areaShape);
                label = new DvtMapLabel(this._tmap.getCtx(), labelText, [[areaDim.x, areaDim.y, areaDim.w, areaDim.h]], this._labelDisplay, this.LabelContainer, this._tmap.supportsVectorEffects());
              }
  ...
  DvtMapAreaLayer.prototype._addAreaAndLabel = function (area, fadeInObjs) {
    if (this.AreaShapes[area]) {
      ...
      if (label) {
        if (this._renderLabel[area]) label.update(this.PzcMatrix);else label.reset();
  ...
  DvtMapAreaLayer.prototype._renderSelectedAreasAndLabels = function (areas, fadeInObjs) {
    for (var i = 0; i < areas.length; i++) {
      this._createAreaAndLabel(areas[i]); // Do not render areas that were rendered in the data layer
  ...
  DvtMapAreaLayer.prototype.__showDropSiteFeedback = function (obj) {
    ...
    if (obj) {
      ...
      if (this._dropSiteFeedback) {
        ...
        var strokeWidth = this._dropSiteCSSStyle.getStyle(dvt.CSSStyle.BORDER_WIDTH) ? this._dropSiteCSSStyle.getStyle(dvt.CSSStyle.BORDER_WIDTH).substring(0, this._dropSiteCSSStyle.getStyle(dvt.CSSStyle.BORDER_WIDTH).indexOf('px')) : 1;
  ...
  var DvtMapDataLayer = function DvtMapDataLayer(tmap, parentLayer, clientId, eventHandler, options) {
  ...
  DvtMapDataLayer.prototype.Init = function (tmap, parentLayer, clientId, eventHandler, options) {
    ...
    this._linkMap = {}; // Drag and drop support - not used in JET
    ...
    this._dataLinkLayer = new dvt.Container(this._tmap.getCtx()); // Add containers to head of parent container so parent layer objects are always drawn first
  ...
  DvtMapDataLayer.prototype.trackLink = function (start, end) {
    ...
    if (!this._linkMap[key]) this._linkMap[key] = 1;else this._linkMap[key]++;
  ...
  DvtMapDataLayer._getLinkKey = function (start, end) {
    if (start.x < end.x) return start.x.toString() + start.y.toString() + end.x.toString() + end.y.toString();else return end.x.toString() + end.y.toString() + start.x.toString() + start.y.toString();
  };
  /**
  ...
  DvtMapDataLayer.prototype.render = function (pzcMatrix) {
    ...
    var areaLabelsToRemove = {}; // first make a copy of markers and then sort by size to prevent overlapping
    // original order should be kept for automation purposes

    var dataObjs = this._markerObjs.slice();

    dataObjs.sort(function compare(a, b) {
      if (a.getSize() < b.getSize()) return 1;else if (a.getSize() > b.getSize()) return -1;else return 0;
    });
    ...
    for (var i = 0; i < this._areaObjs.length; i++) {
      if (areaLabelsToRemove[this._areaObjs[i].getLocation()]) this._areaObjs[i].removeLabel(); // areaObjs array can be modified by _renderAreaAndLabel if area has
      // been removed from parent area layer due to path simplification routine

      if (!this._renderAreaAndLabel(i)) i--;
    }
    ...
    for (var i = 0; i < dataObjs.length; i++) {
      ...
      if (disp instanceof DvtCustomDataItem) {
        ...
        if (Array.isArray(rootObj)) {
          rootObj.forEach(function (node) {
            rootObj.destroy && rootObj.destroy();
          }.bind(this));
  ...
  DvtMapDataLayer.prototype.getNavigableLinksForNodeId = function (markerId) {
    ...
    for (var i = 0; i < this._dataLinkCollection.length; i++) {
      ...
      if (startMarker && dvt.Obj.compareValues(ctx, startMarker.getId(), markerId)) links.push(link);else if (endMarker && dvt.Obj.compareValues(ctx, endMarker.getId(), markerId)) links.push(link);
    }
  ...
  var DvtThematicMapKeyboardHandler = function DvtThematicMapKeyboardHandler(tmap, manager) {
  ...
  DvtThematicMapKeyboardHandler.prototype.processKeyDown = function (event) {
    ...
    if (keyCode == dvt.KeyboardEvent.CLOSE_BRACKET) {
      ...
      this._linkNavType = DvtThematicMapKeyboardHandler.LINK_NAV; // Traverse data objects in order of areas ] markers ] links

      var navigables = []; // If we're currently in the area data layer, move to markers

      if (focusObj instanceof DvtMapAreaPeer) {
        navigables = this._tmap.getNavigableMarkers(); // If there were no navigable data markers, try links
      ...
      // Set the data layer link navigation type
      this._linkNavType = DvtThematicMapKeyboardHandler.LINK_NAV; // Traverse data objects in order of links [ markers [ areas

      var navigables = [];

      if (focusObj instanceof DvtMapLinkPeer) {
        navigables = this._tmap.getNavigableMarkers(); // If there were no navigable data markers, try areas
      ...
      focusObj = DvtThematicMapKeyboardHandler.superclass.processKeyDown.call(this, event); // update the clicked object for a navigation and selection event
  ...
  DvtThematicMapKeyboardHandler.getFirstNavigableLink = function (marker, event, listOfLinks) {
    ...
    for (var i = 0; i < listOfLinks.length; i++) {
      ...
      var linkPt;
      if (dvt.Obj.compareValues(ctx, marker.getId(), link.getStartMarker().getId())) linkPt = link.getEndPoint();else linkPt = link.getStartPoint();

      if (direction == dvt.KeyboardEvent.OPEN_ANGLED_BRACKET && linkPt.x <= markerPt.x || direction == dvt.KeyboardEvent.CLOSE_ANGLED_BRACKET && linkPt.x >= markerPt.x) {
        break;
      }
    }
  ...
  var DvtThematicMapEventManager = function DvtThematicMapEventManager(context, callback, callbackObj) {
  ...
  DvtThematicMapEventManager.prototype.OnClick = function (event) {
    ...
    this.SetClickInfo(obj); // Clear all selection handlers if something not selectable is clicked

    if (!(obj && obj.isSelectable && obj.isSelectable())) {
      for (var clientId in this._selectionHandlers) {
        var bSelectionChanged = this._selectionHandlers[clientId].processClick(null, event.ctrlKey); // If the selection has changed, fire an event
  ...
  DvtThematicMapEventManager.prototype.SetClickInfo = function (obj) {
    ...
    if (obj) {
      if (obj instanceof DvtMapObjPeer) clickedObj = obj.getDisplayable();else if (obj instanceof DvtMapArea) clickedObj = obj;
  ...
  DvtThematicMapEventManager.prototype.ProcessKeyboardEvent = function (event) {
    ...
    var focusDisp = focusObj.getDisplayable(); // Mashup
    ...
    if (keyCode != dvt.KeyboardEvent.TAB && this._bPassOnEvent) {
      ...
      event.preventDefault();
        ...
        event.preventDefault();
          ...
          if (event.altKey) this._tmap.fitRegion(focusDisp);else this._tmap.fitSelectedRegions();
          event.preventDefault();
        } // Mashups
  ...
  DvtThematicMapEventManager.prototype.OnComponentTouchClick = function (event) {
    ...
    if (consumed) return;
    var obj = this.GetLogicalObject(event.target);
    this.SetClickInfo(obj); // If logical object is dvt.ThematicMap, then background was tapped and we should call data layer
  ...
  DvtThematicMapEventManager.prototype.ProcessRolloverEvent = function (event, obj, bOver) {
    ...
    this.RolloverHandler.processEvent(rolloverEvent, this._tmap.getNavigableAreas().concat(this._tmap.getNavigableMarkers()).concat(this._tmap.getNavigableLinks()), hoverBehaviorDelay, options['highlightMatch'] == 'any');
  };
  /**
   * @override
   */
  ...
  DvtThematicMapEventManager.prototype.ShowFocusEffect = function (event, obj) {
    if (!this._tmap.isPanning()) DvtThematicMapEventManager.superclass.ShowFocusEffect.call(this, event, obj);
  };
  /**
   * @license
   * Copyright (c) 2011 %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  var DvtThematicMapJsonParser = function DvtThematicMapJsonParser(tmap) {
    this.Init(tmap);
  };
  ...
  DvtThematicMapJsonParser.prototype._parseMapProperties = function (options) {
    ...
    if (typeof animDur == 'string') {
      if (animDur.slice(-2) == 'ms') animDur = parseInt(animDur.slice(0, -2)) / 1000;else if (animDur.slice(-1) == 's') animDur = parseFloat(animDur.slice(0, -1));
    } else {
    ...
    if (!this._mapProvider) this._tmap.setMapName(options['basemap']); // zooming attributes
  ...
  DvtThematicMapJsonParser.prototype._parseAreaLayer = function (areaLayers) {
    ...
    var layer = areaLayer['layer']; // layer and basemap names not required when using mapProvider
    ...
    if (areaLabelStyle) this._areaLayerStyle.merge(areaLabelStyle); // areaLayers.labelStyle is a string or object
    ...
    if (this._mapProvider) {
      ...
      paths = DvtBaseMapManager.getAreaPaths(basemap, layer, this._tmap.getWidth(), this._tmap.getHeight(), this._tmap.getMaxZoomFactor());
    ...
    if (areaLayer['areaDataLayer']) this.ParseDataLayers([areaLayer['areaDataLayer']], mapLayer, null, true);
    if (areaLayer['pointDataLayers']) this.ParseDataLayers(areaLayer['pointDataLayers'], mapLayer, null, false);
  };
  /**
  ...
  DvtThematicMapJsonParser.prototype.ParseDataLayers = function (dataLayers, parentLayer, topLayerName, isAreaDataLayer) {
    ...
    for (var i = 0; i < dataLayers.length; i++) {
      ...
      if (parentLayer) {
        if (parentLayer instanceof DvtMapAreaLayer && isAreaDataLayer) parentLayer.resetRenderedAreas();
      } else {
      ...
      if (selectionMode == 'single') dataLayer.setSelectionMode(dvt.SelectionHandler.TYPE_SINGLE);else if (selectionMode == 'multiple') dataLayer.setSelectionMode(dvt.SelectionHandler.TYPE_MULTIPLE);
      dataLayer.setAnimation(dataLayerOptions['animationOnDataChange']);
      dataLayer.setAnimationDuration(this._tmap.getAnimationDuration()); //Add initially isolated area

      var isolatedRowKey = null;
      if (parentLayer instanceof DvtMapAreaLayer) isolatedRowKey = dataLayerOptions['isolatedItem'];
      var isolatedAreaId;
      var isAreaDataLayer = parentLayer instanceof DvtMapAreaLayer; // Parse data objects
      ...
      if (areas) {
        for (var j = 0; j < areas.length; j++) {
          ...
          if (isolatedRowKey) {
            if (!dvt.Obj.compareValues(ctx, isolatedRowKey, areas[j]['id'])) continue;else isolatedAreaId = areaId;
          }
      ...
      if (markers) {
        ...
        for (var j = 0; j < markers.length; j++) {
          ...
          if (isolatedRowKey) {
            if (!dvt.Obj.compareValues(ctx, isolatedRowKey, markers[j]['id'])) continue;else isolatedAreaId = areaId;
          }
      ...
      if (images) {
        for (var j = 0; j < images.length; j++) {
          ...
          if (isolatedRowKey) {
            if (!dvt.Obj.compareValues(ctx, isolatedRowKey, images[j]['id'])) continue;else isolatedAreaId = areaId;
          }
      ...
      if (initSelections && initSelections.length > 0) dataLayer.setInitialSelections(initSelections);
      if (topLayerName) parentLayer.updateDataLayer(dataLayer, this._tmap.getPanZoomCanvas().getContentPane().getMatrix(), topLayerName);else parentLayer.addDataLayer(dataLayer);
    }
  };
  /**
  ...
  DvtThematicMapJsonParser.prototype._parseStyles = function (styles) {
    ...
    if (areaStyle && !(areaStyle instanceof Object)) areaStyle = dvt.CSSStyle.cssStringToObject(areaStyle); //Merge the areaStyle from public defaults with areaStyle from ThematicMap defaults
  ...
  DvtThematicMapJsonParser._getAreaCSSStyle = function (styleObj) {
    var style = new dvt.CSSStyle(); //Area CSS Style supports only border-color and background-color attributes
  ...
  DvtThematicMapJsonParser.prototype._createPathShapes = function (paths) {
    ...
    for (var area in paths) {
      ...
      path.setCmds(paths[area]); // Style area layer border and background colors
      ...
      if (backgroundColor != 'transparent') path.setSolidFill(backgroundColor);else //TODO set on area layer instead
        path.setFill(null);
  ...
  DvtThematicMapJsonParser.prototype._createArea = function (layer, dataLayer, data) {
    ...
    var areaShape = layer.getAreaShape(areaId); // only render data area if we have the path info for it and if it has a data color
    ...
    if (areaShape) {
      ...
      path.setHoverStroke(hs, null).setSelectedStroke(sis, sos); // disable labels in area layer if data layer exists and has label
  ...
  DvtThematicMapJsonParser.prototype._createMarker = function (layer, dataLayer, data, isParentAreaDataLayer) {
    var size = data['_size'];
    var center; // MapProvider coordinates are already projected, but need to flip the y coordinate for svg
    // because 0,0 is top left instead of bottom left

    if (this._mapProvider && !data['location']) center = new dvt.Point(data['x'], -data['y']);else center = DvtThematicMapJsonParser.getCenter(dataLayer, data['location'], data['x'], data['y']); // Skip over data where no marker center was determined or values resulted in a calculated size of 0 pixels

    if (!center || size === 0) return null; // merge data marker default styles, need to handle label style differently because we want to merge the two css strings
    ...
    data['labelStyle'] = markerLabelStyle; // Parse data object scales. Save original scale to maintain size despite zoom.
    ...
    var br = data['borderRadius']; // id is used for custom marker definition lookup

    var marker;

    if (data['source']) {
      marker = new dvt.ImageMarker(this._tmap.getCtx(), center.x, center.y, width, height, br, data['source'], data['sourceSelected'], data['sourceHover'], data['sourceHoverSelected']);
    } else {
  ...
  DvtThematicMapJsonParser.prototype._createImage = function (layer, dataLayer, data, isParentAreaDataLayer) {
    ...
    var height = data['height']; // set x/y only if both width and height are set, otherwise x/y will be set in the callback
    ...
    if (!width || !height) {
      var callback = function callback(imageInfo) {
  ...
  DvtThematicMapJsonParser.prototype._createCustomDataItem = function (layer, dataLayer, data, customContent, isParentAreaDataLayer) {
    ...
    if (this._mapProvider && !data['location']) center = new dvt.Point(data['x'], -data['y']);else center = DvtThematicMapJsonParser.getCenter(dataLayer, data['location'], data['x'], data['y']);
    if (!center) // no matching city
      return null; // disable labels in area layer if data layer exists and has label
  ...
  DvtThematicMapJsonParser.prototype._createLink = function (dataLayer, data, idToMarkerMap, idToAreaMap) {
    ...
    if (startLoc['id']) {
      ...
      if (startMarker) {
        startPt = startMarker.getCenter();
          ...
          if (startMarker) {
            startPt = this._getPtFromLocation(dataLayer, {
              'location': startMarker.getLocation()
            });
          }
        }
    } else {
    ...
    if (endLoc['id']) {
      endMarker = idToMarkerMap.get(endLoc['id']); // Get x/y from marker. Markers are created before links.
      ...
      if (endMarker) {
        endPt = endMarker.getCenter();
          ...
          if (endMarker) {
            endPt = this._getPtFromLocation(dataLayer, {
              'location': endMarker.getLocation()
            });
          }
        }
    } else {
  ...
  DvtThematicMapJsonParser.prototype._createLabel = function (layer, dataLayer, data, displayable, isParentAreaDataLayer) {
    var areaId = data['location'];
    var labelText = data['label']; // if data label is provided, assume label display is on and if is from an area data layer, use area layer's label display

    var labelDisplay = labelText ? 'on' : 'off';
    if (isParentAreaDataLayer) labelDisplay = layer.getLabelDisplay();
    var isArea = displayable instanceof dvt.Path; // If object is in an areaDataLayer see if label is provided, if not, use the default area label

    if (!labelText && isParentAreaDataLayer && (isArea && labelDisplay != 'off' || !isArea && labelDisplay == 'on')) {
    ...
    if (labelText) {
      ...
      var label;
      if (isArea) label = new DvtMapLabel(context, labelText, layer.getLabelInfoForArea ? layer.getLabelInfoForArea(areaId) : null, labelDisplay, dataLayer.getDataLabelContainer(), this._tmap.supportsVectorEffects());else label = new dvt.OutputText(context, labelText, 0, 0); // Label styling

      var labelStyle = isArea ? layer.getLayerCSSStyle() : new dvt.CSSStyle(); // add label style by merging styles sent from skin and tag
      ...
      label.setCSSStyle(labelStyle); // color label to contrast with data color if none provided or in high contrast mode
  ...
  DvtThematicMapJsonParser.prototype._styleDisplayable = function (style, displayable) {
    var backgroundColor = style['color']; // handle custom svg where color is set by user
    ...
    if (displayable instanceof dvt.SimpleMarker) {
      ...
      if (strokeType != 'none') {
        ...
        if (typeof borderWidth == 'string') {
          if (borderWidth.slice(-2) == 'px') borderWidth = parseFloat(style['borderWidth'].slice(0, -2));else borderWidth = parseFloat(style['borderWidth']);
        }

        var stroke = new dvt.Stroke(style['borderColor'], 1, borderWidth, !this._tmap.isMarkerZoomBehaviorFixed(), dvt.Stroke.getDefaultDashProps(strokeType, borderWidth));
  ...
  DvtThematicMapJsonParser.getCenter = function (dataLayer, location, x, y) {
    ...
    if (location) {
      ...
      if (areaCenter) {
        ...
        var dim; // TODO: PathUtils.getDimensions is slightly faster than DOM getDimensions, but
        // there's a bug in the method where the complicated paths found for areas aren't
        // returning the correct bounds
  ...
  DvtThematicMapJsonParser._calcBubbleSizes = function (tmap, markers) {
    ...
    for (var i = 0; i < markers.length; i++) {
      var value = markers[i][valKey]; // Negative and zero marker values don't correlate to a marker radius size so we skip them when determining range
    ...
    if (minValue === Infinity) return; // Min/max allowed marker sizes

    var zoomMargins = 2 * DvtThematicMapJsonParser._PZC_PADDING;
    var mapWidth = tmap.getWidth() - zoomMargins;
    var mapHeight = tmap.getHeight() - zoomMargins; // Adjust maxSize by limiting basemap aspect ratio so that 1.3 < w/h < 1.7. Temporary heuristical approach
    // to fixing map aspct ratio issue which we can't determine until render time.

    var ratio = mapWidth / mapHeight;
    if (ratio < 1.3) mapHeight = mapWidth * 2 / 3;else if (ratio > 1.7) mapWidth = mapHeight * 1.5;
    var maxSize = DvtThematicMapJsonParser._MAX_MARKER_SIZE_RATIO * Math.min(mapWidth, mapHeight); // Loop through the data and update the sizes

    for (var i = 0; i < markers.length; i++) {
      var value = markers[i][valKey]; // Treat markers with missing values the same as we treat negative/zero valued markers and set size to 0 so we skip rendering them
  ...
  DvtThematicMapJsonParser.calcCurve = function (dataLayer, origStartPt, origEndPt, startPt, endPt) {
    ...
    var numLinks = dataLayer.trackLink(origStartPt, origEndPt); // Heuristic to find a fixed arc increment for overlapping links based on map size

    var arcIncr = Math.min(tmap.getWidth(), tmap.getHeight()) / 35; // Check for straight lines

    if (x2 - x1 === 0 || y2 - y1 === 0) {
      var controlX1, controlY1, controlX2, controlY2;

      if (numLinks > 1) {
        arcIncr *= numLinks - 1; // Vertical

        if (x2 - x1 === 0) {
          var dist = Math.abs(y1 - y2) * 0.3; // Calculate the 1st control point

          controlX1 = x1 + arcIncr;
          controlY1 = y1 > y2 ? y1 - dist : y1 + dist; // Calculate the 2nd control point

          controlX2 = controlX1;
          controlY2 = y2 > y1 ? y2 - dist : y2 + dist;
        } else {
          // Horizontal, render overlap curve above
          var dist = Math.abs(x1 - x2) * 0.3; // Calculate the 1st control point

          controlX1 = x1 > x2 ? x1 - dist : x1 + dist;
          controlY1 = y1 - arcIncr; // Calculate the 2nd control point
      ...
      var dist = Math.sqrt(Math.pow(x2 - x1, 2), Math.pow(y2 - y1, 2)); // Start arc height at 1/4 of height of circle btwn the start/end points if no other links
      ...
      var perpSlope = -1 / slope; // Force arc to always point up

      if (perpSlope > 0) arcHeight *= -1; // d / sqrt(1 + m^2)

      var distVal = dist * 0.3 / Math.sqrt(1 + Math.pow(slope, 2));
      var isX2AfterX1 = x2 - x1 > 0; // Find point on original line to calculate 1st control point that is approx 30% of the way from the start point

      var xc1 = x1 + (isX2AfterX1 ? 1 : -1) * distVal;
      var yc1 = slope * (xc1 - x1) + y1; // Find point on original line to calculate 2nd control point that is approx 30% of the way from the end point

      var xc2 = x2 + (isX2AfterX1 ? -1 : 1) * distVal;
      var yc2 = slope * (xc2 - x2) + y2; // d / sqrt(1 + m^2)

      var controlDistVal = arcHeight / Math.sqrt(1 + Math.pow(perpSlope, 2)); // Calculate the 1st control point

      var controlX1 = xc1 + controlDistVal;
      var controlY1 = perpSlope * (controlX1 - xc1) + yc1; // Calculate the 2nd control point
  ...
  DvtThematicMapJsonParser._getLocationName = function (basemap, dataLayer, data) {
    ...
    if (location) {
      var mapLayer = dataLayer.getMapLayer(); // For data objects associated with supported areas or cities we prepend the area/city name before the datatip

      if (!(mapLayer instanceof DvtMapAreaLayer)) return DvtBaseMapManager.getCityLabel(basemap, location);else return mapLayer.getLongAreaName(location);
    }
  ...
  DvtThematicMapProjections.project = function (x, y, basemap) {
    ...
    switch (basemap) {
      case 'africa':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._AFRICA_BOUNDS, DvtThematicMapProjections._getMercatorProjection(x, y));
        break;

      case 'asia':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._ASIA_BOUNDS, DvtThematicMapProjections._getAlbersEqualAreaConicProjection(40, 95, 20, 60, x, y), DvtThematicMapProjections.toRadians(5));
        break;
      ...
      case 'europe':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._EUROPE_BOUNDS, DvtThematicMapProjections._getAlbersEqualAreaConicProjection(35, 25, 40, 65, x, y), DvtThematicMapProjections.toRadians(10));
        break;

      case 'northAmerica':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._N_AMERICA_BOUNDS, DvtThematicMapProjections._getAlbersEqualAreaConicProjection(23, -96, 20, 60, x, y));
        break;

      case 'southAmerica':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._S_AMERICA_BOUNDS, new dvt.Point(x, y), DvtThematicMapProjections.toRadians(5));
        break;

      case 'apac':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._APAC_BOUNDS, DvtThematicMapProjections._getMercatorProjection(x, y));
        break;

      case 'emea':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._EMEA_BOUNDS, DvtThematicMapProjections._getMercatorProjection(x, y));
        break;

      case 'latinAmerica':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._L_AMERICA_BOUNDS, new dvt.Point(x, y));
        break;

      case 'usaAndCanada':
        point = DvtThematicMapProjections._getAffineProjection(DvtThematicMapProjections._USA_CANADA_BOUNDS, DvtThematicMapProjections._getAlbersEqualAreaConicProjection(23, -96, 20, 60, x, y));
        break;
    ...
    if (point) return new dvt.Point(point.x * 10, point.y * 10); // multiply by 10 because basemaps are 10x bigger
    else return null;
  };
  /**
  ...
  DvtThematicMapProjections._getAustraliaProjection = function (x, y) {
    var viewPortTransform;
    if (DvtThematicMapProjections._NEW_ZEALAND_BOUNDS.containsPoint(x, y)) viewPortTransform = DvtThematicMapProjections._getViewPortTransformation(DvtThematicMapProjections._NEW_ZEALAND_BOUNDS, DvtThematicMapProjections._NEW_ZEALAND_RECT);else viewPortTransform = DvtThematicMapProjections._getViewPortTransformation(DvtThematicMapProjections._AUSTRALIA_BOUNDS, DvtThematicMapProjections._VIEWPORT_BOUNDS);
  ...
  DvtThematicMapProjections._getRobinsonProjection = function (x, y) {
    ...
    if (ycriteria >= DvtThematicMapProjections._ROBINSON_COORDINATES.length - 1) ycriteria = DvtThematicMapProjections._ROBINSON_COORDINATES.length - 2;
  ...
  DvtThematicMapProjections.inverseProject = function (x, y, basemap) {
    var point; // divide by 10 because basemaps are 10x larger than original projected maps
    ...
    switch (basemap) {
      case 'africa':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._AFRICA_BOUNDS, new dvt.Point(x, y));
        point = DvtThematicMapProjections._getInverseMercatorProjection(point.x, point.y);
        break;

      case 'asia':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._ASIA_BOUNDS, new dvt.Point(x, y), DvtThematicMapProjections.toRadians(5));
      ...
      case 'europe':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._EUROPE_BOUNDS, new dvt.Point(x, y), DvtThematicMapProjections.toRadians(10));
        point = DvtThematicMapProjections._getInverseAlbersEqualAreaConicProjection(35, 25, 40, 65, point.x, point.y);
        break;

      case 'northAmerica':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._N_AMERICA_BOUNDS, new dvt.Point(x, y));
        point = DvtThematicMapProjections._getInverseAlbersEqualAreaConicProjection(23, -96, 20, 60, point.x, point.y);
        break;

      case 'southAmerica':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._S_AMERICA_BOUNDS, new dvt.Point(x, y), DvtThematicMapProjections.toRadians(5));
        break;

      case 'apac':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._APAC_BOUNDS, new dvt.Point(x, y));
        point = DvtThematicMapProjections._getInverseMercatorProjection(point.x, point.y);
        break;

      case 'emea':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._EMEA_BOUNDS, new dvt.Point(x, y));
        point = DvtThematicMapProjections._getInverseMercatorProjection(point.x, point.y);
        break;

      case 'latinAmerica':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._L_AMERICA_BOUNDS, new dvt.Point(x, y));
        break;

      case 'usaAndCanada':
        point = DvtThematicMapProjections._getInverseAffineProjection(DvtThematicMapProjections._USA_CANADA_BOUNDS, new dvt.Point(x, y));
        point = DvtThematicMapProjections._getInverseAlbersEqualAreaConicProjection(23, -96, 20, 60, point.x, point.y);
        break;
    ...
    if (point) return point;else return new dvt.Point(x, y);
  };
  /**
  ...
  DvtThematicMapProjections._getInverseAustraliaProjection = function (x, y) {
    var viewPortTransform;
    if (DvtThematicMapProjections._NEW_ZEALAND_RECT.containsPoint(x, y)) viewPortTransform = DvtThematicMapProjections._getViewPortTransformation(DvtThematicMapProjections._NEW_ZEALAND_BOUNDS, DvtThematicMapProjections._NEW_ZEALAND_RECT);else viewPortTransform = DvtThematicMapProjections._getViewPortTransformation(DvtThematicMapProjections._AUSTRALIA_BOUNDS, DvtThematicMapProjections._VIEWPORT_BOUNDS);
    viewPortTransform = viewPortTransform.invert();
    var point = viewPortTransform.transformPoint(new dvt.Point(x, y));
    return DvtThematicMapProjections._getInverseMercatorProjection(point.x, point.y);
  };
  ...
  DvtThematicMapProjections._getInverseRobinsonProjection = function (x, y) {
    ...
    if (criteria >= DvtThematicMapProjections._ROBINSON_COORDINATES.length - 1) criteria = DvtThematicMapProjections._ROBINSON_COORDINATES.length - 2;
  ...
  DvtMapProviderUtils._PROPERTIES = 'properties'; // GeoJSON Types
  ...
  DvtMapProviderUtils._TYPE_MULTI_POLYGON = 'MultiPolygon'; // Our internal properties keys required for determining an area's id, shortLabel, and longLabel
  ...
  DvtMapProviderUtils.parseMapInfo = function (context, mapProvider) {
    ...
    if (!keys[DvtMapProviderUtils._ID]) throw new Error('Missing required mapProvider.propertiesKeys.id property.'); // Determine the GeoJSON top-level type

    var type = geoJson[DvtMapProviderUtils._TYPE]; // Only a Feature will have a properties object which gives us a place to look up
    // an area's id, shortLabel, and longLabel.

    if (type === DvtMapProviderUtils._TYPE_FEATURE_COLLECTION) {
      var features = geoJson[DvtMapProviderUtils._FEATURES]; // Check each feature for an ID and skip if not provided
      ...
      throw new Error('GeoJSON type of ' + type + ' is not supported. ' + 'Only Feature and FeatureCollection types are supported.');
  ...
  DvtMapProviderUtils._parseFeature = function (context, feature, keys, areas, labels) {
    ...
    if (!DvtMapProviderUtils._isSupportedGeometry(geom)) {
      context['oj']['Logger']['warn']('A geometry of type ' + geom[DvtMapProviderUtils._TYPE] + ' is not supported and will be skipped.');
      return;
    }
    ...
    if (!id) {
      context['oj']['Logger']['warn']("No 'id' value found in the mapProvider.propertiesKey object. " + "A Feature's 'properties' object must have an id in the field specified by " + 'the mapProvider.propertiesKey.id value.');
      return;
    } // Generate label info
  ...
  DvtMapProviderUtils._isSupportedGeometry = function (geom) {
    ...
    if (type === DvtMapProviderUtils._TYPE_POLYGON || type === DvtMapProviderUtils._TYPE_MULTI_POLYGON) return true;
    return false;
  };
  /**
  ...
  DvtMapProviderUtils._polygonToPath = function (coords) {
    var path = ''; // The coordinates of a Polygon are an array of LinearRing coordinate arrays
  ...
  DvtMapProviderUtils._linearRingToPath = function (coords) {
    ...
    if (coords) {
      for (var i = 0; i < coords.length; i++) {
        ...
        var relX = x - prevX;
        var relY = y - prevY; // check to see if we can convert a l to a h/v command
  ...
  DvtMapProviderUtils._getCoord = function (path, idx) {
    ...
    for (var i = idx; i < path.length; i++) {
      ...
      if (isNaN(currChar) || currChar === ' ') {
        ...
        return {
          coord: parseInt(path.substring(idx, i)),
          idx: i,
          cmd: cmd
        };
      }
  ...
  DvtMapProviderUtils._pathToPolygon = function (path) {
    ...
    if (path) {
      ...
      for (var i = 1; i < path.length; i++) {
        ...
        var coord2 = null; // Update idx

        i = coord1.idx; // If command isn't v or h look for 2nd coordinate

        if (currCmd !== 'v' && currCmd !== 'h') {
          coord2 = DvtMapProviderUtils._getCoord(path, ++i); // Update idx
        ...
        coordAr.push([currX, -currY]); // Update command

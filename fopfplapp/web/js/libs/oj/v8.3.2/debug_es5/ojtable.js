...
(function () {
  oj.__registerWidget('oj.ojTable', $.oj.baseComponent, {
    ...
    options: {
       ...
       */
      selected: {
        row: new oj.KeySetImpl(),
        column: new oj.KeySetImpl()
      },
     ...
     */
    getDataForVisibleRow: function getDataForVisibleRow(rowIndex) {
    ...
    getNodeBySubId: function getNodeBySubId(locator) {
      ...
      if (subId === 'oj-table-cell') {
      ...
      } else if (subId === 'oj-table-header' || subId === 'oj-table-sort-ascending' || subId === 'oj-table-sort-descending') {
        ...
        if (tableHeaderColumn != null) {
          if (subId === 'oj-table-header') {
            ...
            var tableHeaderColumnSortAsc = domUtils.getTableElementsByClassName(tableHeaderColumn, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_ASC_ICON_CLASS);

            if (tableHeaderColumnSortAsc.length > 0) {
              return tableHeaderColumnSortAsc[0];
            }
          } else {
            var tableHeaderColumnSortDsc = domUtils.getTableElementsByClassName(tableHeaderColumn, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_DSC_ICON_CLASS);
    ...
    // @inheritdoc
    getSubIdByNode: function getSubIdByNode(node, ignoreSortIcons) {
      var domUtils = this._getTableDomUtils();

      var cell = domUtils.getFirstAncestor(node, '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_CLASS, true);
      ...
      var headerSortAsc = domUtils.getFirstAncestor(node, '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_ASC_ICON_CLASS, true);
      ...
      var headerSortDsc = domUtils.getFirstAncestor(node, '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_DSC_ICON_CLASS, true);
      ...
      var header = domUtils.getFirstAncestor(node, '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);
      ...
      var footer = domUtils.getFirstAncestor(node, '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS, true);
     ...
     */
    refreshRow: function refreshRow(rowIdx) {
     ...
     * @private
     */
    _refreshRow: function _refreshRow(rowIdx, resetFocus) {
      var dataprovider = this._getData(); // if no data then bail
      ...
      return self._queueTask(function () {
        return dataprovider.fetchByKeys({
          ...
          var tableBodyRow = self._getTableDomUtils().getTableBodyRow(rowIdx); // eslint-disable-next-line no-param-reassign
    ...
    _ComponentCreate: function _ComponentCreate() {
      ...
      this._registerDomEventListeners(); // cache the options
    ...
    _AfterCreate: function _AfterCreate() {
      this._super(); // create the context menu
    ...
    _SetupResources: function _SetupResources() {
      ...
      this._registerResizeListener(); // register event listeners for table on the datasource so that the table
     ...
     * @private
     */
    _cleanComponent: function _cleanComponent(isDestroy) {
      ...
      if (isDestroy) {
        ...
        this.element.children().remove('.' + oj.TableDomUtils.CSS_CLASSES._TABLE_BODY_MESSAGE_ROW_CLASS); //  - DomUtils.unwrap() will avoid unwrapping if the node is being destroyed by Knockout

        oj.DomUtils.unwrap(this.element, $(this._getTableDomUtils().getTableContainer()));
        this.element[0].classList.remove(oj.TableDomUtils.CSS_CLASSES._TABLE_CLASS); // If this._data is a TableDataSourceAdapter, call destroy so that it can remove listeners
     ...
     * @private
     */
    _clearAllComponentTimeouts: function _clearAllComponentTimeouts() {
     ...
     * @private
     */
    _clearShowStatusTimeout: function _clearShowStatusTimeout() {
     ...
     * @private
     */
    _NotifyContextMenuGesture: function _NotifyContextMenuGesture(contextMenu, event, eventType) {
      var openOptions = {};
      this._contextMenuEvent = event.originalEvent; // first check if we are invoking on an editable or clickable element, or draggable element on touch event. If so bail

      if (this._isNodeEditable(this._contextMenuEvent.target) || this._isNodeClickable(this._contextMenuEvent.target) || eventType === 'touch' && this._isNodeDraggable(this._contextMenuEvent.target)) {
        return;
      }
      ...
      var tableBodyCell = domUtils.getFirstAncestor(this._contextMenuEvent.target, '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_CLASS, true);
      ...
      if (tableBodyCell != null) {
        ...
        headerColumn = domUtils.getFirstAncestor(this._contextMenuEvent.target, '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);
      }
      ...
      if (this._contextMenuEvent.type === 'keydown') {
        ...
        var footerCell = domUtils.getFirstAncestor(this._contextMenuEvent.target, '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS, true);
      ...
      this._contextMenuEventHeaderColumn = headerColumn; // for jQuery it's the menu instance, for custom element it's the element node
      ...
      if (headerColumn && headerColumn.getAttribute('data-oj-sortable') === this._OPTION_ENABLED) {
        contextMenuItemAsc = contextMenuNode.querySelectorAll('[data-oj-command=oj-table-sortAsc]');
        ...
        contextMenuItemDsc = contextMenuNode.querySelectorAll('[data-oj-command=oj-table-sortDsc]');
      ...
      if (headerColumn && headerColumn.getAttribute('data-oj-resizable') === this._OPTION_ENABLED) {
        contextMenuItemResize = contextMenuNode.querySelectorAll('[data-oj-command=oj-table-resize]');
    ...
    _draw: function _draw() {
      ...
      this.element[0].setAttribute(oj.TableDomUtils.DOM_ATTR._ROLE, 'table'); // create the initial table structure

      this._getTableDomUtils().createInitialTable(this._isTableHeaderless(), this._isTableFooterless()); // style the initial table structure
      ...
      this._queueTask(function () {
        ...
        self._refreshTableHeader(); // populate the table footer DOM with footer content
    ...
    _events: {
      ...
      focusout: function focusout(event) {
        ...
        var table = domUtils.getTable(); // Components that open popups (such as ojSelect, ojCombobox, ojInputDate, etc.) will trigger
        ...
        for (var i = 0; i < popups.length; i++) {
          ...
          var launcher = oj.DomUtils.getLogicalParent($(popupElem)); // Check if the table contains the launcher
        ...
        this._focusoutTimeout = setTimeout(function () {
          ...
          if (this._focusEditCell) {
            this._focusEditCell.classList.remove(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_EDIT_CLASS);
      ...
      focusin: function focusin(event) {
        ...
        if ($(tableBody).has(event.target).length > 0) {
          ...
          if (!this._isNodeEditable(event.target) && !this._isNodeClickable(event.target) && !this._isTableActionableMode() && !this._hasEditableRow() && !this._temporaryTableChildElementFocus && focusedRowIdx == null && focusedHeaderColumnIdx == null) {
       ...
       */
      'focus .oj-table-data-cell': function focusOjTableDataCell(event) {
        ...
        if (rowIdx === this._getEditableRowIdx()) {
          ...
          if (this._focusEditCell && this._focusEditCell !== eventTarget && this._focusEditCell.classList.contains(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_EDIT_CLASS)) {
            this._focusEditCell.classList.remove(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_EDIT_CLASS);
       ...
       */
      'click .oj-table-checkbox-acc-select-column': function clickOjTableCheckboxAccSelectColumn(event) {
        var eventTarget = this._getEventTargetElement(event);

        var columnIdx = this._getTableDomUtils().getElementColumnIdx(eventTarget);

        var selected = $(eventTarget).is(':checked'); // if selected then focus on the column
       ...
       */
      'click .oj-table-checkbox-acc-select-row': function clickOjTableCheckboxAccSelectRow(event) {
        var eventTarget = this._getEventTargetElement(event);

        var rowIdx = this._getTableDomUtils().getElementRowIdx(eventTarget);

        var selected = $(eventTarget).is(':checked');
        var focused = false; // if selected then focus on the row
      ...
      keydown: function keydown(event) {
        this._addKeyboardKey(event.keyCode); // ignore key event on the footer or target is editable


        var keyboardCode1 = this._getKeyboardKeys()[0];

        if (keyboardCode1 !== this._KEYBOARD_CODES._KEYBOARD_CODE_ESC && keyboardCode1 !== this._KEYBOARD_CODES._KEYBOARD_CODE_ENTER && keyboardCode1 !== this._KEYBOARD_CODES._KEYBOARD_CODE_F2 && keyboardCode1 !== this._KEYBOARD_CODES._KEYBOARD_CODE_TAB && (this._isNodeEditable(event.target) || this._getTableDomUtils().getTableFooter() != null && $(this._getTableDomUtils().getTableFooter()).has(event.target).length > 0)) {
          return;
        } // process single or two key events


        if (this._getKeyboardKeys().length === 1 || this._getKeyboardKeys().length === 2 && event[this._KEYBOARD_CODES._KEYBOARD_MODIFIER_SHIFT]) {
          if (this._isKeyboardKeyPressed(this._KEYBOARD_CODES._KEYBOARD_CODE_UP) || this._isKeyboardKeyPressed(this._KEYBOARD_CODES._KEYBOARD_CODE_DOWN) || this._isKeyboardKeyPressed(this._KEYBOARD_CODES._KEYBOARD_CODE_HOME) || this._isKeyboardKeyPressed(this._KEYBOARD_CODES._KEYBOARD_CODE_END)) {
          ...
          if (this._isKeyboardKeyPressed(this._KEYBOARD_CODES._KEYBOARD_CODE_UP) || this._isKeyboardKeyPressed(this._KEYBOARD_CODES._KEYBOARD_CODE_DOWN)) {
            this._handleKeydownUpDown(event, isExtend && this._getRowSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE);
          } else if (this._isKeyboardKeyPressed(this._KEYBOARD_CODES._KEYBOARD_CODE_LEFT) || this._isKeyboardKeyPressed(this._KEYBOARD_CODES._KEYBOARD_CODE_RIGHT)) {
            this._handleKeydownLeftRight(event, isExtend && this._getColumnSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE);
      ...
      keyup: function keyup(event) {
        ...
        if (this._getKeyboardKeys().length === 1) {
          var keyboardCode1 = this._getKeyboardKeys()[0]; // ignore key event on the footer or target is editable


          if (this._isNodeEditable(event.target) || this._getTableDomUtils().getTableFooter() != null && $(this._getTableDomUtils().getTableFooter()).has(event.target).length > 0) {
       ...
       */
      'mouseleave .oj-table-element': function mouseleaveOjTableElement() {
        this._getTableResizeUtils().clearTableHeaderColumnsResize();
      },
       ...
       */
      'mousedown .oj-table-body': function mousedownOjTableBody(event) {
       ...
       * Keep track of mousedown/mouseup for multiple selection
       */
      'mouseup .oj-table-body': function mouseupOjTableBody() {
        this._mouseDownRowIdx = null;
      },
       ...
       */
      'mouseenter .oj-table-body-row': function mouseenterOjTableBodyRow(event) {
        var eventTarget = this._getEventTargetElement(event);
       ...
       */
      'mouseleave .oj-table-body-row': function mouseleaveOjTableBodyRow(event) {
        var eventTarget = this._getEventTargetElement(event);
       ...
       */
      'mousedown .oj-table-column-header-cell': function mousedownOjTableColumnHeaderCell(event) {
        this._lastSelectedHeaderIdx = null;

        if (event.which === 1) {
          if (!this._getTableResizeUtils().handleHeaderColumnResizeStart(event)) {
            // get the column index
            var columnIdx = this._getTableDomUtils().getElementColumnIdx(this._getEventTargetElement(event)); // set the column focus if shift key is not pressed
       ...
       */
      'mouseenter .oj-table-column-header-cell': function mouseenterOjTableColumnHeaderCell(event) {
       ...
       */
      'mousemove .oj-table-header': function mousemoveOjTableHeader(event) {
       ...
       */
      'mousemove .oj-table-column-header-cell': function mousemoveOjTableColumnHeaderCell(event) {
        var eventTarget = this._getEventTargetElement(event);
       ...
       */
      'mouseleave .oj-table-column-header-cell': function mouseleaveOjTableColumnHeaderCell(event) {
        var eventTarget = this._getEventTargetElement(event);
       ...
       */
      'mouseup .oj-table-column-header-cell': function mouseupOjTableColumnHeaderCell(event) {
       ...
       */
      'mousedown .oj-table-data-cell': function mousedownOjTableDataCell(event) {
        this._lastSelectedRowIdx = null; // get the row index of the cell element

        var eventTarget = this._getEventTargetElement(event);

        var rowIdx = this._getTableDomUtils().getElementRowIdx(eventTarget); // set row focus only if shift key is not pressed
       ...
       * handle column resizing.
       */
      'mouseup .oj-table-data-cell': function mouseupOjTableDataCell(event) {
        this._getTableResizeUtils().handleHeaderColumnResizeEnd(event);
      },
       ...
       */
      'click .oj-table-body-row': function clickOjTableBodyRow(event) {
        ...
        if (isShift) {
          ...
          this._handleSelectionGesture(rowIdx, true, true); // ctrl/meta (only) key pressed, update selection anchor
          ...
          this._handleSelectionGesture(rowIdx, true, isTouch && this._nonContiguousSelection); // update selection anchor
          ...
          if (isTouch && rowSelected && !this._nonContiguousSelection && this._getRowSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE) {
       ...
       * Set row to editable.
       */
      'dblclick .oj-table-data-cell': function dblclickOjTableDataCell(event) {
        var columnIdx = this._getTableDomUtils().getElementColumnIdx(event.target);
       ...
       */
      'contextmenu .oj-table-data-cell': function contextmenuOjTableDataCell(event) {
        // get the row index of the cell element
       ...
       */
      'click .oj-table-column-header-cell': function clickOjTableColumnHeaderCell(event) {
        // get the column index
        var eventTarget = this._getEventTargetElement(event);
        ...
        var columnIdx = this._getTableDomUtils().getElementColumnIdx(eventTarget); // check if we need to focus
        ...
        if (isShift) {
          ...
          if (focusedHeaderColumnIdx != null && this._getColumnSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE) {
       ...
       */
      'dragstart .oj-table-column-header-cell': function dragstartOjTableColumnHeaderCell(event) {
        var eventTarget = this._getEventTargetElement(event);
       ...
       */
      'dragenter .oj-table-column-header-cell': function dragenterOjTableColumnHeaderCell(event) {
       ...
       */
      'dragover .oj-table-column-header-cell': function dragoverOjTableColumnHeaderCell(event) {
       ...
       */
      'dragleave .oj-table-column-header-cell': function dragleaveOjTableColumnHeaderCell(event) {
       ...
       */
      'drop .oj-table-column-header-cell': function dropOjTableColumnHeaderCell(event) {
       ...
       */
      'dragend .oj-table-column-header-cell': function dragendOjTableColumnHeaderCell(event) {
       ...
       */
      'dragstart .oj-table-body-row': function dragstartOjTableBodyRow(event) {
       ...
       */
      'drag .oj-table-body-row': function dragOjTableBodyRow(event) {
       ...
       */
      'dragend .oj-table-body-row': function dragendOjTableBodyRow(event) {
       ...
       */
      'dragenter .oj-table-body': function dragenterOjTableBody(event) {
       ...
       */
      'dragover .oj-table-body': function dragoverOjTableBody(event) {
       ...
       */
      'dragleave .oj-table-body': function dragleaveOjTableBody(event) {
       ...
       */
      'drop .oj-table-body': function dropOjTableBody(event) {
    ...
    _setOption: function _setOption(key, value) {
      if (key === 'selection') {
        this._selectionSet = true; // update row index/key values in the 'selection' option

        this._syncRangeSelection(value);

        this._superApply(arguments); // after applying the new selection value, sync the selection state
        ...
        this._superApply(arguments); // after applying the new selected value, sync the selection state
     ...
     * @private
     */
    _invalidateRangeSelection: function _invalidateRangeSelection() {
     ...
     * @private
     */
    _beforeDataRefresh: function _beforeDataRefresh() {
      this._clearIdleCallback();
     ...
     * @private
     */
    _adjustScrollPositionOnRefresh: function _adjustScrollPositionOnRefresh() {
      ...
      if (selectedRowIdxs != null && selectedRowIdxs.length > 0 && this._isRowSelectionEnabled()) {
        if (!isNaN(this._selectionAnchorIdx) && selectedRowIdxs.indexOf(this._selectionAnchorIdx) > -1) {
        ...
        // remain at the top
        scrollPosition.y = 0;
        scrollPosition.rowIndex = 0; // row key gets re-computed after refresh
      ...
      scrollPosition.offsetY = 0; // explicitly set changed to false so option change will not be triggered, syncScrollPosition
     ...
     * @private
     */
    _isScrollPositionAdjusted: function _isScrollPositionAdjusted() {
     ...
     * @private
     */
    _getColumnKeys: function _getColumnKeys() {
      var columnKeys = [];
     ...
     * @private
     */
    _getLocalRowKeys: function _getLocalRowKeys() {
      var rowKeys = [];
     ...
     * @private
     */
    _syncTableSizing: function _syncTableSizing(skipRequiredCheck) {
      // don't provide a width and height to ensure we resize contents even if overall size is the same
      this._getTableDomUtils().refreshTableDimensions(null, null, skipRequiredCheck); // if loadMoreOnScroll then check if we have underflow and do a fetch if we do


      if (this._isLoadMoreOnScroll() && !this._dataFetching && this._domScroller) {
        this._setDataWaitingState(false);

        this._domScroller.checkViewport().then(this._domScrollerSuccessFunc, this._clearDataWaitingState); // syncScrollPosition would be invoked after data are fetched and new rows are rendered
     ...
     * @private
     */
    _syncScrollPosition: function _syncScrollPosition(position, skipKeyValidation) {
      // only supported in custom element.  Check whether it's scrollable
      if (!this._IsCustomElement() || this._noDataMessageShown || !this._isScrollableX() && !this._isScrollableY()) {
        return true;
      }
      ...
      if (!skipKeyValidation && position.rowKey != null) {
        ...
        if (promise) {
          ...
          promise.then(function (valid) {
            ...
            this._clearComponentBusyState(keyValidationResolveFunc); // try again
      ...
      var scrollLeft = isNaN(this._scrollLeft) ? 0 : this._scrollLeft; // check if only x updated
     ...
     * @private
     */
    _setScrollX: function _setScrollX(x) {
      var scroller = this._getTableDomUtils().getScroller(); // flag it so that handleScroll won't do anything
     ...
     * @private
     */
    _setScrollY: function _setScrollY(y) {
      var scroller = this._getTableDomUtils().getScroller(); // flag it so that handleScroll won't do anything


      this._skipScrollUpdate = true;
      var initialScrollTop = scroller.scrollTop;
      scroller.scrollTop = y === -1 ? this._getScrollHeight() - this._getClientHeight() : y; // only set scroll pos busy state if a scroll was triggered
     ...
     * @private
     */
    _getScrollTopByIndex: function _getScrollTopByIndex(rowIndex) {
     ...
     * @private
     */
    _getScrollTopByKey: function _getScrollTopByKey(rowKey) {
     ...
     * @private
     */
    _getScrollLeftByIndex: function _getScrollLeftByIndex(columnIndex) {
      if (columnIndex === 0) {
        return 0;
      }
     ...
     * @private
     */
    _getScrollLeftByKey: function _getScrollLeftByKey(columnKey) {
    ...
    _getCurrentHorizontalScrollPosition: function _getCurrentHorizontalScrollPosition(scrollLeft) {
      var scrollPosition = {};
    ...
    _getCurrentVerticalScrollPosition: function _getCurrentVerticalScrollPosition(scrollTop) {
      var scrollPosition = {};
      scrollPosition.y = scrollTop === undefined ? this._scrollTop : scrollTop; // precheck for row Height in case it wasn't set earlier.
     ...
     * @private
     */
    _getCurrentScrollPosition: function _getCurrentScrollPosition(scrollLeft, scrollTop) {
     ...
     * @private
     */
    _getColumnLocations: function _getColumnLocations() {
    ...
    _findClosestElementToTop: function _findClosestElementToTop(scrollPosition, currScrollTop) {
      ...
      if (Math.abs(prevScrollPosition.y - currScrollTop) < this._rowHeight && prevScrollPosition.rowKey != null && !isNaN(prevScrollPosition.rowIndex)) {
    ...
    _hasMoreToFetch: function _hasMoreToFetch() {
     ...
     * @private
     */
    _addKeyboardKey: function _addKeyboardKey(keyCode) {
      var foundCode = false;
    ...
    _addComponentBusyState: function _addComponentBusyState(msg) {
      ...
      var options = {
        description: "The component identified by '" + this._getTableDomUtils().getTableId() + "' " + msg
      };
     ...
     * @private
     */
    _createComponentBusyState: function _createComponentBusyState(msg) {
     ...
     * @private
     */
    _clearComponentBusyState: function _clearComponentBusyState(busyStateResolveFunc) {
     ...
     * @private
     */
    _clearBusyStateStack: function _clearBusyStateStack() {
      if (this._busyStateStack) {
     ...
     * @private
     */
    _setDelayRefreshTableDimensionsBusyState: function _setDelayRefreshTableDimensionsBusyState() {
     ...
     * @private
     */
    _clearDelayRefreshTableDimensionsBusyState: function _clearDelayRefreshTableDimensionsBusyState() {
      if (this._delayRefreshTableDimensionsResolveFunc) {
     ...
     * @private
     */
    _setIdleRenderBusyState: function _setIdleRenderBusyState() {
     ...
     * @private
     */
    _clearIdleRenderBusyState: function _clearIdleRenderBusyState() {
      if (this._idleRenderResolveFunc) {
     ...
     * @private
     */
    _setScrollPosBusyState: function _setScrollPosBusyState() {
     ...
     * @private
     */
    _clearScrollPosBusyState: function _clearScrollPosBusyState() {
      if (this._scrollPosResolveFunc) {
     ...
     * @private
     */
    _clearAllComponentBusyStates: function _clearAllComponentBusyStates() {
     ...
     * @private
     */
    _addSingleTableBodyRow: function _addSingleTableBodyRow(rowIdx, row, docFrag, docFragStartIdx) {
      var tableBodyRow = this._getTableDomUtils().createTableBodyRow(rowIdx, row[this._CONST_KEY]);

      this._getTableDomUtils().styleTableBodyRow(tableBodyRow, true); // insert the <tr> element in to the table body DOM
     ...
     * @private
     */
    _animateVisibleRows: function _animateVisibleRows(tableBodyRows, rowIdxArray, action) {
      // eslint-disable-next-line no-param-reassign
     ...
     * @private
     */
    _checkRowOrHeaderColumnFocus: function _checkRowOrHeaderColumnFocus(event) {
      var focusedRowIdx = this._getFocusedRowIdx();
     ...
     * @private
     */
    _cleanTemplateNodes: function _cleanTemplateNodes(rootNode) {
     ...
     * @private
     */
    _clearCachedMetadata: function _clearCachedMetadata() {
     ...
     * @private
     */
    _clearCachedDataMetadata: function _clearCachedDataMetadata() {
      if (this._data != null) {
        this._unregisterDataSourceEventListeners();
      }
    ...
    _clearCachedDimensions: function _clearCachedDimensions() {
     ...
     * @private
     */
    _clearDataWaitingState: function _clearDataWaitingState() {
     ...
     * @private
     */
    _clearKeyboardKeys: function _clearKeyboardKeys() {
      this._keyboardKeys = [];
    },
     ...
     * @private
     */
    _clearFocusedHeaderColumn: function _clearFocusedHeaderColumn() {
      var focusedHeaderColumnIdx = this._getFocusedHeaderColumnIdx();
     ...
     * @private
     */
    _clearFocusedRow: function _clearFocusedRow(updateCurrentRow) {
      var focusedRowIdx = this._getFocusedRowIdx();
     ...
     * @private
     */
    _clearSelectedRows: function _clearSelectedRows() {
     ...
     * @private
     */
    _clearSelectionState: function _clearSelectionState() {
      this._selectionAnchorIdx = null;
      this._lastSelectedRowIdx = null;
      this._lastSelectedHeaderIdx = null;
     ...
     * @private
     */
    _clearSortedHeaderColumn: function _clearSortedHeaderColumn(columnIdx) {
     ...
     * @private
     */
    _executeTableBodyRowsAdd: function _executeTableBodyRowsAdd(eventDetail) {
      var self = this;
      ...
      if (eventDetailKeys && eventDetailKeys.size > 0) {
        this._queueTask(function () {
          ...
          self._getTableDomUtils().clearCachedDomRowData(); // row values may have changed so refresh the footer


          self._refreshTableFooter(); // for high watermark scrolling, we need to reset the domscroller in case there are more rows to fetch
    ...
    _executeTableBodyRowsChange: function _executeTableBodyRowsChange(eventDetail, startIndex) {
      var self = this;
     ...
     * @private
     */
    _executeTableBodyRowsRemove: function _executeTableBodyRowsRemove(eventDetail) {
      var self = this;
      var eventDetailKeys = eventDetail[this._CONST_KEYS];
      ...
      if (eventDetailKeys && eventDetailKeys.size > 0) {
        this._queueTask(function () {
          ...
          var currentRowKey = currentRow != null ? currentRow.rowKey : null; // first check if we are removing all rows. If so, we can do a removeAll
     ...
     * @private
     */
    _getColumnMetadata: function _getColumnMetadata() {
      // get the columns metadata
      var columns = this.options.columns;
      var columnsDefault = this.options.columnsDefault;

      if ((columns.length === 0 || columns.length === 1 && columns[0].id == null && columns[0].headerText == null && columns[0].field == null) && columnsDefault.headerText == null && columnsDefault.field == null) {
        return [];
      }
      ...
      var columnsSortedArray = []; // add the rest of the columns in the array
      ...
      for (i = 0; i < defaultedColumnsCount; i++) {
        ...
        if ((columnsSortedArray[i].sortable == null || columnsSortedArray[i].sortable === this._OPTION_AUTO) && sortSupportedData) {
     ...
     * @private
     */
    _getColumnIdxForColumnKey: function _getColumnIdxForColumnKey(columnKey) {
      var columns = this._getColumnDefs();
     ...
     * @private
     */
    _getColumnIdxsForElementsWithStyleClass: function _getColumnIdxsForElementsWithStyleClass(styleClass) {
      var domUtils = this._getTableDomUtils();
     ...
     * @private
     */
    _getFirstVisibleColumnIndex: function _getFirstVisibleColumnIndex(currentIndex, isForward) {
      var columns = this._getColumnDefs();
     ...
     * @private
     */
    _getNextVisibleColumnIndex: function _getNextVisibleColumnIndex(currentIndex, isForward) {
      var startIndex;
     ...
     * @private
     */
    _getColumnKeyForColumnIdx: function _getColumnKeyForColumnIdx(columnIdx) {
      var columns = this._getColumnDefs();
     ...
     * @private
     */
    _getColumnRenderer: function _getColumnRenderer(columnIdx, type) {
      var columns = this._getColumnDefs();
     ...
     * @private
     */
    _getCurrentRow: function _getCurrentRow() {
      var dataprovider = this._getData(); // if no data then bail
    ...
    _getData: function _getData() {
      if (!this._data && this.options.data != null) {
        ...
        if (oj.TableDataSource && dataprovider instanceof oj.TableDataSource || oj.PagingTableDataSource && dataprovider instanceof oj.PagingTableDataSource) {
          this._data = new oj.TableDataSourceAdapter(dataprovider);
        } else if (oj.DataProviderFeatureChecker.isDataProvider(dataprovider)) {
          if (!(dataprovider instanceof oj.ListDataProviderView) && !oj.DataProviderFeatureChecker.isTreeDataProvider(dataprovider)) {
     ...
     * @private
     */
    _getFocusedHeaderColumnIdx: function _getFocusedHeaderColumnIdx() {
      // focused column headers have the focused style class. There should only be one focused header
      return this._getColumnIdxsForElementsWithStyleClass('.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS + '.' + oj.TableDomUtils.MARKER_STYLE_CLASSES._FOCUS_HIGHLIGHT + ',' + '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS + '.' + oj.TableDomUtils.MARKER_STYLE_CLASSES._FOCUS)[0];
    },
     ...
     * @private
     */
    _getFocusedRowIdx: function _getFocusedRowIdx() {
      // focused rows have cells with focused style class. There should only be one focused row
      return this._getRowIdxsForElementsWithStyleClass('.' + oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_CLASS + '.' + oj.TableDomUtils.MARKER_STYLE_CLASSES._FOCUS_HIGHLIGHT + ',' + '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_CLASS + '.' + oj.TableDomUtils.MARKER_STYLE_CLASSES._FOCUS)[0];
    },
     ...
     * @private
     */
    _getHeaderColumnSelection: function _getHeaderColumnSelection(columnIdx) {
     ...
     * @private
     */
    _getColumnSelectionMode: function _getColumnSelectionMode() {
     ...
     * @private
     */
    _isColumnSelectionEnabled: function _isColumnSelectionEnabled() {
      var colSelectionMode = this._getColumnSelectionMode();

      return colSelectionMode === this._OPTION_SELECTION_MODES._SINGLE || colSelectionMode === this._OPTION_SELECTION_MODES._MULTIPLE;
    },
     ...
     * @private
     */
    _getKeyboardKeys: function _getKeyboardKeys() {
     ...
     * @private
     */
    _getRowIdxRowArray: function _getRowIdxRowArray(resultObject, startIndex) {
      var rowIdxRowArray = [];
        ...
        for (var i = 0; i < indexesCount; i++) {
          rowIdxRowArray.push({
            row: {
              ...
              metadata: resultObject[this._CONST_METADATA] ? resultObject[this._CONST_METADATA][i] : null,
     ...
     * @private
     */
    _getEditableRowIdx: function _getEditableRowIdx() {
      var rowIdx = null;
     ...
     * @private
     */
    _getEditableRowKey: function _getEditableRowKey() {
     ...
     * @private
     */
    _getEventTargetElement: function _getEventTargetElement(event) {
     ...
     * @private
     */
    _getRowIdxsForElementsWithStyleClass: function _getRowIdxsForElementsWithStyleClass(styleClass) {
      var domUtils = this._getTableDomUtils();
     ...
     * @private
     */
    _getRowKeyForRowIdx: function _getRowKeyForRowIdx(rowIdx) {
     ...
     * @private
     */
    _getRowKey: function _getRowKey(tableBodyRow, index) {
      if (tableBodyRow == null) {
        // eslint-disable-next-line no-param-reassign
     ...
     * @private
     */
    _getRowRenderer: function _getRowRenderer() {
     ...
     * @private
     */
    _getRowSelection: function _getRowSelection(rowIdx) {
     ...
     * @private
     */
    _getRowSelectionMode: function _getRowSelectionMode() {
     ...
     * @private
     */
    _isRowSelectionEnabled: function _isRowSelectionEnabled() {
      var rowSelectionMode = this._getRowSelectionMode();

      return rowSelectionMode === this._OPTION_SELECTION_MODES._SINGLE || rowSelectionMode === this._OPTION_SELECTION_MODES._MULTIPLE;
    },
     ...
     * @private
     */
    _getSelectedHeaderColumnIdxs: function _getSelectedHeaderColumnIdxs() {
     ...
     * @private
     */
    _getSelectedRowIdxs: function _getSelectedRowIdxs() {
     ...
     * @private
     */
    _getSlotTemplate: function _getSlotTemplate(slotName) {
     ...
     * @private
     */
    _isDefaultCellTemplateSlotValid: function _isDefaultCellTemplateSlotValid() {
     ...
     * @private
     */
    _isDefaultHeaderTemplateSlotValid: function _isDefaultHeaderTemplateSlotValid() {
     ...
     * @private
     */
    _isDefaultFooterTemplateSlotValid: function _isDefaultFooterTemplateSlotValid() {
     ...
     * @private
     */
    _isDefaultTemplateSlotValid: function _isDefaultTemplateSlotValid(defaultSlotName) {
      var columns = this._getColumnDefs();
      ...
      for (var i = 0; i < columnsCount; i++) {
        ...
        if (defaultSlotName === cellTemplateName || defaultSlotName === headerTemplateName || defaultSlotName === footerTemplateName) {
          return false;
        }
      }
     ...
     * @private
     */
    _getSortedTableHeaderColumnIdx: function _getSortedTableHeaderColumnIdx() {
     ...
     * @private
     */
    _getTabbableElements: function _getTabbableElements(element, ignoreHidden) {
     ...
     * @private
     */
    _getTableAnimationUtils: function _getTableAnimationUtils() {
     ...
     * @private
     */
    _getTableDndContext: function _getTableDndContext() {
     ...
     * @private
     */
    _getTableDomUtils: function _getTableDomUtils() {
     ...
     * @private
     */
    _getTableResizeUtils: function _getTableResizeUtils() {
     ...
     * @private
     */
    _getTouchEventTargetElement: function _getTouchEventTargetElement(event) {
     ...
     * @private
     */
    _getVisibleRowIdxs: function _getVisibleRowIdxs() {
      ...
      if (tableBodyRows.length > 0 && tableBody.offsetHeight > 0) {
        ...
        var tableElemX = tableBodyRect.left >= 0 ? tableBodyRect.left + 1 : tableBodyRect.right - 1; // check the first visible row
     ...
     * @private
     */
    _selectRange: function _selectRange(firstSelectedIndex, lastSelectedIndex, isRow) {
      ...
      if (isRow) {
        ...
        if (this._isRowSelectionEnabled()) {
          rowKeySet = rowKeySet.clear(); // selected rows should be added in the order that they are selected
        ...
        if (this._isColumnSelectionEnabled()) {
          columnKeySet = columnKeySet.clear(); // selected columns should be added in the order that they are selected
    ...
    _handleContextMenuSelect: function _handleContextMenuSelect(event, ui) {
      ...
      var headerColumn = domUtils.getFirstAncestor(this._contextMenuEvent.target, '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);
      ...
      if (menuItemCommand === 'oj-table-sortAsc') {
        ...
        this._getTableDomUtils().removeTableBodyRowTouchSelectionAffordance(); // update to disable command
        ...
        this._nonContiguousSelection = false; // update to enable command
     ...
     * @private
     */
    _handleContextMenuResizePopup: function _handleContextMenuResizePopup() {
      var domUtils = this._getTableDomUtils();
     ...
     * @private
     */
    _handleDataFetchStart: function _handleDataFetchStart() {
     ...
     * @private
     */
    _handleDataRowMutate: function _handleDataRowMutate(event) {
      if (this._dataFetching) {
        return;
      }
     ...
     * @private
     */
    _getRowsFromEventDetailAdd: function _getRowsFromEventDetailAdd(eventDetail) {
      var dataprovider = this._getData();
      ...
      var eventAfterKeys = []; // afterKeys is deprecated, but continue to support it until we can remove it.
      // forEach can be called on both array and set.

      var afterKeyIter = eventDetail[this._CONST_ADDBEFOREKEYS] ? eventDetail[this._CONST_ADDBEFOREKEYS] : eventDetail[this._CONST_AFTERKEYS];
      ...
      if (dataprovider instanceof oj.TableDataSourceAdapter) {
        ...
        } else if (oj.PagingTableDataSource && dataprovider.tableDataSource instanceof oj.PagingTableDataSource && oj.FlattenedTreeTableDataSource && dataprovider.tableDataSource.dataSource instanceof oj.FlattenedTreeTableDataSource) {
      ...
      var maxRowIndex = Math.max(fetchSize, rowCount); // if specified, afterKeys takes precendence over indexes
      ...
      for (i = 0; i < eventDataCount; i++) {
        ...
        if (!isNaN(eventIndex)) {
          rowIdx = eventIndex - offset; // for high watermark scrolling, we only care about rows in our current viewport
     ...
     * @private
     */
    _handleDataRowAdd: function _handleDataRowAdd(eventDetail) {
      try {
     ...
     * @private
     */
    _getRowsFromEventDetailChange: function _getRowsFromEventDetailChange(eventDetail) {
      var eventData = eventDetail[this._CONST_DATA];
      var eventMetadata = eventDetail[this._CONST_METADATA];
      var eventKeys = [];
     ...
     * @private
     */
    _handleDataRowChange: function _handleDataRowChange(eventDetail) {
      try {
     ...
     * @private
     */
    _getRowsFromEventDetailRemove: function _getRowsFromEventDetailRemove(eventDetail) {
      var self = this;
      var rowArray = [];
     ...
     * @private
     */
    _handleDataRowRemove: function _handleDataRowRemove(eventDetail) {
      try {
     ...
     * @private
     */
    _handleKeydownEnter: function _handleKeydownEnter(event) {
      ...
      if (focusedColumnIdx != null && this._getColumnDefs()[focusedColumnIdx].sortable === this._OPTION_ENABLED) {
        ...
        var sorted = $(tableHeaderColumn).data('sorted'); // if not already sorted then sort ascending. If already sorted
     ...
     * @private
     */
    _handleKeydownF2: function _handleKeydownF2(event) {
      event.preventDefault();
      event.stopPropagation();
    ...
    _handleKeydownLeftRight: function _handleKeydownLeftRight(event, isExtend) {
      ...
      if (focusedHeaderIndex != null) {
        ...
        if (isExtend) {
          this._isNavigate = false; // select all columns between focusedHeaderIndex and target
    ...
    _handleKeydownSpacebar: function _handleKeydownSpacebar(event) {
      ...
      event.preventDefault();
      event.stopPropagation(); // pressing spacebar selects the focused row/column
     ...
     * @private
     */
    _handleKeydownTab: function _handleKeydownTab(event) {
      ...
      if ((focusedRowIdx != null || focusedHeaderColumnIdx != null) && (this._isTableActionableMode() || this._hasEditableRow())) {
        ...
        if (this._getEditableRowIdx() === focusedRowIdx) {
          ...
          if (rowElementTabIndex === tabbableElementsInRowCount - 1 && !event[this._KEYBOARD_CODES._KEYBOARD_MODIFIER_SHIFT]) {
            ...
            event.stopPropagation();
            return;
          } else if (rowElementTabIndex === 0 && event[this._KEYBOARD_CODES._KEYBOARD_MODIFIER_SHIFT]) {
      ...
      if (!tabHandled) {
        ...
        var tabbableElementsInTableCount = tabbableElementsInTable != null ? tabbableElementsInTable.length : 0; // Table should not set focus to elements outside of itself because
    ...
    _handleKeydownUpDown: function _handleKeydownUpDown(event, isExtend) {
      ...
      if (focusedRowIdx != null) {
        ...
        if (isExtend) {
          this._isNavigate = false; // select all rows between focusedRowIdx and target
     ...
     * @private
     */
    _handleMouseEnterColumnHeader: function _handleMouseEnterColumnHeader(element) {
      if (element) {
        element.classList.add(oj.TableDomUtils.MARKER_STYLE_CLASSES._HOVER); // get the column index of the header element
     ...
     * @private
     */
    _handleMouseLeaveColumnHeader: function _handleMouseLeaveColumnHeader(element) {
      if (element) {
        element.classList.remove(oj.TableDomUtils.MARKER_STYLE_CLASSES._HOVER); // get the column index of the header element

        var columnIdx = this._getTableDomUtils().getElementColumnIdx(element); // hide the asc/dsc links for the header
     ...
     * @private
     */
    _handleMouseEnterSelection: function _handleMouseEnterSelection(element, isTouchAffordance) {
     ...
     * @private
     */
    _handleScrollerMaxRowCount: function _handleScrollerMaxRowCount() {
     ...
     * @private
     */
    _handleScrollerScrollLeft: function _handleScrollerScrollLeft(scrollLeft) {
     ...
     * @private
     */
    _handleScrollerScrollTop: function _handleScrollerScrollTop(scrollTop) {
      ...
      this._scrollTop = scrollTop; // keep track when scrollTop is triggered by self (syncScrollPosition)
     ...
     * @private
     */
    _handleSortTableHeaderColumn: function _handleSortTableHeaderColumn(columnIdx, ascending, event) {
      // clear the sorted indicator on any other column
      this._clearSortedHeaderColumn(columnIdx); // get the column metadata


      var column = this._getColumnDefs()[columnIdx]; // get which field to sort on


      var sortField = column.sortProperty == null ? column.field : column.sortProperty; // invoke sort on the data
     ...
     * @private
     */
    _hasEditableRow: function _hasEditableRow() {
      if (!this._isTableEditMode()) {
        return false;
      }
     ...
     * @private
     */
    _hasRowOrCellRendererOrTemplate: function _hasRowOrCellRendererOrTemplate(columnIdx) {
     ...
     * @private
     */
    _hideNoDataMessage: function _hideNoDataMessage() {
     ...
     * @private
     */
    _hideStatusMessage: function _hideStatusMessage() {
      this._clearShowStatusTimeout();
     ...
     * @private
     */
    _hideTableHeaderColumnSortIcon: function _hideTableHeaderColumnSortIcon(columnIdx) {
      ...
      if (this._getColumnDefs()[columnIdx].sortable === this._OPTION_ENABLED) {
        ...
        var tableHeaderColumn = domUtils.getTableHeaderColumn(columnIdx); // check if the column is currently sorted
     ...
     * @private
     */
    _initFetch: function _initFetch(options, force) {
      // eslint-disable-next-line no-param-reassign
      options = options || {};
      var self = this;

      var dataprovider = this._getData(); // do an initial fetch if a TableDataSource
      // paging control should do the fetches for PagingTableDataSource


      if (dataprovider != null && oj.DataProviderFeatureChecker.isDataProvider(dataprovider) && (!this._isPagingModelDataProvider() || force)) {
        return this._queueTask(function () {
     ...
     * @private
     */
    _initTemplateEngine: function _initTemplateEngine() {
      // initialize the template engine
      var self = this;
     ...
     * @private
     */
    _initializeFetchFirstOptions: function _initializeFetchFirstOptions(options) {
     ...
     * @private
     */
    _invokeDataFetchRows: function _invokeDataFetchRows(options) {
      // eslint-disable-next-line no-param-reassign
      ...
      if (dataprovider != null) {
        ...
        return new Promise(function (resolve) {
          ...
          self._dataProviderAsyncIterator.next().then(function (result) {
            ...
            var metadataArray = value[self._CONST_METADATA]; // Need to clear DOM scroller before refreshAll potentially triggers
     ...
     * @private
     */
    _invokeDataSort: function _invokeDataSort(sortField, ascending, event) {
      var dataprovider = this._getData(); // if no data then bail
     ...
     * @private
     */
    _isColumnMetadataUpdated: function _isColumnMetadataUpdated() {
        ...
        for (var i = 0; i < columnsMetadataCount; i++) {
          ...
          for (var j = 0; j < props.length; j++) {
            ...
            if (columnsMetadata[i][prop] !== this._columnDefArray[i][prop]) {
              if (prop !== 'id' || columnsMetadata[i][prop] == null || columnsMetadata[i][prop].indexOf(this._COLUMN_HEADER_ID_PREFIX) !== 0 || this._columnDefArray[i][prop] == null || this._columnDefArray[i][prop].indexOf(this._COLUMN_HEADER_ID_PREFIX) !== 0) {
     ...
     * @private
     */
    _isKeyboardKeyPressed: function _isKeyboardKeyPressed(keyCode) {
      var keyboardKeys = this._getKeyboardKeys();
     ...
     * @private
     */
    _isLoadMoreOnScroll: function _isLoadMoreOnScroll() {
     ...
     * @private
     */
    _isNodeEditable: function _isNodeEditable(node) {
     ...
     * @private
     */
    _isNodeClickable: function _isNodeClickable(node) {
     ...
     * @private
     */
    _isNodeDraggable: function _isNodeDraggable(node) {
     ...
     * @private
     */
    _isNodeType: function _isNodeType(node, type) {
      var table = this._getTableDomUtils().getTable();

      while (node != null && node !== table) {
        var nodeName = node.nodeName;

        if (nodeName === oj.TableDomUtils.DOM_ELEMENT._TD || nodeName === oj.TableDomUtils.DOM_ELEMENT._TH) {
          break;
        ...
        if (node.nodeType !== 3) {
          ...
          if (nodeName.match(type)) {
            var tabIndex = node.getAttribute(oj.TableDomUtils.DOM_ATTR._TABINDEX); // ignore elements with tabIndex == -1
     ...
     * @private
     */
    _isPagingModelDataProvider: function _isPagingModelDataProvider() {
      var dataprovider = this._getData();

      if (dataprovider.getStartItemIndex != null && dataprovider.getStartItemIndex() !== null && dataprovider.getStartItemIndex() >= 0) {
        return true;
      }
     ...
     * @private
     */
    _isStatusMessageShown: function _isStatusMessageShown() {
     ...
     * @private
     */
    _isTableActionableMode: function _isTableActionableMode() {
     ...
     * @private
     */
    _isTableEditMode: function _isTableEditMode() {
     ...
     * @private
     */
    _isTableFooterless: function _isTableFooterless() {
      var columns = this._getColumnDefs();
     ...
     * @private
     */
    _isTableHeaderless: function _isTableHeaderless() {
      var columns = this._getColumnDefs();
      ...
      for (var i = 0; i < columnsCount; i++) {
        ...
        if (columns[i].headerText != null || columns[i].headerStyle != null || columns[i].sortable != null && columns[i].sortable !== this._OPTION_DISABLED || columns[i].sortProperty != null || columns[i].headerRenderer != null || headerSlotTemplate != null) {
          return false;
        }
     ...
     * @private
     */
    _isTableHeaderColumnsRendered: function _isTableHeaderColumnsRendered() {
     ...
     * @private
     */
    _isTableRefreshNeeded: function _isTableRefreshNeeded(key, value) {
     ...
     * @private
     */
    _isTableSortable: function _isTableSortable() {
      var columns = this._getColumnDefs();
     ...
     * @private
     */
    _isTableColumnsResizable: function _isTableColumnsResizable() {
      var columns = this._getColumnDefs();
     ...
     * @private
     */
    _processFetchSort: function _processFetchSort(result) {
      try {
        var self = this;
     ...
     * @private
     */
    _refreshSortTableHeaderColumn: function _refreshSortTableHeaderColumn(key, ascending) {
      var columns = this._getColumnDefs();
      ...
      this._clearSortedHeaderColumn(columnIdx); // get the column header DOM element
      ...
      if (ascending && sorted !== this._COLUMN_SORT_ORDER._ASCENDING) {
        accessibilityText = this.getTranslatedString('accessibleSortAscending', {
          id: key
        });
        ...
        accessibilityText = this.getTranslatedString('accessibleSortDescending', {
          id: key
        });
        domUtils.setTableStatusAccNotification(accessibilityText);
     ...
     * @private
     */
    _renderRow: function _renderRow(rowData, tableBodyDocFrag, startIndex) {
      var row = rowData.row;
     ...
     * @private
     */
    _delayRefreshTableDimensions: function _delayRefreshTableDimensions() {
      ...
      this._refreshTableDimensionsTimer = setTimeout(function () {
        ...
        if (beforeScrollTop !== currentScrollTop) {
          ...
          self._queueTask(function () {
            self._refreshTableDimensionsTimer = null; // final task should be fully executed
     ...
     * @private
     */
    _afterRowsRendered: function _afterRowsRendered(tableBody, delayRefresh) {
      this._getTableDomUtils().clearCachedDomRowData(); // call a forced table size refresh here since we have rendered a bunch of rows
      ...
      this._clearIdleRenderBusyState(); // only bother calling subtree attached if there are potentially
     ...
     * @private
     */
    _clearIdleCallback: function _clearIdleCallback() {
     ...
     * @private
     */
    _requestIdleCallback: function _requestIdleCallback(isMouseWheel, callback) {
     ...
     * @private
     */
    _renderRowsWhenIdle: function _renderRowsWhenIdle(rows, tableBody, startIndex, resolve, reject, isMouseWheel) {
      if (rows.length === 0) {
        this.m_idleCallback = null;

        this._afterRowsRendered(tableBody, true).then(function (value) {
          resolve(value);
        }, function (reason) {
          reject(reason);
        });
      ...
      function afterRowsProcessed() {
        window.requestAnimationFrame(function () {
          ...
          if (isMouseWheel && oj.AgentUtils.getAgentInfo().browser === oj.AgentUtils.BROWSER.FIREFOX && tableBody.scrollTop !== self._scrollTop) {
            // eslint-disable-next-line no-param-reassign
     ...
     * @private
     */
    _refreshTableBody: function _refreshTableBody(resultObject, startIndex) {
      var tableBody = this._getTableDomUtils().getTableBody();
      ...
      var tableBodyRow;
      var resetFocus = false;
      var i; // Reset _scrollLeft and scrollTop to 0 if undefined to prevent later issues.
      ...
      if (rows.length === 0 && tableBodyRows.length === 0) {
        ...
        if (startIndex > 0 && this._isOverflow() && !this._fetchBySyncScroll) {
          ...
          return new Promise(function (resolve, reject) {
            self._setIdleRenderBusyState(); // clone the rows array since we'll manipulate it


            self._renderRowsWhenIdle(rows.slice(0), tableBody, startIndex, resolve, reject, resultObject.isMouseWheel);
          });
        }
     ...
     * @private
     */
    _refreshTableBodyRow: function _refreshTableBodyRow(rowIdx, row, tableBodyRow, docFrag, docFragStartIdx, isNew) {
      var domUtils = this._getTableDomUtils();
      ...
      if (tableBodyRow == null) {
        ...
        if (this._hasCellTemplate) {
          this._cleanTemplateNodes(tableBodyRow); // No need to set this._hasCellTemplate to false here since only one row is refreshed

        }

        $(tableBodyRow).empty(); // eslint-disable-next-line no-param-reassign

        isNew = true;
        domUtils.createTableBodyCellAccSelect(rowIdx, row[this._CONST_KEY], rowHashCode, tableBodyRow, isNew);
      }
      ...
      if (rowRenderer != null) {
        ...
        domUtils.createTableBodyCellAccSelect(rowIdx, row[this._CONST_KEY], rowHashCode, tableBodyRow, false); // set the cell attributes and styling. Skip the 1st one
        // because it's the acc row select td

        var tableBodyCells = domUtils.getTableElementsByTagName(tableBodyRow, oj.TableDomUtils.DOM_ELEMENT._TD);
        var i;
        ...
        if (this._columnsDestMap != null) {
          for (i = 0; i < this._columnsDestMap.length - 1; i++) {
            if (this._columnsDestMap[i] > i) {
              ...
              tableBodyCells = domUtils.getTableElementsByTagName(tableBodyRow, oj.TableDomUtils.DOM_ELEMENT._TD);
            }
          }
        }
      } else {
     ...
     * @private
     */
    _refreshTableFooter: function _refreshTableFooter() {
      var columns = this._getColumnDefs();
      ...
      if (columns && columns.length > 0) {
        ...
        for (var i = 0; i < columnsCount; i++) {
          ...
          var footerCell = this._getTableDomUtils().createTableFooterCell(i);
          ...
          if (footerRenderer) {
            ...
            var footerContext = oj.TableRendererUtils.getRendererContextObject(this, footerCell, {}); // Copy additional properties to top-level context to work with custom element
     ...
     * @private
     */
    _refreshTableHeader: function _refreshTableHeader() {
      var self = this;
      ...
      if (columns && columns.length > 0) {
        ...
        for (var i = 0; i < columnsCount; i++) {
          ...
          var headerColumn = this._getTableDomUtils().createTableHeaderColumn(i);

          if (headerRenderer) {
            // if headerRenderer is defined then call that
            var headerContext = oj.TableRendererUtils.getRendererContextObject(this, headerColumn, {}); // Copy additional properties to top-level context to work with custom element
            ...
            if (headerColumnContent != null) {
              ...
              $(headerColumn).empty(); // Use jquery append() for this as a convenience because
              ...
              this._getTableDomUtils().styleTableHeaderColumn(i, headerColumn, false);
            }
          } else {
     ...
     * @private
     */
    _refreshTableStatusMessage: function _refreshTableStatusMessage() {
     ...
     * @private
     */
    _registerCustomEvents: function _registerCustomEvents() {
      var jqEvent =
      /** @type {{special: Object}} */
      $.event;
      var jqEventSpecial = jqEvent.special; // ojtablebeforecurrentrow handlers will be passed an object which contains the
     ...
     * @private
     */
    _registerDataSourceEventListeners: function _registerDataSourceEventListeners() {
      ...
      if (dataprovider != null) {
        ...
        for (var i = 0; i < dataProviderEventHandlersCount; i++) {
          var ev = dataprovider.addEventListener(this._dataProviderEventHandlers[i].eventType, this._dataProviderEventHandlers[i].eventHandler);
     ...
     * @private
     */
    _registerDomEventListeners: function _registerDomEventListeners() {
      ...
      if (scroller != null) {
        ...
        if (this._scrollEventListener == null) {
          this._scrollEventListener = function (event) {
            ...
            this._handleScrollerScrollTop(event.target.scrollTop); // do not update scrollPosition if we are already adjusting
     ...
     * @private
     */
    _registerDomScroller: function _registerDomScroller() {
      var i;
      var self = this; // clear any existing DomScroller references
      ...
      this._domScrollerSuccessFunc = function (result) {
        ...
        if (result != null) {
          ...
          if (result.maxCountLimit) {
            ...
            var metadataArray = value[self._CONST_METADATA]; // remove any duplicate rows that are already in the DOM
      ...
      this._domScroller = new oj.DomScroller(this._getTableDomUtils().getScroller(), this._getData(), {
        ...
        beforeFetch: function beforeFetch(scrollTop) {
     ...
     * @private
     */
    _unregisterDomScroller: function _unregisterDomScroller() {
     ...
     * @private
     */
    _registerResizeListener: function _registerResizeListener() {
      ...
      if (!this._resizeListener) {
        this._resizeListener = function () {
          ...
          var tableContainer = domUtils.getTableContainer(); // Check if the width and height values should result in a sizing refresh

          if (domUtils.isSizingRefreshRequired(tableContainer.offsetWidth, tableContainer.offsetHeight)) {
     ...
     * @private
     */
    _removeKeyboardKey: function _removeKeyboardKey(keyCode) {
      var keyboardKeys = this._getKeyboardKeys();
     ...
     * @private
     */
    _scrollColumnIntoViewport: function _scrollColumnIntoViewport(columnIdx) {
      var domUtils = this._getTableDomUtils();
     ...
     * @private
     */
    _scrollRowIntoViewport: function _scrollRowIntoViewport(rowIdx) {
      var domUtils = this._getTableDomUtils();
      ...
      var hasBottomOverflow = bottomOverflowDiff >= 0; // don't adjust scroll position if row has overflow in both directions
     ...
     * @private
     */
    _setCurrentRow: function _setCurrentRow(currentRow, event, optionChange) {
      ...
      if (currentRow == null) {
        if (existingCurrentRow != null) {
          try {
            ...
            errDetail = Translations.applyParameters(this._LOGGER_MSG._ERR_PRECURRENTROW_ERROR_DETAIL, {
              error: err.toString()
            });
            Logger.info(errSummary + '\n' + errDetail); // do not update the currentRow to the new value if an exception was caught
      ...
      var rowIdx = this._getRowIdxForRowKey(rowKey); // eslint-disable-next-line no-param-reassign
      ...
      if (rowIdx !== -1 && (!dataprovider || rowIdx < -1 || rowIdx === null || rowKey === null)) {
        errSummary = this._LOGGER_MSG._ERR_CURRENTROW_UNAVAILABLE_INDEX_SUMMARY;
        errDetail = Translations.applyParameters(this._LOGGER_MSG._ERR_CURRENTROW_UNAVAILABLE_INDEX_DETAIL, {
          rowIdx: rowIdx
        });
     ...
     * @private
     */
    _setEditRow: function _setEditRow(editRow) {
      if (!this._isTableEditMode()) {
        return;
      }
      ...
      if (rowKey != null || rowIndex > -1) {
        ...
        var changed = this._setCurrentRow({
          rowKey: rowKey,
          rowIndex: rowIndex
        }, null, false);
        ...
        if (changed === this._CURRENT_ROW_STATUS._UPDATED) {
          ...
          if (!this._syncScrollPosition({
            rowKey: rowKey,
            rowIndex: rowIndex
          })) {
            var self = this; // this will be invoked after scroll and fetch is done
     ...
     * @private
     */
    _setDataWaitingState: function _setDataWaitingState(showMessage) {
     ...
     * @private
     */
    _setCellFocus: function _setCellFocus(rowIdx, columnIdx) {
      var domUtils = this._getTableDomUtils();
     ...
     * @private
     */
    _setCellInRowFocus: function _setCellInRowFocus(rowIdx, columnIdx, forwardSearch) {
     ...
     * @private
     */
    _setComponentNotReady: function _setComponentNotReady() {
     ...
     * @private
     */
    _setComponentReady: function _setComponentReady() {
      if (this._readyResolveFunc) {
     ...
     * @private
     */
    _setEditableRowIdx: function _setEditableRowIdx(rowIdx) {
     ...
     * @private
     */
    _setFocusInHandler: function _setFocusInHandler(focusHandler) {
     ...
     * @private
     */
    _setFocusOutHandler: function _setFocusOutHandler(focusHandler) {
     ...
     * @private
     */
    _setHeaderColumnFocus: function _setHeaderColumnFocus(columnIdx, focused, skipScrollColumn) {
      if (columnIdx != null) {
        if (focused) {
          // clear focused row
        ...
        this._setHeaderColumnState(columnIdx, {
          focused: focused
        });
      }
    },
     ...
     * @private
     */
    _getColumnSelectionFromKeySet: function _getColumnSelectionFromKeySet(keySet) {
      var selection = [];
     ...
     * @private
     */
    _applyColumnSelection: function _applyColumnSelection(columnIdx, selected) {
      this._setHeaderColumnState(columnIdx, {
        selected: selected
      ...
      if (accSelectionColumn) {
        var accSelectCheckbox = domUtils.getTableElementsByClassName(accSelectionColumn, oj.TableDomUtils.CSS_CLASSES._CHECKBOX_ACC_SELECT_COLUMN_CLASS)[0];
     ...
     * @private
     */
    _setHeaderColumnState: function _setHeaderColumnState(columnIdx, state) {
      var headerColumn = this._getTableDomUtils().getTableHeaderColumn(columnIdx);
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _applyHeaderColumnState: function _applyHeaderColumnState(headerColumn, columnIdx, state) {
     ...
     * @private
     */
    _setLastHeaderColumnSelection: function _setLastHeaderColumnSelection(columnIdx, selected) {
     ...
     * @private
     */
    _setLastRowSelection: function _setLastRowSelection(rowIdx, selected) {
     ...
     * @private
     */
    _setPreviousRowEditable: function _setPreviousRowEditable(columnIdx, event) {
      var editableRowIdx = this._getEditableRowIdx();
     ...
     * @private
     */
    _setRowFocus: function _setRowFocus(rowIdx, focused, updateCurrentRow, element, event) {
      var focusedRowIdx;
      var updateRowFocus;
      ...
      if (focused) {
        if (updateCurrentRow) {
          ...
          updateRowFocus = this._setCurrentRow({
            rowKey: rowKey
          }, event, false);
        ...
        this._scrollRowIntoViewport(rowIdx); // apply focused style classes
        ...
        this._clearFocusedHeaderColumn(); // unset table actionable mode


        this._setTableActionableMode(false);
      } else {
        this._focusOutHandler($(tableBodyRow)); // update focus style for the cells
     ...
     * @private
     */
    _getRowSelectionFromKeySet: function _getRowSelectionFromKeySet(keySet) {
      var selection = [];
      var iterator = keySet.isAddAll() ? keySet.deletedValues() : keySet.values();
      iterator.forEach(function (key) {
        var range = {
     ...
     * @private
     */
    _applyRowSelection: function _applyRowSelection(rowIdx, tableBodyRow, selected) {
      ...
      if (accSelectionCell) {
        var accSelectCheckbox = domUtils.getTableElementsByClassName(accSelectionCell, oj.TableDomUtils.CSS_CLASSES._CHECKBOX_ACC_SELECT_ROW_CLASS)[0];
        accSelectCheckbox.checked = selected;
      }
    },
     ...
     * @private
     */
    _processRowRangeSelection: function _processRowRangeSelection(rangeObj, rowKeySet, columnKeySet) {
      var startRowIndex;
      var endRowIndex;
      var startRowIdx;
      var endRowIdx;
      var startRowKey;
      var endRowKey;
      var status = true; // if keys are specified, we get the index from the key
      ...
      if (startRowIndex === undefined && endRowIndex === undefined) {
        ...
        if (startRowIndex != null && endRowIndex != null && startRowIndex <= endRowIndex) {
          ...
          for (j = startRowIdx; j <= endRowIdx; j++) {
            // update keySet
            var rowKey = this._getRowKeyForRowIdx(j); // eslint-disable-next-line no-param-reassign
          ...
          Logger.warn('Error: Cannot resolve row range in selection - \n start row key: ' + startRowKey + '\n end row key: ' + endRowKey + '\n start row index: ' + startRowIndex + '\n end row index: ' + endRowIndex); // although row keys are possibly out of range, we still need to update our keyset

          if (startRowKey != null && endRowKey != null) {
            // eslint-disable-next-line no-param-reassign
            rowKeySet = rowKeySet.add([startRowKey]); // eslint-disable-next-line no-param-reassign

            rowKeySet = rowKeySet.add([endRowKey]); // eslint-disable-next-line no-param-reassign
     ...
     * @private
     */
    _processColumnRangeSelection: function _processColumnRangeSelection(rangeObj, rowKeySet, columnKeySet) {
      var startColumnKey;
      var endColumnKey;
      ...
      if (startColumnIndex === undefined && endColumnIndex === undefined) {
        ...
        status = false;
      } else if (startColumnIndex != null && endColumnIndex != null && !isNaN(startColumnIndex) && !isNaN(endColumnIndex) && startColumnIndex <= endColumnIndex) {
        // this is a column based selection
        for (var j = startColumnIndex; j <= endColumnIndex; j++) {
          // update keySet
          var columnKey = this._getColumnKeyForColumnIdx(j); // eslint-disable-next-line no-param-reassign
     ...
     * @private
     */
    _syncRangeSelection: function _syncRangeSelection(selection) {
     ...
     * @private
     */
    _getSelectedEquivalent: function _getSelectedEquivalent(selection) {
      var selected;
      ...
      if (selection != null) {
        ...
        for (var i = 0; i < selectionCount; i++) {
          ...
          if (rangeObj.startKey == null && rangeObj[this._CONST_STARTINDEX] == null || rangeObj.endKey == null && rangeObj[this._CONST_ENDINDEX] == null) {
            Logger.error('Error: Invalid range object in selection. Both start and end objects must be specified'); // eslint-disable-next-line no-continue
          ...
          columnKeySet = retObj.columnKeySet; // it's not a row range, proceed to process column ranges
     ...
     * @private
     */
    _applySelected: function _applySelected(selected) {
      var domUtils = this._getTableDomUtils();

      var rowKeySet = selected.row;
      var columnKeySet = selected.column; // remove selection state from previously selected rows that are no longer selected
     ...
     * @private
     */
    _setSelected: function _setSelected(selected, skipSelectionUpdate) {
      ...
      if (selected != null) {
        ...
        if (this._selectionSet) {
          ...
          if (!DataCollectionUtils.areKeySetsEqual(currentSelected.row, selected.row) || !DataCollectionUtils.areKeySetsEqual(currentSelected.column, selected.column)) {
            updateSelected = true;
          }
        ...
        if (updateSelected) {
          ...
          if (this._isSelectionRequired() && this._validatedSelectedRowKeyData) {
            for (var i = this._validatedSelectedRowKeyData.length - 1; i >= 0; i--) {
              var rowKeyData = this._validatedSelectedRowKeyData[i]; // if the key is no longer in the selected value, remove it from the local storage
        ...
        this._applySelected(selected); // update firstSelectedRow option
        ...
        this._setFirstSelectedRow(newKey); // sync selection option with new selected value
     ...
     * @private
     */
    _setFirstSelectedRow: function _setFirstSelectedRow(newKey) {
    ...
    _syncSelectionState: function _syncSelectionState() {
      var selected = this.option('selected');
      var selection = this.option('selection'); // generate new selected value from existing selection value if required

      if (this._selectionSet) {
        // recreate 'selected' option in case selection ranges need to be applied
        selected = this._getSelectedEquivalent(selection); // update row index/key values in the 'selection' option
     ...
     * @private
     */
    _validateInitialSelectionState: function _validateInitialSelectionState(isOptionUpdate) {
      var useSelection;
      ...
      var rowKeySet = selected.row;
      var columnKeySet = selected.column;
      this._validatedSelectedRowKeyData = null; // generate new selected value from existing selection value if required

      if (this._selectionSet || !isOptionUpdate && selection != null && selection.length !== 0 && rowKeySet === rowKeySet.clear() && columnKeySet === columnKeySet.clear()) {
        // recreate 'selected' option in case selection ranges need to be applied
      ...
      if (this._isSelectionRequired()) {
        ...
        if (!validateSelectedResult.isLocal) {
          ...
          validateSelectedResult.result.then(function (validSelected) {
            this._enforceSelectionRequired(selected, validSelected, useSelection); // clear selection-required rowKey validation busy state
     ...
     * @private
     */
    _enforceSelectionRequired: function _enforceSelectionRequired(selected, validSelected, useSelection) {
      if (useSelection) {
     ...
     * @private
     */
    _handleSelectionGesture: function _handleSelectionGesture(index, isRow, isMultiSelectGesture) {
      var selected = this.option('selected');
      var rowKeySet = selected.row;
      var columnKeySet = selected.column;
      ...
      if (isRow) {
        ...
        if (this._isRowSelectionEnabled()) {
          ...
          if (!rowKeySet.has(rowKey)) {
            ...
            if (isMultiSelectGesture && this._getRowSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE) {
          ...
          } else if (!isMultiSelectGesture && this._getSelectedRowIdxs().length > 1 && this._getRowSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE) {
            // selection is now the new row key
            rowKeySet = new oj.KeySetImpl([rowKey]);
          } else {
        ...
        if (this._isColumnSelectionEnabled()) {
          ...
          if (!columnKeySet.has(columnKey)) {
            // handle multiple selection gesture
            if (isMultiSelectGesture && this._getColumnSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE) {
          ...
          } else if (!isMultiSelectGesture && this._getSelectedHeaderColumnIdxs().length > 1 && this._getColumnSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE) {
            // selection is now the new column key
            columnKeySet = new oj.KeySetImpl([columnKey]);
          } else {
     ...
     * @private
     */
    _hasSelected: function _hasSelected(selected) {
      if (selected == null) {
        // eslint-disable-next-line no-param-reassign
      ...
      var rowKeySet = selected.row;
      var columnKeySet = selected.column;
      return rowKeySet.isAddAll() || rowKeySet.values().size > 0 || columnKeySet.isAddAll() || columnKeySet.values().size > 0;
    },
     ...
     * @private
     */
    _isSelectionRequiredSatisfied: function _isSelectionRequiredSatisfied(selected) {
      if (this._isSelectionRequired()) {
     ...
     * @private
     */
    _validateSelected: function _validateSelected(selected) {
      var columnKeySet = selected.column;
          ...
          if (validRowKeyDataPromise) {
            var validateSelectedPromise = new Promise(function (resolve) {
              validRowKeyDataPromise.then(function (validRowKeyData) {
                ...
                resolve({
                  row: validatedRowKeySet,
                  column: validatedColumnKeySet
                });
              }.bind(this));
            }.bind(this));
            return {
              isLocal: false,
              result: validateSelectedPromise
            };
          }
     ...
     * @private
     */
    _selectFirstRowOrColumn: function _selectFirstRowOrColumn() {
      var newKeySet;
     ...
     * @private
     */
    _getSelectionEquivalent: function _getSelectionEquivalent(selected) {
      var selection = this._getRowSelectionFromKeySet(selected.row); // if there is a row selection, return that selection as it takes precedence over column selection
      ...
      selection = this._getColumnSelectionFromKeySet(selected.column); // otherwise, if there is a column selection, return that selection
     ...
     * @private
     */
    _toggleTableActionableMode: function _toggleTableActionableMode() {
      if (this._isTableActionableMode()) {
        this._setTableActionableMode(false);
      } else {
        this._setTableActionableMode(true);
     ...
     * @private
     */
    _setTableActionableMode: function _setTableActionableMode(value, skipFocusReset) {
      // don't do anything if actionable mode was not updated
      if (this._tableActionableMode != null && this._tableActionableMode === value || this._tableActionableMode == null && value === false) {
        return;
      }
     ...
     * @private
     */
    _fireEditRowChangeEvent: function _fireEditRowChangeEvent(newValue, event) {
      var self = this; // use _queueTask as _isSettingProperty is preventing writeback
     ...
     * @private
     */
    _setTableEditable: function _setTableEditable(editable, cancelled, columnIdx, forwardSearch, event, skipFocusReset) {
      if (!this._isTableEditMode()) {
        return undefined;
      }
      ...
      if (currentRow != null) {
        ...
        if (editable) {
          // set the editable row index
          this._setEditableRowIdx(rowIdx); // re-render the newly editable row
     ...
     * @private
     */
    _showNoDataMessage: function _showNoDataMessage() {
      if (!this._noDataMessageShown) {
     ...
     * @private
     */
    _showStatusMessage: function _showStatusMessage() {
      if (this._showStatusTimeout) {
        return;
      }
     ...
     * @private
     */
    _showTableHeaderColumnSortIcon: function _showTableHeaderColumnSortIcon(columnIdx) {
      if (this._getColumnDefs()[columnIdx].sortable === this._OPTION_ENABLED) {
        var domUtils = this._getTableDomUtils();
        ...
        var sorted = $(tableHeaderColumn).data('sorted'); // we should only show the ascending sort icon if the column is not sorted

        if (sorted == null) {
          var headerColumnAscIcon = domUtils.getTableElementsByClassName(tableHeaderColumn, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_ASC_ICON_CLASS);
          var sortContainer = domUtils.getSortIconContainer(tableHeaderColumn);
     ...
     * @private
     */
    _touchEventDoubleTapFunction: function _touchEventDoubleTapFunction(event, eventHandler) {
      var eventTarget = $(event.target);

      if (this._lastTapTime != null && new Date().getTime() - this._lastTapTime < 250 && this._lastTapTarget[0] === eventTarget[0]) {
     ...
     * @private
     */
    _unregisterChildStateListeners: function _unregisterChildStateListeners(parent) {
      var self = this;
     ...
     * @private
     */
    _unregisterDataSourceEventListeners: function _unregisterDataSourceEventListeners() {
      var dataprovider = this._getData(); // unregister the listeners on the datasource
     ...
     * @private
     */
    _unregisterResizeListener: function _unregisterResizeListener() {
      var element = this._getTableDomUtils().getTableContainer();
     ...
     * @private
     */
    _updateHeaderColumnStateCellsClass: function _updateHeaderColumnStateCellsClass(columnIdx) {
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _updateRowStateCellsClass: function _updateRowStateCellsClass(rowIdx, state, blur) {
      var tableBodyCells = this._getTableDomUtils().getTableBodyCells(rowIdx);
    ...
    _hasPendingTasks: function _hasPendingTasks() {
      return this._taskCount > 1;
    },
    _queueTask: function _queueTask(task) {
      var self = this;
    ...
    _CompareOptionValues: function _CompareOptionValues(option, value1, value2) {
      switch (option) {
        ...
        case 'selected':
          return DataCollectionUtils.areKeySetsEqual(value1.row, value2.row) && DataCollectionUtils.areKeySetsEqual(value1.column, value2.column);
    ...
    _registerTouchEvents: function _registerTouchEvents() {
      var regularTouchEventsAndListeners = {
        /*
         ...
         */
        'touchend .oj-table-body': function touchendOjTableBody(evt) {
          var self = this;
      ...
      var passiveTouchEventsAndListeners = {
        /*
         * Keep track of touchstart on selection affordance
         */
        'touchstart .oj-table-body-row-touch-selection-affordance-touch-area': function touchstartOjTableBodyRowTouchSelectionAffordanceTouchArea(event) {
          ...
          if (fingerCount === 1 && this._getRowSelectionMode() === this._OPTION_SELECTION_MODES._MULTIPLE) {
            if (event.target.classList.contains(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_CLASS) || event.target.classList.contains(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_ICON_CLASS)) {
              event.preventDefault();
              this._mouseDownRowIdx = $(this._getTableDomUtils().getTableBodyRowTouchSelectionAffordanceBottom()).data('rowIdx');
            } else if (event.target.classList.contains(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_CLASS) || event.target.classList.contains(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_ICON_CLASS)) {
              event.preventDefault();
         ...
         * column resizing
         */
        'touchstart .oj-table-column-header-cell': function touchstartOjTableColumnHeaderCell(event) {
          var fingerCount = event.originalEvent.touches.length;

          if (fingerCount === 1) {
            if (this._isTableColumnsResizable() && this._getTableResizeUtils().handleHeaderColumnResizeStart(event)) {
              event.preventDefault();
            }
          }
        },
         ...
         */
        'touchmove .oj-table-header': function touchmoveOjTableHeader(event) {
         ...
         */
        'touchmove .oj-table-body-row-touch-selection-affordance-touch-area': function touchmoveOjTableBodyRowTouchSelectionAffordanceTouchArea(event) {
          if (this._mouseDownRowIdx != null) {
            event.preventDefault();
      ...
      var tableContainer = this._getTableDomUtils().getTableContainer(); // register regular touch events


      this._on($(tableContainer), regularTouchEventsAndListeners); // register touchstart & touchmove with passive option for custom elements


      if (this._IsCustomElement()) {
        var createDelegatedListener = function createDelegatedListener(selector, listener) {
        ...
        var TABLE_BODY_SELECTOR = '.oj-table-body-row-touch-selection-affordance-touch-area';
        var TOUCHSTART_CELL_HEADER_SELECTOR = '.oj-table-column-header-cell';
        var TOUCHMOVE_TABLE_HEADER_SELECTOR = '.oj-table-header';
        this._delegatedTouchStartTableBodyListener = createDelegatedListener(TABLE_BODY_SELECTOR, passiveTouchEventsAndListeners["".concat(TOUCHSTART, " ").concat(TABLE_BODY_SELECTOR)].bind(this));
        this._delegatedTouchStartCellHeaderListener = createDelegatedListener(TOUCHSTART_CELL_HEADER_SELECTOR, passiveTouchEventsAndListeners["".concat(TOUCHSTART, " ").concat(TOUCHSTART_CELL_HEADER_SELECTOR)].bind(this));
        this._delegatedTouchMoveTableHeaderListener = createDelegatedListener(TOUCHMOVE_TABLE_HEADER_SELECTOR, passiveTouchEventsAndListeners["".concat(TOUCHMOVE, " ").concat(TOUCHMOVE_TABLE_HEADER_SELECTOR)].bind(this));
        this._delegatedTouchMoveTableBodyListener = createDelegatedListener(TABLE_BODY_SELECTOR, passiveTouchEventsAndListeners["".concat(TOUCHMOVE, " ").concat(TABLE_BODY_SELECTOR)].bind(this));
...
oj.TableDndContext.prototype._removeDragMarkerClass = function () {
  ...
  var dragColumns = domUtils.getTableElementsByClassName(domUtils.getTableHeader(), oj.TableDomUtils.MARKER_STYLE_CLASSES._DRAG);
  ...
  domUtils.setTableColumnCellsClass(null, false, oj.TableDomUtils.MARKER_STYLE_CLASSES._DRAG);
};
/**
...
oj.TableDndContext.prototype._cloneTableContainer = function (tableContainer) {
  ...
  var tableBody = domUtils.getTableElementsByTagName(tableContainer, oj.TableDomUtils.DOM_ELEMENT._TBODY)[0];
  var scrollLeft = $(tableBody).scrollLeft();
  tableContainerClone = tableContainer.cloneNode(true);
  var tableBodyClone = domUtils.getTableElementsByTagName(tableContainerClone, oj.TableDomUtils.DOM_ELEMENT._TBODY, true)[0]; // Use absolute positioning with a large negative top to put it off the screen without
  // affecting the scrollbar.  Fixed positioning does not work on Safari.

  tableContainerClone.style[oj.TableDomUtils.CSS_PROP._BACKGROUND_COLOR] = oj.TableDomUtils.CSS_VAL._TRANSPARENT;
  tableContainerClone.style[oj.TableDomUtils.CSS_PROP._BORDER_COLOR] = oj.TableDomUtils.CSS_VAL._TRANSPARENT;
  tableContainerClone.style[oj.TableDomUtils.CSS_PROP._POSITION] = oj.TableDomUtils.CSS_VAL._ABSOLUTE;
  tableContainerClone.style[oj.TableDomUtils.CSS_PROP._HEIGHT] = tableBody.scrollHeight + oj.TableDomUtils.CSS_VAL._PX;
  tableContainerClone.style[oj.TableDomUtils.CSS_PROP._TOP] = -10000 + oj.TableDomUtils.CSS_VAL._PX; // Set both overflow and overflow-x/y because on IE, setting overflow does not
  ...
  tableBodyClone.style[oj.TableDomUtils.CSS_PROP._BORDER_COLOR] = oj.TableDomUtils.CSS_VAL._TRANSPARENT;
  tableBodyClone.style[oj.TableDomUtils.CSS_PROP._WIDTH] = $(tableBody).width() + oj.TableDomUtils.CSS_VAL._PX;
  tableBodyClone.style[oj.TableDomUtils.CSS_PROP._HEIGHT] = tableBody.scrollHeight + oj.TableDomUtils.CSS_VAL._PX;
...
oj.TableDndContext.prototype._getOverRowIndex = function (event) {
  ...
  var overRow = domUtils.getFirstAncestor(event.target, oj.TableDomUtils.DOM_ELEMENT._TR, true); // Find the index from the DOM directly instead of calling TableDomUtils.getElementRowIdx.  There was a change in
  ...
  if (cell != null) {
    var tableBodyRow = domUtils.getFirstAncestor(cell, '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_CLASS, true);

    if (tableBodyRow != null) {
      newRowIndex = $(tableBodyRow).index();
    }
  } else if ($(overRow).hasClass(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_DRAG_INDICATOR_BEFORE_CLASS)) {
...
oj.TableDndContext.prototype.handleColumnDragStart = function (event) {
  if (this._isColumnReorderEnabled()) {
    ...
    this._setReorderColumnsDataTransfer(event, columnIdx); // Remove any row and column selection


    this.component._clearSelectionState(); // Remove text selection
...
oj.TableDndContext.prototype.handleColumnReorderDragOver = function (event) {
  ...
  if (dragStartColumnIdx != null && dragStartColumnIdx !== columnIdx) {
    this._currentDropColumnBefore = this._isDragOverBeforeColumn(event); // Check the current before/after column position against indicator position
    // to see if we need to move the indicator

    if (!(this._currentDropColumnBefore && dragStartColumnIdx === columnIdx - 1) && !(!this._currentDropColumnBefore && dragStartColumnIdx === columnIdx + 1)) {
...
oj.TableDndContext.prototype.handleColumnReorderDrop = function (event) {
  ...
  this.component._columnsDestMap = domUtils.moveTableHeaderColumn(this._dragStartColumnIdx, columnIdx, event);
  event.preventDefault();
};
/**
...
oj.TableDndContext.prototype.handleRowDrop = function (event) {
  var dropRowIndex = this._dropRowIndex; // Perform any cleanup
...
oj.TableDndContext.prototype._hideUnselectedRows = function (tableContainerClone, selArray) {
  ...
  var tableBodyRows = domUtils.getTableElementsByClassName(tableContainerClone, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_CLASS, true);
  ...
  for (i = tableBodyRows.length - 1; i >= 0; i--) {
    if (selArray.indexOf(i) === -1) {
      ...
      for (j = 0; j < cells.length; j++) {
        cells[j].style[oj.TableDomUtils.CSS_PROP._BORDER_COLOR] = oj.TableDomUtils.CSS_VAL._TRANSPARENT;
      }
    } else {
      firstRow = i;
    }
  ...
  var tableHeader = domUtils.getTableElementsByTagName(tableContainerClone, oj.TableDomUtils.DOM_ELEMENT._THEAD, true);
  ...
  if (tableHeader != null && tableHeader.length > 0) {
    ...
    tableHeader[0].style[oj.TableDomUtils.CSS_PROP._BORDER_COLOR] = oj.TableDomUtils.CSS_VAL._TRANSPARENT;
    ...
    if (headerRow != null) {
      ...
      for (i = 0; i < headerCells.length; i++) {
        headerCells[i].style[oj.TableDomUtils.CSS_PROP._BORDER_COLOR] = oj.TableDomUtils.CSS_VAL._TRANSPARENT;
      }
    }
  }

  var tableFooter = domUtils.getTableElementsByTagName(tableContainerClone, oj.TableDomUtils.DOM_ELEMENT._TFOOT, true);
    ...
    if (callback && typeof callback === 'function') {
      try {
        if (this.component._IsCustomElement()) {
          ...
          // eslint-disable-next-line no-param-reassign
          event.dataTransfer = event.originalEvent.dataTransfer; // Invoke callback function
...
oj.TableDndContext.prototype._isColumnReorderEnabled = function () {
  ...
  if (dndOption && dndOption.reorder && dndOption.reorder.columns === this.component._OPTION_ENABLED) {
    return true;
  }
...
oj.TableDndContext.prototype._isDndEventInElement = function (event, element) {
  ...
  var domEvent = event.originalEvent; // clientX and clientY are only available on the original DOM event.
  // IE returns client rect in non-integer values.  Trim it down to make sure
  // the pointer is really still in the element.

  return domEvent.clientX >= Math.ceil(rect.left) && domEvent.clientX < Math.floor(rect.right) && domEvent.clientY >= Math.ceil(rect.top) && domEvent.clientY < Math.floor(rect.bottom);
};
/**
...
oj.TableDndContext.prototype._isDragOverBeforeColumn = function (event) {
  ...
  if (event.originalEvent.clientX != null) {
    var cursorPosX = columnRect.right - event.originalEvent.clientX; // First figure out whether the pointer is on the left half or right half

    var onRightHalf = cursorPosX < (columnRect.right - columnRect.left) / 2; // Whether we are before/after the column depends on the reading direction
...
oj.TableDndContext.prototype._matchDragDataType = function (event, itemType) {
  ...
  if (dndOption && dndOption[itemType] && dndOption[itemType].dataTypes) {
    ...
    var allowedTypeArray = typeof allowedTypes === 'string' ? [allowedTypes] : allowedTypes; // dragDataTypes can be either an array of strings (Chrome) or a
...
oj.TableDndContext.prototype._clearHeaderColumnDraggable = function (headerColumn) {
  // eslint-disable-next-line no-param-reassign
  headerColumn.draggable = ''; // eslint-disable-next-line no-param-reassign
...
oj.TableDndContext.prototype._setDragRowsDataTransfer = function (event, dataTypes, selArray) {
  var rowDataArray = []; // Add data for selected rows to an array
  ...
  if (rowDataArray.length) {
    this._setDragRowsData(event.originalEvent, dataTypes, rowDataArray); // Call _destroyDragImage before creating a new one in case the last drag image was not removed.
    // This could happen sometimes in some versions of Chrome.


    this._destroyDragImage();

    this._dragImage = this._setDragRowsImage(event.originalEvent, $(this._getTableDomUtils().getFirstAncestor(event.currentTarget, oj.TableDomUtils.DOM_ELEMENT._TABLE, true)).parent()[0], selArray);
    return {
      rows: rowDataArray
    };
  }
...
oj.TableDndContext.prototype._setReorderColumnsDataTransfer = function (event, columnIdx) {
  ...
  } catch (e) {// MS Edge doesn't allow calling setDragImage()
  }
};
/**
...
oj.TableDomUtils.prototype.createContextMenu = function (handleContextMenuSelect) {
  ...
  var self = this;
  var enableNonContiguousSelectionMenu = this.component._isTouchDevice() ? this.component._getRowSelectionMode() === this.component._OPTION_SELECTION_MODES._MULTIPLE : false;
  ...
  if (menuContainer) {
  ...
  } else if (this.component._isTableSortable() || enableNonContiguousSelectionMenu || this.component._isTableColumnsResizable()) {
    ...
    this._menuContainer = menuContainer;
    $(menuContainer).on('ojselect', handleContextMenuSelect);
    this.component.option('contextMenu', '#' + menuContainer.getAttribute(oj.TableDomUtils.DOM_ATTR._ID), {
      _context: {
        internalSet: true
      }
...
oj.TableDomUtils.prototype.createContextMenuLabel = function (command, useOjOption) {
  ...
  var textNode = document.createTextNode(commandString); // for custom elements, no <a> tag is required
...
oj.TableDomUtils.prototype.createTableBodyCellAccSelect = function (rowIdx, rowKey, rowHashCode, tableBodyRow, isNew) {
  ...
  accSelectionCell.style.border = '0'; // ensure proper role is set for wai-aria
...
oj.TableDomUtils.prototype.getTouchAffordanceGlassPane = function () {
  var tableContainer = this.getTableContainer();
  var touchAffordanceGlassPane = this.getTableElementsByClassName(tableContainer, oj.TableDomUtils.CSS_CLASSES._TABLE_TOUCH_AFFORDANCE_GLASS_PANE_CLASS);
...
oj.TableDomUtils.prototype.createTouchAffordanceGlassPane = function () {
  ...
  if (!touchAffordanceGlassPane) {
    ...
    touchAffordanceGlassPane.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_TOUCH_AFFORDANCE_GLASS_PANE_CLASS);
...
oj.TableDomUtils.prototype.createTableBodyRowTouchSelectionAffordance = function (rowIdx) {
  ...
  if (!topAffordance) {
    ...
    topAffordance.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_CLASS);
    topAffordance.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOUCH_AREA_CLASS);
    var topIcon = document.createElement(oj.TableDomUtils.DOM_ELEMENT._DIV);
    topIcon.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_ICON_CLASS);
  ...
  if (!bottomAffordance) {
    ...
    bottomAffordance.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_CLASS);
    bottomAffordance.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOUCH_AREA_CLASS);
    var bottomIcon = document.createElement(oj.TableDomUtils.DOM_ELEMENT._DIV);
    bottomIcon.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_ICON_CLASS);
...
oj.TableDomUtils.prototype.createTableContainer = function () {
  ...
  this._cacheDomElement(oj.TableDomUtils.CSS_CLASSES._TABLE_CONTAINER_CLASS, tableContainer); // add main css class to container
...
oj.TableDomUtils.prototype.createTableFooterAccSelect = function (tableFooterRow) {
  var accFooterCell = this.getTableElementsByClassName(tableFooterRow, oj.TableDomUtils.CSS_CLASSES._HIDDEN_CONTENT_ACC_CLASS);
  ...
  accFooterCell.setAttribute(oj.TableDomUtils.DOM_ATTR._TABINDEX, '-1'); // ensure proper role is set for wai-aria
...
oj.TableDomUtils.prototype.createTableHeaderAccSelectRowColumn = function () {
  ...
  headerColumn.setAttribute(oj.TableDomUtils.DOM_ATTR._ID, this.getTableId() + ':' + oj.TableDomUtils._COLUMN_HEADER_ROW_SELECT_ID); // ensure proper role is set for wai-aria
...
oj.TableDomUtils.prototype.createTableHeaderColumn = function (columnIdx) {
  ...
  this.styleTableHeaderColumn(columnIdx, headerColumnCell, true); // add abbr for acc

  headerColumnCell.setAttribute('abbr', column.headerText); // add title for tooltip
...
oj.TableDomUtils.prototype.createTableHeaderColumnDragImage = function (columnIdx) {
  ...
  headerColumnDragImage.style[oj.TableDomUtils.CSS_PROP._ZINDEX] = '-999'; // The drag image element must be either visible or a child of document.body
...
oj.TableDomUtils.prototype.createTableHeaderColumnAccSelect = function (columnIdx, columnSelectionMode) {
  if (columnSelectionMode !== oj.TableDomUtils._OPTION_SELECTION_MODES._SINGLE && columnSelectionMode !== oj.TableDomUtils._OPTION_SELECTION_MODES._MULTIPLE) {
    return null;
  }
...
oj.TableDomUtils.prototype.getElementColumnIdx = function (element) {
  var tableBodyCell = this.getFirstAncestor(element, '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_CLASS, true);

  if (tableBodyCell != null) {
    return $(tableBodyCell.parentNode).children('.' + oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_CLASS).index(tableBodyCell);
  }

  var tableHeaderColumn = this.getFirstAncestor(element, '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);
  ...
  var tableFooterCell = this.getFirstAncestor(element, '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS, true);
...
oj.TableDomUtils.prototype.getElementRowIdx = function (element) {
  var tableBodyRow = this.getFirstAncestor(element, '.' + oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_CLASS, true);
...
oj.TableDomUtils.prototype.getFirstAncestor = function (element, selector, tableOwnedOnly) {
  ...
  var elementMatches = this._isIE() && !Element.prototype.matches ? element.msMatchesSelector(selector) : element.matches(selector);
...
oj.TableDomUtils.prototype.getScrollBarWidth = function () {
  return this._tableHeightConstrained && !this._isVerticalScrollBarHidden() ? this.getDefaultScrollBarSize() : 0;
};
/**
...
oj.TableDomUtils.prototype._isVerticalScrollBarHidden = function () {
  return !this.component._isTouchDevice() && this.getTableContainer().classList.contains(oj.TableDomUtils.MARKER_STYLE_CLASSES._HIDE_VERTICAL_SCROLLBAR);
};
/**
...
oj.TableDomUtils.prototype.getTableBodyCellAccSelect = function (tableBodyRow) {
  if (tableBodyRow != null) {
    var accSelectionCell = this.getTableElementsByClassName(tableBodyRow, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_ACC_SELECT_CLASS);
...
oj.TableDomUtils.prototype.getTableBodyCells = function (rowIdx, tableBodyRow) {
  ...
  var tableBodyCellElements = this.getTableElementsByClassName(tableBodyRow, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_CLASS);
...
oj.TableDomUtils.prototype.getTableBodyMessageCell = function () {
  ...
  if (tableBody) {
    var messageCell = this.getTableElementsByClassName(tableBody, oj.TableDomUtils.CSS_CLASSES._TABLE_BODY_MESSAGE_CLASS);
...
oj.TableDomUtils.prototype.getTableBodyMessageRow = function () {
  ...
  if (tableBody) {
    var messageRow = this.getTableElementsByClassName(tableBody, oj.TableDomUtils.CSS_CLASSES._TABLE_BODY_MESSAGE_ROW_CLASS);
...
oj.TableDomUtils.prototype.getTableBodyRowTouchSelectionAffordanceTop = function () {
  var tableContainer = this.getTableContainer();
  var topAffordance = this.getTableElementsByClassName(tableContainer, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_CLASS);
...
oj.TableDomUtils.prototype.getTableBodyRowTouchSelectionAffordanceBottom = function () {
  var tableContainer = this.getTableContainer();
  var bottomAffordance = this.getTableElementsByClassName(tableContainer, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_CLASS);
...
oj.TableDomUtils.prototype.getTableFooterCells = function () {
  ...
  var tableFooterCells = tableFooterRow != null ? this.getTableElementsByClassName(tableFooterRow, oj.TableDomUtils.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS) : null;
...
oj.TableDomUtils.prototype.getTableHeaderColumnAccSelect = function (columnIdx) {
  ...
  if (headerColumn != null) {
    var accSelectionCell = this.getTableElementsByClassName(headerColumn, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_ACC_SELECT_COLUMN_CLASS);
...
oj.TableDomUtils.prototype.getTableHeaderColumns = function () {
  ...
  if (tableHeaderRow != null) {
    var headerColumnElements = this.getTableElementsByClassName(tableHeaderRow, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS);
...
oj.TableDomUtils.prototype.hashCode = function (str) {
  ...
  for (var i = 0; i < strCount; i++) {
    var charVal = str.charCodeAt(i); // eslint-disable-next-line no-bitwise

    hash = (hash << 5) - hash + charVal; // eslint-disable-next-line no-bitwise
...
oj.TableDomUtils.prototype.insertTableBodyCell = function (rowIdx, rowKey, rowHashCode, columnIdx, tableBodyCell, tableBodyRow, isNew) {
  ...
  if (columnIdx === 0) {
    ...
    var tableBodyCells = this.getTableElementsByClassName(tableBodyRow, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_CLASS);
...
oj.TableDomUtils.prototype.insertTableFooterCell = function (columnIdx, tableFooterCell) {
  var tableFooterRow = this.getTableFooterRow();
  var tableFooterCells = this.getTableElementsByClassName(tableFooterRow, oj.TableDomUtils.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS);
...
oj.TableDomUtils.prototype.insertTableHeaderColumn = function (columnIdx, tableHeaderColumn) {
  var tableHeaderRow = this.getTableHeaderRow();
  var tableHeaderColumns = this.getTableHeaderColumns(); // save the column index on the element

  this.setTableHeaderColumnAttributes(columnIdx, tableHeaderColumn); // if there is an existing th at the index then replace it
  ...
  if (oldTableHeaderColumn) {
    ...
    var accSelectionColumn = this.getTableElementsByClassName(tableHeaderRow, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_ACC_SELECT_ROW_CLASS);
...
oj.TableDomUtils.prototype.moveTableBodyRowTouchSelectionAffordanceTop = function (rowIdx) {
  ...
  if (topAffordance != null) {
    ...
    topAffordance.style[oj.TableDomUtils.CSS_PROP._TOP] = tableBodyRowRect.top - touchAffordanceGlassPaneRect.top - topAffordance.clientHeight / 2 + oj.TableDomUtils.CSS_VAL._PX;
...
oj.TableDomUtils.prototype.moveTableBodyRowTouchSelectionAffordanceBottom = function (rowIdx) {
  ...
  if (bottomAffordance != null) {
    ...
    var touchAffordanceGlassPane = this.getTouchAffordanceGlassPane();
    var touchAffordanceGlassPaneRect = touchAffordanceGlassPane.getBoundingClientRect();
    bottomAffordance.style[oj.TableDomUtils.CSS_PROP._TOP] = tableBodyRowRect.top - touchAffordanceGlassPaneRect.top + tableBodyRowRect.height - bottomAffordance.clientHeight / 2 + oj.TableDomUtils.CSS_VAL._PX;
...
oj.TableDomUtils.prototype.moveTableHeaderColumn = function (columnIdx, destIdx, event) {
  ...
  clonedColumnsOption.splice(destColIdx, 0, columnOption[0]); // re-order the column definition metadata
  ...
  this._columnsDestMap.splice(destColIdx, 0, columnsDestMapItem[0]); // clone the array so we can trigger that it's changed
...
oj.TableDomUtils.prototype.removeDragOverIndicatorColumn = function () {
  var table = this.getTable();
  var indicatorElements = this.tableQuerySelectorAll(table, '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_DRAG_INDICATOR_BEFORE_CLASS + ',' + '.' + oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_DRAG_INDICATOR_AFTER_CLASS);
  var i = 0;
...
oj.TableDomUtils.prototype.removeDragOverIndicatorRow = function () {
  var tableBody = this.getTableBody();
  var indicatorRowBeforeElements = this.getTableElementsByClassName(tableBody, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_DRAG_INDICATOR_BEFORE_CLASS);
  var indicatorRowAfterElements = this.getTableElementsByClassName(tableBody, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_DRAG_INDICATOR_AFTER_CLASS);
  var i;
...
oj.TableDomUtils.prototype.removeTableBodyRow = function (rowIdx) {
  ...
  if (tableBodyRow != null) {
    ...
    if (this.component._hasCellTemplate) {
      this.component._cleanTemplateNodes(tableBodyRow); // No need to set this.component._hasCellTemplate to false here since only one row is removed
  ...
  if (tableBodyRows.length > 0) {
    ...
    if (tableBody != null) {
      ...
      if (this.component._hasCellTemplate) {
        ...
        this.component._hasCellTemplate = false; // need to re-register DOM event listeners after cleaning the node
...
oj.TableDomUtils.prototype.removeTableBodyRowTouchSelectionAffordance = function () {
  var tableContainer = this.getTableContainer();
  var touchAffordance = this.getTableElementsByClassName(tableContainer, oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOUCH_AREA_CLASS);
  var i;
...
oj.TableDomUtils.prototype.removeTableHeaderColumnDragImage = function () {
  var tableContainer = this.getTableContainer();
  var headerDragImage = this.getTableElementsByClassName(tableContainer, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_DRAG_IMAGE);
...
oj.TableDomUtils.prototype.setScrollLeft = function (element, scrollLeft) {
  if (this.component._GetReadingDirection() === 'rtl') {
    ...
    if (this._isWebkit()) {
      var maxScrollLeft = element.scrollWidth - element.clientWidth; // eslint-disable-next-line no-param-reassign
  ...
  if (tableBodyRows.length > 0) {
    if (columnIdx === null) {
      ...
      if (!add) {
        ...
        tableBodyCells = this.getTableElementsByTagName(tableBody, oj.TableDomUtils.DOM_ELEMENT._TD);
      }
...
oj.TableDomUtils.prototype.styleInitialTable = function () {
  ...
  tableBody = tableBody.length > 0 ? tableBody[0] : null; // set the tabindex

  var tabIndex = parseInt(this.component._getRootElement().getAttribute(oj.TableDomUtils.DOM_ATTR._TABINDEX), 10);

  if (tabIndex !== -1) {
    table.setAttribute(oj.TableDomUtils.DOM_ATTR._TABINDEX, '0'); // set focusable
...
oj.TableDomUtils.prototype.styleTableBody = function (tableBody) {
  tableBody.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_BODY_CLASS); // Add a special marker attribute to tell child components that they are container within table

  tableBody.setAttribute(Components._OJ_CONTAINER_ATTR, this.component.widgetName); // ensure proper role is set for wai-aria
...
oj.TableDomUtils.prototype.styleTableBodyCell = function (columnIdx, tableBodyCell, isNew) {
  var column = this.component._getColumnDefs()[columnIdx]; // ensure proper role is set for wai-aria
  ...
  if (this._isVerticalGridEnabled() && (isNew || !tableBodyCell.classList.contains(oj.TableDomUtils.CSS_CLASSES._TABLE_VGRID_LINES_CLASS))) {
...
oj.TableDomUtils.prototype.styleTableHeader = function (tableHeader) {
  ...
  tableHeader.classList.add(oj.TableDomUtils.CSS_CLASSES._TABLE_HEADER_CLASS); // eslint-disable-next-line no-param-reassign
...
oj.TableDomUtils.prototype.styleTableHeaderColumn = function (columnIdx, tableHeaderColumn, isNew) {
  var column = this.component._getColumnDefs()[columnIdx]; // ensure proper role is set for wai-aria
...
oj.TableDomUtils.prototype._isHorizontalGridEnabled = function () {
  if (this.options.horizontalGridVisible === oj.TableDomUtils._OPTION_ENABLED || this.options.horizontalGridVisible === oj.TableDomUtils._OPTION_AUTO) {
    return true;
  }
...
oj.TableDomUtils.prototype._isVerticalGridEnabled = function () {
  if (this.options.verticalGridVisible === oj.TableDomUtils._OPTION_ENABLED || this.options.verticalGridVisible === oj.TableDomUtils._OPTION_AUTO && this.options.display === oj.TableDomUtils._OPTION_DISPLAY._GRID) {
    return true;
  }
...
oj.TableDomUtils.prototype.isSizingRefreshRequired = function (width, height) {
  if (width != null && height != null) {
    ...
    if (this._cacheDomDimensions != null) {
      return width > 0 && height > 0 && (width !== this._cacheDomDimensions.tableContainerWidth || height !== this._cacheDomDimensions.tableContainerHeight);
    }
...
oj.TableDomUtils.prototype.refreshTableDimensions = function (width, height, skipRequiredCheck) {
  ...
  var tableBody = this.getTableBody();
  var tableFooter = this.getTableFooter();
  var tableBottomSlot = this.getTableBottomSlot(); // preserve the scrollTop & scrollLeft

  var scrollTop = null;
  var scrollLeft = null;

  if (this.component._scrollTop != null && this.component._scrollTop > 0) {
    scrollTop = this.component._scrollTop;
  }

  if (this.component._scrollLeft != null && this.component._scrollLeft > 0) {
  ...
  this.styleTableContainer(tableContainer); // set the forced column widths before checking for scroll state, otherwise we
  ...
  if (this._tableHeightConstrained || this._tableWidthConstrained) {
    ...
    var containerClientHeight = tableContainer.clientHeight; // If there is overflow in only one direction, we need to check if the addition of
    ...
    if (browserScrollBarSize > 0) {
      if (!this._tableWidthConstrained && !this._isVerticalScrollBarHidden()) {
        ...
        table.style[oj.TableDomUtils.CSS_PROP._WIDTH] = containerClientWidth + browserScrollBarSize + oj.TableDomUtils.CSS_VAL._PX;
        ...
        table.style[oj.TableDomUtils.CSS_PROP._HEIGHT] = containerClientHeight + browserScrollBarSize + oj.TableDomUtils.CSS_VAL._PX;
    ...
    var actualScrollBarWidth = this.getScrollBarWidth(); // now that overflow is determined, set the column widths

    this._setColumnWidths(actualScrollBarWidth); // add in scrolling class for specific vertical or horizontal scrolling
    ...
    tableBody.style[oj.TableDomUtils.CSS_PROP._WIDTH] = containerClientWidth + oj.TableDomUtils.CSS_VAL._PX;
    ...
    var tableBodyHeight = containerClientHeight - tableHeaderHeight - tableFooterHeight - captionHeight - tableBottomSlotHeight - tableBodyBorderHeight;
      ...
      if (tableBodyRows.length > 0) {
        if (tableHeader != null) {
          if (actualScrollBarWidth > 0) {
            ...
            tableHeader.style[oj.TableDomUtils.CSS_PROP._WIDTH] = containerClientWidth - actualScrollBarWidth + oj.TableDomUtils.CSS_VAL._PX;
          } else {
            tableHeader.style[oj.TableDomUtils.CSS_PROP._WIDTH] = containerClientWidth + oj.TableDomUtils.CSS_VAL._PX;
          }
        }
      } else {
    ...
    this._setColumnWidths(0); // if all columns have a set width, set overall table width to auto to avoid acc column being shown
...
oj.TableDomUtils.prototype.refreshTouchAffordanceGlassPanePosition = function () {
  ...
  if (touchAffordanceGlassPane) {
    ...
    touchAffordanceGlassPane.style[oj.TableDomUtils.CSS_PROP._TOP] = tableBodyRect.top - tableContainerRect.top + oj.TableDomUtils.CSS_VAL._PX;
    touchAffordanceGlassPane.style[oj.TableDomUtils.CSS_PROP._BOTTOM] = tableBodyRect.bottom - tableContainerRect.bottom + oj.TableDomUtils.CSS_VAL._PX;
...
oj.TableDomUtils.prototype.refreshTableStatusPosition = function () {
  ...
  var tableStatusMessageText = tableStatusMessage.children[0]; // refresh the status message position

  var isRTL = this.component._GetReadingDirection() === 'rtl'; // position status in the center
  ...
  /** @type {Object} */
  $(tableStatusMessage).position(options);
  var msgTextOptions = {
    my: 'center',
    at: 'center',
    collision: 'none',
    of: tableStatusMessage
  };
  msgTextOptions = oj.PositionUtils.normalizeHorizontalAlignment(msgTextOptions, isRTL);

  /** @type {Object} */
  $(tableStatusMessageText).position(msgTextOptions);
};
/**
  * Cache DOM Element
...
oj.TableDomUtils.prototype._getChildElementByClassName = function (parentElement, className) {
  if (parentElement.childNodes != null && parentElement.childNodes.length > 0) {
    for (var i = 0; i < parentElement.childNodes.length; i++) {
      if (parentElement.childNodes[i].classList != null && parentElement.childNodes[i].classList.contains(className)) {
...
oj.TableDomUtils.prototype._removeTableDimensionsStyling = function () {
  ...
  tableContainer.classList.remove(oj.TableDomUtils.CSS_CLASSES._TABLE_SCROLL_HORIZONTAL_CLASS); // first remove any styling so that the browser sizes the table
  ...
  if (tableHeader != null) {
    ...
    var headerColumnTextDivs = this.getTableElementsByClassName(tableHeaderRow, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_TEXT_CLASS);
...
oj.TableDomUtils.prototype._applyColumnHeaderHeight = function (headerColumnCell, height) {
  var headerColumnDiv = this.getTableElementsByClassName(headerColumnCell, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_CLASS);

  if (headerColumnDiv.length > 0) {
    headerColumnDiv[0].style[oj.TableDomUtils.CSS_PROP._MIN_HEIGHT] = height + oj.TableDomUtils.CSS_VAL._PX;
  }
};
/**
 * @param {Element} cell
...
oj.TableDomUtils.prototype._applyForcedColumnWidth = function (cell, forcedWidth) {
  var columnElement = cell; // add 'px' to the end of the value if width is not being cleared
...
oj.TableDomUtils.prototype._setForcedColumnWidths = function () {
  ...
  var columnsCount = columns.length;
  var tableBodyRows = this.getTableBodyRows(); // find and set the widths of the forced width columns, cells, and footers first
  ...
  for (var i = 0; i < columnsCount; i++) {
    ...
    if (columnWidth > 0) {
      ...
      if (typeof columnWidth !== 'number') {
        ...
        errDetail = Translations.applyParameters(this.component._LOGGER_MSG._ERR_WIDTH_INVALID_TYPE_DETAIL, {
          width: columnWidth
        });
        Logger.warn(errSummary + '\n' + errDetail);
...
oj.TableDomUtils.prototype._setColumnWidths = function (scrollBarWidth) {
  ...
  var columnsCount = this.component._getColumnDefs().length; // loop through remaining columns to get the remaining column widths
  ...
  for (i = 0; i < columnsCount; i++) {
    if (this._forcedWidthColumns[i] !== false) {
      ...
      if (headerCell != null) {
        this._applyColumnHeaderHeight(headerCell, headerHeights[i]); // extend width of last header cell by scrollbar width only when verical scrollbar is present
  ...
  for (var i = 0; i < columnsCount; i++) {
    ...
    if (headerColumnCell != null) {
      ...
      if (headerColumnDiv.length > 0) {
        ...
        var headerColumnTextDiv = this.getTableElementsByClassName(headerColumnCell, oj.TableDomUtils.CSS_CLASSES._COLUMN_HEADER_TEXT_CLASS);
...
oj.TableDomUtils.prototype.getSortIconContainer = function (header) {
  return this.getTableElementsByClassName(header, oj.TableDomUtils.CSS_CLASSES._TABLE_SORT_ICON_CONTAINER_CLASS)[0];
};
/**
...
oj.TableRendererUtils.tableBodyCellDefaultRenderer = function (component, rowIdx, columnIdx, row, rowHashCode, context) {
  ...
  if (!cellRenderer && !cellSlotTemplate) {
    ...
    var cellContext = this.getRendererContextObject(component, tableBodyCell, {
      row: row
    }); // Copy additional properties to top-level context to work with custom element
...
oj.TableRendererUtils.getRendererContextObject = function (component, parentElement, options) {
  ...
  var dataSource = component.options.data; // unwrap the datasource if we have a PagingTableDataSource
  ...
  if (options.row != null) {
    ...
    if (component._hasEditableRow()) {
      ...
      var editableRowKey = component._getEditableRowKey(); // only set to edit mode for the editable row
...
oj.TableRendererUtils._copyMetadata = function (context, row, dataSource) {
  ...
  if (metadataContext) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var key = keys[i]; // eslint-disable-next-line no-param-reassign
...
oj.TableRendererUtils._getObjectPath = function (obj, path) {
  if (obj != null && (typeof path === 'string' || path instanceof String) && (path.indexOf('.') !== -1 || path.indexOf('[') !== -1 && path.indexOf(']') !== -1)) {
    var currentObj = obj;
    var foundPath = false;
    var pathArray = path.split('.');
    pathArray.map(function (pathSegment) {
      if (currentObj != null && pathSegment.indexOf('[') !== -1 && pathSegment.indexOf(']') !== -1) {
...
oj.TableResizeUtils.prototype.setResizeCursor = function (event) {
  ...
  if (column.resizable === this.component._OPTION_DISABLED && this._resizeStartColumnIdx !== columnIdx - 1 && this._resizeStartColumnIdx !== columnIdx + 1) {
    return false;
  }
  ...
  if (columnIdx === this._resizeStartColumnIdx || this._resizeColumnStart && columnIdx === this._resizeStartColumnIdx - 1 || !this._resizeColumnStart && columnIdx === this._resizeStartColumnIdx + 1) {
...
oj.TableResizeUtils.prototype.handleHeaderColumnResizeStart = function (event) {
  ...
  if (columnIdx != null) {
    ...
    if (column.resizable === this.component._OPTION_ENABLED && this._resizeColumnStart !== null) {
...
oj.TableResizeUtils.prototype.handleHeaderColumnResizeEnd = function (event) {
  ...
  var columnIdx = domUtils.getElementColumnIdx(eventTarget); // only resize if we end the resize on the same column or adjacent columns

  if (columnIdx !== null && columnIdx === this._resizeStartColumnIdx || this._resizeColumnStart && columnIdx === this._resizeStartColumnIdx - 1 || !this._resizeColumnStart && columnIdx === this._resizeStartColumnIdx + 1) {
    ...
    if (headerColumn != null) {
      ...
      var headerColumnWidth = headerColumnRect.width; // when scrollbar is present, reduce last column width by scrollbar width
      ...
      if (this.component._GetReadingDirection() === 'rtl' && this._resizeColumnStart || this.component._GetReadingDirection() === 'ltr' && !this._resizeColumnStart) {
      ...
      if (Math.abs(widthChange) > 2) {
        ...
        clonedColumnsOption[this._resizeStartColumnIdx].width = updatedWidth; // a resize operation should increase one column and decrease the other

        var headerColumnAdjacentIdx = this._resizeColumnStart ? this._resizeStartColumnIdx - 1 : this._resizeStartColumnIdx + 1; // when scrollbar is present, reduce last column width by scrollbar width
...
oj.TableResizeUtils.prototype._isHeaderColumnResizeStart = function (event) {
  ...
  if (headerColumn !== null) {
    ...
    var distFromRight = Math.abs(this._getPageX(event) - columnRect.right); // don't show resize cursor for column dividers at the start and end of the table
...
oj.TableResizeUtils.prototype._setTableHeaderColumnResizeIndicator = function (columnIdx) {
  ...
  if (this._resizeColumnStart) {
    if (this.component._GetReadingDirection() === 'rtl') {
      tableHeaderColumnResizeIndicator.style.left = headerColumnRect.left + headerColumnRect.width - tableRect.left + 'px';
    } else {
      tableHeaderColumnResizeIndicator.style.left = headerColumnRect.left - tableRect.left + 'px';
    }

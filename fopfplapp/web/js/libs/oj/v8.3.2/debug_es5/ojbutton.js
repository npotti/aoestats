...
(function () {
  ...
  oj.__registerWidget('oj.ojButton', $.oj.baseComponent, {
    ...
    // added to externs.js, since this is an override of a superclass member.  (That's the rule for public methods, what about protected fields?)  Would @override do the job and be better than externing?
    ...
    _InitOptions: function _InitOptions(originalDefaults, constructorOptions) {
      this._super(originalDefaults, constructorOptions); // if custom element we are supporting slots
      ...
      if (!this._IsCustomElement()) {
        if (!('disabled' in constructorOptions)) {
          ...
          this.option('disabled', !!this.element[0].disabled, {
            _context: {
              internalSet: true
            }
      ...
      if (!('label' in constructorOptions)) {
        ...
        this.option('label', this.type === 'inputPush' ? this.buttonElement.val() : this.buttonElement[0].innerHTML, // @HTMLUpdateOK getter not setter
        {
          _context: {
            internalSet: true
          }
    ...
    _ComponentCreate: function _ComponentCreate() {
      this._super(); // facilitate removing menuButton handlers separately, if app sets/clears the "menu" option
      ...
      _setChromingClass(this.rootElement, this.options.chroming); // Called for touchend/cancel on both button and document.  Listening only on button isn't completely reliable
      ...
      this._touchStartHandler = function () {
        ...
        self._toggleDefaultClasses(); // don't pass "touchend touchcancel", due to semantics of one() : it's called once per event type.
      ...
      if (this.type === 'checkbox') {
        ...
        this.buttonElement.bind('mousedown' + this.eventNamespace, function (event) {
          ...
          var isAnchor = self.type === 'anchor'; // now that anchor doesn't support Space, still keep this line, in case users try to click via Space
      ...
      this._setupMenuButton(null); // call this at the *end* of _ComponentCreate, since it needs to know whether any state classes like oj-active, oj-disabled, oj-selected, oj-hover, .oj-focus
      // have been applied.


      this._toggleDefaultClasses(); // _childMutationObserver would have been created in _setDisplayOptionOnDom if one is needed
    ...
    _NotifyContextMenuGesture: function _NotifyContextMenuGesture(menu, event, eventType) {
      ...
      this._OpenContextMenu(event, eventType, {
        position: {
          of: eventType === 'keyboard' ? $(this.rootElement) : event
        }
      });
    },
    _addMutationObserver: function _addMutationObserver() {
      ...
      var self = this;
      this._childMutationObserver = new ChildMutationObserver(this.rootElement, function (mutations) {
        mutations.forEach(function (mutation) {
          ...
          if (mutation.type === 'childList') {
            ...
            if (node && node.nodeType === 3 && target && target.nodeType === 1 && target.classList.contains('oj-button-label')) {
    ...
    _processSlots: function _processSlots() {
      var self = this;
      var elem = this.element[0];
      var rootElem = elem.parentNode;
      var isMenuButton = rootElem.tagName === 'OJ-MENU-BUTTON'; // we are responsible for reparenting any slots because of use of inner elem

      var rootSlots = oj.BaseCustomElementBridge.getSlotMap(rootElem); // don't add context menu here since we do not ever need to move it
      ...
      var slots = oj.BaseCustomElementBridge.getSlotMap(elem); // rearrange slots
    ...
    _getMenuNode: function _getMenuNode() {
    ...
    _initButtonTypes: function _initButtonTypes() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    ...
    _initButtonTypes2: function _initButtonTypes2() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      var elem = this.element[0];
    ...
    _destroy: function _destroy() {
      ...
      delete this._touchStartHandler; // TBD: won't need this after the restore-attrs feature is in place.

      var elem = this.element[0];
      elem.classList.remove('oj-helper-hidden-accessible');
      elem.removeAttribute('aria-labelledby');
      this.element.removeUniqueId(); // If disabled, we want to run the "changing from disabled to enabled" part of callee, to restore original tabindex.
      ...
      var isToggle = this._isToggle; // TBD: won't need this after the restore-attrs feature is in place.
    ...
    _updateEffectivelyDisabled: function _updateEffectivelyDisabled() {
      var elem = this.element[0];

      var effectivelyDisabled = this._IsEffectivelyDisabled(); // Unlike JQUI, root element should have exactly one of .oj-enabled and .oj-disabled at any point in time, for all flavors of Button.
    ...
    _setOption: function _setOption(key, value, flags) {
      ...
      this._super(key, value, flags); // TBD: Currently the below code relies on super already having been called.  Consider removing that dependency
      ...
      switch (key) {
        ...
        case 'disabled':
          ...
          this._manageAnchorTabIndex(oldValue, value); // must call this *after* _super(), as discussed in callee


          this._updateEffectivelyDisabled(); // If this button is inside a toolbar, the toolbar needs to be refreshed for new disabled setting
        ...
        case 'menu':
          // setting/clearing the menu sets whether this is a menuButton
    ...
    _applyCheckedStateFromDom: function _applyCheckedStateFromDom(wholeGroup) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      if (this.type === 'radio') {
     ...
     */
    _handleLabelAndIconsAtCreateTime: function _handleLabelAndIconsAtCreateTime() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      if (this.type === 'inputPush') {
     ...
     */
    _setLabelOnDomAtCreateTime: function _setLabelOnDomAtCreateTime() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    ...
    _setTextSpanIdAndLabelledBy: function _setTextSpanIdAndLabelledBy(textSpan) {
     ...
     */
    _setLabelOption: function _setLabelOption() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      if (this.type === 'inputPush') {
        // <input type=button|submit|reset> doesn't support child nodes, thus no icons, etc.
        this._setLabelOnDomOfSpanlessButton();
     ...
     */
    _setLabelOnDomOfSpanlessButton: function _setLabelOnDomOfSpanlessButton() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    ...
    _setIconsOption: function _setIconsOption() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      if (this.type === 'inputPush') {
        // <input type=button|submit|reset> doesn't support child nodes, thus no icons, etc.
        return;
     ...
     */
    _setIconOnDom: function _setIconOnDom(isStart) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
     ...
     * All params optional.
     */
    _setDisplayOptionOnDom: function _setDisplayOptionOnDom(textSpan, hasStartIcon, hasEndIcon) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      // The case for menu buttons and top level buttons
      var buttonLabelElem = this.buttonElement.children('.oj-button-label'); // The case for buttonset buttons
    ...
    _manageAnchorTabIndex: function _manageAnchorTabIndex(oldDisabled, disabled) {
      // bail if:
      // - truthiness of disabled option is same as before, e.g. changed from "a" to "b"
      // - not a standalone anchor button
      if (!oldDisabled === !disabled || this.type !== 'anchor' || this._getEnclosingContainerElement('buttonset').length || this._getEnclosingContainerElement('toolbar').length) {
        return;
      }
      ...
      if (disabled) {
        ...
        elem.setAttribute('tabindex', this._oldAnchorTabIndex); // no need to clear ivar
      }
    },
    ...
    _getEnclosingContainerElement: function _getEnclosingContainerElement(component) {
      return $(this.rootElement).closest(this._selectorMap[component]);
    },
    // component param is "buttonset" or "toolbar"
    // Returns buttonset/toolbar component, or null if none.
    _getEnclosingContainerComponent: function _getEnclosingContainerComponent(component) {
     ...
     */
    _setupMenuButton: function _setupMenuButton(oldMenuOption) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      ...
      if (this._getMenuNode()) {
        var self = this;
        this.element.attr('aria-haspopup', true).on('keydown' + this.menuEventNamespace, function (event) {
     ...
     */
    _removeMenuBehavior: function _removeMenuBehavior(menuOption) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      this.element.removeAttr('aria-haspopup').off(this.menuEventNamespace);

      this._dismissMenu(menuOption); // access menu elem directly, rather than using _getMenuOnly(menuOption).widget(), so listener is cleared even if component no longer exists.
    ...
    _getMenu: function _getMenu() {
      ...
      var menu = this._getMenuOnly(this._getMenuNode()); // if no element found, or if element has no JET Menu
      ...
      if (!this._menuListenerSet) {
        var self = this;
        var closeEvent; // Use the close event type that's appropriate for the menu, not the button.  It's possible that
     ...
     */
    _getMenuOnly: function _getMenuOnly(menuOption) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
     ...
     */
    _toggleMenu: function _toggleMenu(event, focus) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      ...
      var menuElem = menu.widget(); // If the menu is already opened, click MenuButton again will close the menu.
      ...
      this._menuVisible = true; // If menu has neither aria-label nor aria-labelledby after menu.open() calls the beforeOpen listeners, then set aria-labelledby
     ...
     */
    _dismissMenu: function _dismissMenu(menuOption, event) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
     ...
     */
    _menuDismissHandler: function _menuDismissHandler() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
     ...
     */
    _toggleDefaultClasses: function _toggleDefaultClasses() {
  ...
  oj.__registerWidget('oj.ojButtonset', $.oj.baseComponent, {
    ...
    _setCheckedOnDom: function _setCheckedOnDom(checked, $buttons) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      var type = $.type(checked);
      var valid;
      var allCheckboxes;
      var elem = this.element[0]; // whether buttonset contains exclusively checkboxes, therefore is buttonset many

      allCheckboxes = elem.tagName === 'OJ-BUTTONSET-MANY' || $buttons.length > 0 && $buttons.filter('input[type=checkbox]').length === $buttons.length; // requires an array in buttonset many case
    ...
    _setCheckedOnButtons: function _setCheckedOnButtons(checked, $buttons, type, isMany) {
      // loop once for reference comparison
      var checkedFoundCount = this._shallowCompare(checked, $buttons, isMany); // loop second for deep object comparison, supported in custom elements only, and only if no references were found in prior check
    ...
    _shallowCompare: function _shallowCompare(checked, $buttons, isMany) {
      var self = this;
      var checkedFoundCount = 0;
      $buttons.each(function () {
        // gets value off ojOption in custom element case
        var value = self._getInputValue(this); // if a buttonset many, checked is an array so do indexOf for reference comparison
    ...
    _shouldDeepCompare: function _shouldDeepCompare(checked, isMany, checkedFoundCount) {
      return isMany ? checkedFoundCount !== checked.length : !checkedFoundCount;
    },
    // compares objects deeply using
    _deepCompare: function _deepCompare(checked, $buttons, isMany, checkedFoundCount) {
      var resultCheckedFoundCount = checkedFoundCount;
      var self = this;
      $buttons.each(function () {
        // gets value off ojOption in custom element case
        var value = self._getInputValue(this); // if array loop over values and only compare objects
    ...
    _deepCompareValues: function _deepCompareValues(value, checked) {
    ...
    _getOjOptionFromInput: function _getOjOptionFromInput(input) {
    ...
    _getCheckedFromDom: function _getCheckedFromDom($buttons) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      var checked;
      var isRadio = null;
      var name = null;
    ...
    _CompareOptionValues: function _CompareOptionValues(option, value1, value2) {
      ...
      if (option === 'checked') {
        ...
        return $.type(value1) === 'array' && $.type(value2) === 'array' && this._compareArraysAsSets(value1, value2);
      }

      return this._superApply(arguments);
    },
    // Params must be arrays.  Returns true iff the arrays have the same set of elements regardless of order.
    _compareArraysAsSets: function _compareArraysAsSets(first, second) {
    ...
    _removeNonOjOptions: function _removeNonOjOptions() {
    ...
    _addOptionDecoration: function _addOptionDecoration(option) {
      // add classes to slots and wrap text if applicable
      this._addOptionClasses(option); // is the buttonset one or many


      var isOne = this.element[0].tagName === 'OJ-BUTTONSET-ONE';
      var type = isOne ? 'radio' : 'checkbox'; // get values from the oj option to set on the input

      var value = option.value;
      var disabled = option.disabled; // create label/input to decorate oj option

      var input = document.createElement('input');
      var label = document.createElement('label'); // input needs an id so label can set the for attribute

      $(input).uniqueId();
      input.value = value;
      input.type = type;
      input.disabled = disabled; // set the label for attribute

      var id = input.id;
      label.htmlFor = id; // if a radio buttonset need a uniform name, create one using the first input id
    ...
    _removeOptionDecoration: function _removeOptionDecoration(option) {
    ...
    _addOptionClasses: function _addOptionClasses(option) {
    ...
    _removeOptionClasses: function _removeOptionClasses(option) {
      var slotMap = oj.BaseCustomElementBridge.getSlotMap(option);
      var text = slotMap[''] ? slotMap[''] : null;
      var startIcon = slotMap.startIcon ? slotMap.startIcon[0] : null;
      var endIcon = slotMap.endIcon ? slotMap.endIcon[0] : null;
    ...
    _InitOptions: function _InitOptions(originalDefaults, constructorOptions) {
      ...
      this.$buttons = this.element.find(this._items); // At create time, checked can be set via either option or DOM, i.e. the "checked" properties of the buttons.
      // If app set the option, then that wins over the DOM, in which case _ComponentCreate() will later set that value on the DOM.
      // Else DOM wins, in which case we set the option from the DOM here, with any remaining tasks done later in _ComponentCreate().
    ...
    _NotifyContextMenuGesture: function _NotifyContextMenuGesture(menu, event, eventType) {
      ...
      this._OpenContextMenu(event, eventType, {
        ...
        position: {
          of: eventType === 'keyboard' ? currentButton.ojButton('widget') : event
        }
      });
    },
    _propagateDisabled: function _propagateDisabled(disabled) {
    ...
    _setRole: function _setRole(focusManagement) {
      var elem = this.element[0];
    ...
    _setOption: function _setOption(key, value, flags) {
      // Override of protected base class method.  Method name needn't be quoted since is in externs.js.
      var oldValue = this.options[key]; // previously called super at end, so that optionChange (fired at end of super) is fired at very end, but now must call at start, so that
      ...
      if (key === 'disabled') {
        ...
        this._refreshTabStop(); // If this button is inside a toolbar, the toolbar needs to be refreshed for new disabled setting
        ...
        _setChromingClass(this.element[0], value); // refresh the buttons to make them re-fetch their chroming option, in case it's still set to the default dynamic getter,
    ...
    refresh: function refresh() {
      // Override of public base class method (unlike JQUI).  Method name needn't be quoted since is in externs.js.
      this._super(); // Should not need to call refresh on oj-options. If the content of an oj-option is modified then
    ...
    _setup: function _setup(isCreate) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      var self = this;
      var elem = this.element[0];
      ...
      this.$buttons // refresh any buttons underneath us that already exist, like JQUI does
      ...
      .filter(':oj-button').ojButton('refresh').each(function () {
        $(this).data('oj-ojButton')._applyCheckedStateFromDom(false); // set .oj-selected

      }).end() // Create buttons underneath us
      .not(':oj-button').ojButton({
        display: this.options.display
      }) // sets .oj-selected
      .end(); // Update rounded corners, etc.
      ...
      if (this.options.focusManagement === 'oneTabstop') {
        ...
        this.$buttons.unbind('keydown' + this.eventNamespace).bind('keydown' + this.eventNamespace, function (event) {
      ...
      this.element.uniqueId(); // copy labelledBy to aria-labelledBy

      this._updateLabelledBy(elem, null, this.options.labelledBy, this.element); // set describedby on the element as aria-describedby
    ...
    _refreshTabStop: function _refreshTabStop() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      if (this.options.focusManagement === 'oneTabstop') {
        // the subset of Buttonset buttons that are enabled.  Effectively disabled buttons are not tabbable.
        this.$enabledButtons = this.$buttons.filter(function () {
    ...
    // No return value.
    _initTabindexes: function _initTabindexes(isCreate) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    ...
    _mapToTabbable: function _mapToTabbable($button) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    ...
    // No return value.
    _setTabStop: function _setTabStop($button) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    ...
    // No return value.
    _handleKeyDown: function _handleKeyDown(event, $button) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      switch (event.which) {
    ...
    _destroy: function _destroy() {
      ...
      .removeClass('oj-buttonset-first oj-buttonset-last').end() // Recursively destroy Bset's buttons like JQUI.
  ...
  function _getChromingDefault(componentName, element, actualContainers) {
    var containerConstructor = _findContainer(element, actualContainers, _interestingContainers[componentName]); // If the component is in an interesting container (buttonset or toolbar), then the default chroming is the current value of the chroming option of the nearest such container.
    ...
    return (ThemeUtils.parseJSONFromFontFamily('oj-' + componentName + '-option-defaults') || {}).chroming;
  }
...
(function () {
  ...
  oj.CustomElementBridge.register('oj-button', {
    ...
    innerDomFunction: function innerDomFunction(element) {
...
(function () {
  ...
  __oj_buttonset_one_metadata.extension._ALIASED_PROPS = {
    value: 'checked'
  };
...
(function () {
  ...
  __oj_buttonset_many_metadata.extension._ALIASED_PROPS = {
    value: 'checked'
  };

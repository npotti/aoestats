...
(function () {
  ...
  var PENDING_DELETE_INDEX_KEY = createSymbolOrString('_ko_ffe_pending_delete_index'); // Abandoned promise rejection for data provider
  ...
  function OjForEach(element, value, context) {
    ...
    this.pendingDeletes = []; // The headDataPromise and tailDataPromise are used to provide an order for Data Provider events:
    ...
    this.tailDataPromise = null; // Remove existing content.
  ...
  OjForEach.prototype.fetchData = function () {
    ...
    var headPromise; // function passed to iterator.next() promise on success, until result.done is set to true

    var getProcessResultsFunc = function getProcessResultsFunc(changeSet, onArrayChangeCallback, callbackObj) {
      return function (result) {
        if (callbackObj.headDataPromise === headPromise) {
          ...
          if (result.done) {
            ...
            iterator.next().then(getProcessResultsFunc(changeSet, onArrayChangeCallback, callbackObj), function (reason) {
              busyStateCallback();
    ...
    headPromise = new Promise(function (resolve, reject) {
      ...
      iterator.next().then(getProcessResultsFunc([], this.onArrayChange, this), function (reason) {
        busyStateCallback();
        dataPromiseReject(reason);
        this.resetChainIfCompleted(headPromise);
      }.bind(this));
    }.bind(this)); // the catch handles a case when the head promise does not have a chain and we need to catch valid rejection
  ...
  OjForEach.prototype.getDataMutationHelper = function (event, promiseContainer) {
    ...
    if (addDetail) {
      ...
      if (!Array.isArray(eventData)) {
        ...
        busyStateCallback = this.registerBusyState();
        dataPromise = this.data.fetchByKeys({
          keys: eventKeys

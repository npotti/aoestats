...
oj.Object.createSubclass(oj.TreeDataProviderContentHandler, List.DataProviderContentHandler, 'oj.TreeDataProviderContentHandler');
/**
...
oj.TreeDataProviderContentHandler.prototype._fetchChildren = function (parent, parentElem, successCallback) {
  var self = this;
  ...
  this.m_fetching = true; // use -1 to fetch all child rows
  ...
  Promise.all([promise, enginePromise]).then(function (values) {
    ...
    $(parentElem).empty(); // append loading indicator at the end as needed
...
oj.TreeDataProviderContentHandler.prototype._handleFetchSuccess = function (dataObj, parent, parentElem, successCallback, templateEngine) {
  ...
  if (data.length === keys.length) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      this.addItem(fragment, -1, row, this.getMetadata(index, keys[i], row, parentElem), templateEngine);
      index += 1;
    }

    parentElem.appendChild(fragment); // update aria-colspan on the gridcell representing the group header
...
oj.TreeDataProviderContentHandler.prototype.addItemsForModelInsert = function (data, indexes, keys, parentKeys, isBeforeKeys, refKeys) {
  // template engine should have already been loaded
  var templateEngine = this.getTemplateEngine();
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    this.addItem(parentElem, index, data[i], this.getMetadata(index, keys[i], data[i], parentElem), templateEngine, this.afterRenderItemForInsertEvent.bind(this));
...
oj.TreeDataProviderContentHandler.prototype.handleModelRefreshEvent = function (event) {
  ...
  this.m_childDataProviders.clear(); // fetch data
...
oj.StaticContentHandler.prototype.Destroy = function () {
  ...
  if (this.shouldUseGridRole() && this.isCardLayout() && !this.IsHierarchical()) {
    $(this.m_root).children().first().children().unwrap().children().unwrap();
  }
};
/**
...
oj.StaticContentHandler.prototype.restoreContent = function (elem, depth) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (item.hasClass(itemElementStyleClass)) {
      ...
      item.removeClass(itemElementStyleClass).removeClass(itemStyleClass).removeClass(itemLayoutStyleClass).removeClass(this.m_widget.getDepthStyleClass(depth)).removeClass('oj-skipfocus').removeClass('oj-focus').removeClass('oj-hover').removeClass('oj-expanded').removeClass('oj-collapsed').removeClass('oj-selected');
...
/** @lends oj._ojListView.prototype */
{
  ...
  _resetInternal: function _resetInternal() {
    ...
    this.ClearCache(); // give dnd context a chance to clear internals
  ...
  ShouldRefresh: function ShouldRefresh(options) {
    return options.data != null || options.drillMode != null || options.groupHeaderPosition != null || options.item != null || options.scrollPolicy != null || options.scrollPolicyOptions != null || options.gridlines != null;
  },
  ...
  _updateFirstSelectedItem: function _updateFirstSelectedItem(selected) {
    ...
    if (key != null) {
      var value = {
        key: key,
        data: this.getDataForVisibleItem({
          key: key
        })
      };
    ...
    if (options.selection != null || options.selected != null) {
      if (this._isSelectionEnabled()) {
        ...
        if (selected != null && selected.isAddAll()) {
          ...
          var set = selected != null ? selected.values() : options.selection; // remove any non-selectable items, this will effectively clone the selection as well,
          // which we'll need to do.  filterSelection takes an iterable

          var filteredSelection = this._filterSelection(set); // eslint-disable-next-line no-param-reassign


          options.selection = filteredSelection; // eslint-disable-next-line no-param-reassign

          selected = this.GetOption('selected');
          selected = selected.clear();
          selected = selected.add(filteredSelection); // eslint-disable-next-line no-param-reassign
    ...
    if (this.m_keyElemMap == null && this._isSelectionEnabled() && this._isSelectionRequired()) {
      ...
      if (selection.length > 0) {
        this._validateSelection(selection).then(function (valid) {
          ...
          if (!valid) {
            self._selectFirstSelectableItem(); // ... and fire ready event (legacy)
  ...
  _validateSelection: function _validateSelection(selection) {
    var promise = this._validateKeys(selection); // if it cannot validate, treat it as a valid key and return a Promise that resolves to true so that the key does not get remove.
  ...
  _isScrollable: function _isScrollable() {
    return Math.abs(this._getScrollHeight() - this._getClientHeight()) >= 1 || Math.abs(this._getScrollWidth() - this._getClientWidth()) >= 1;
  },
   ...
   * @private
   */
  _filterSelection: function _filterSelection(selection) {
    var filtered = [];

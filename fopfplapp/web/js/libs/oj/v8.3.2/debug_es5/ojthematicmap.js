 ...
 *   See the thematic map <a href="../jetCookbook.html?component=thematicMap&demo=basemaps">Map Provider Demo</a> for an example.
 * </p>
 * <p>
...
oj.__registerWidget('oj.ojThematicMap', $.oj.dvtBaseComponent, {
  ...
  options: {
    ...
    /**
     * The oj.DataProvider for the areas of the thematic map. It should provide data rows where each row will map data
     ...
     * The oj.DataProvider can either have an arbitrary data shape, in which case an <oj-thematic-map-area> element must be specified in the areaTemplate slot or it can have [oj.ojThematicMap.Area]{@link oj.ojThematicMap#Area} as its data shape, in which case no template is required.
     * @expose
     * @name areaData
    ...
    /**
     * The oj.DataProvider for the links of the thematic map. It should provide data rows where each row will map data
     ...
     * The oj.DataProvider can either have an arbitrary data shape, in which case an <oj-thematic-map-link> element
    ...
    /**
     * The oj.DataProvider for the markers of the thematic map. It should provide data rows where each row will map data
     ...
     * The oj.DataProvider can either have an arbitrary data shape, in which case an <oj-thematic-map-marker> element
     ...
     * @instance
     * @type {Object|null}
     * @ojsignature {target: "Type", value: "oj.DataProvider<K3, D3>|null", jsdocOverride:true}
     * @default null
     *
     * @example <caption>Initialize the thematic map with the
  ...
  _InitOptions: function _InitOptions(originalDefaults, constructorOptions) {
    this._super(originalDefaults, constructorOptions); // styleDefaults subproperty defaults are dynamically generated
    ...
    if (numUpdates === 1 && newAreaLayers && oldAreaLayers && oldAreaLayers.length > 0) {
      for (i = 0; i < newAreaLayers.length; i++) {
        ...
        for (var k = 0; k < areaLayerKeys.length; k++) {
          var areaLayerKey = areaLayerKeys[k]; // check to see if option update is a data layer update by seeing if any other area layer property is changed
  ...
  _ProcessOptions: function _ProcessOptions() {
    this._super(); // wrap tooltip function in try catch
    ...
    if (tooltipFun) {
      ...
      this.options._tooltip = {
        renderer: function renderer(context) {
          ...
          try {
            ...
            Logger.warn('Showing default tooltip. ' + error);
    ...
    var i;
    var renderer; // call custom renderers
  ...
  _Render: function _Render() {
    ...
    var areaLayers = this.options.areaLayers;
    var i; // Don't render unless a basemap and area layer or at least a mapProvider for custom element are provided
   ...
   * @memberof oj.ojThematicMap
   */
  _getContextHandler: function _getContextHandler() {
    var thisRef = this;

    var contextHandlerFunc = function contextHandlerFunc(parentElement, rootElement, data, itemData, state, previousState) {
      var context = {
   ...
   * @memberof oj.ojThematicMap
   */
  renderDefaultHover: function renderDefaultHover(context) {
   ...
   * @memberof oj.ojThematicMap
   */
  renderDefaultSelection: function renderDefaultSelection(context) {
   ...
   * @memberof oj.ojThematicMap
   */
  renderDefaultFocus: function renderDefaultFocus(context) {
   ...
   * @private
   */
  _loadBasemap: function _loadBasemap(basemap, areaLayers) {
    ...
    var pointDataLayers = this.options.pointDataLayers; // Don't try and load cities basemap if mapProvider is used
   ...
   * @private
   */
  _loadResourceByUrl: function _loadResourceByUrl(url, bRenderOnFail) {
   ...
   * @private
   */
  _loadBasemapHelper: function _loadBasemapHelper(basemap, layer, locale) {
    var isLoaded = true;
    ...
    if (locale.indexOf('en') === -1) {
      ...
      basemap = basemap.charAt(0).toUpperCase() + basemap.slice(1); // eslint-disable-next-line no-param-reassign

      layer = layer.charAt(0).toUpperCase() + layer.slice(1);
      var bundleName = this._basemapPath + 'resourceBundles/' + basemap + layer + 'Bundle_'; // Go thru list of supported DVT languages
   ...
   * @private
   */
  _updateDataLayerSelection: function _updateDataLayerSelection(options) {
    var selection = options.selection;
    var hasSelection = (this._IsCustomElement() ? selection.length : Object.keys(selection).length) > 0; // Assume ids are unique amongst all data items for custom elements
   ...
   */
  getMarker: function getMarker(index) {
    if (this._IsCustomElement()) {
  ...
  _GetComponentDeferredDataPaths: function _GetComponentDeferredDataPaths() {
    if (this._IsCustomElement()) {
      return {
        root: ['areas', 'markers', 'links', 'areaData', 'markerData', 'linkData']
      };
    }
  ...
  _GetDataContext: function _GetDataContext(options) {
    ...
    return {
      basemap: basemap,
      layer: layer,
      ids: dvt.DvtBaseMapManager.getLayerIds(basemap, layer)
    };
  },
   ...
   * @memberof oj.ojThematicMap
   */
  _mapCustomElementOptions: function _mapCustomElementOptions(options) {
    // AnimationOnMapChange is not exposed, map animationOnDisplay to that property instead
    // eslint-disable-next-line no-param-reassign
    options.animationOnMapChange = options.animationOnDisplay; // Create areaLayers and pointDataLayers objects for data items
    // We will use a static id for these data layers for automation lookup

    var areaDataLayerId = 'adl1';
    var pointDataLayerId = 'pdl1';
    var areaLayers = [{}]; // eslint-disable-next-line no-param-reassign

    options.areaLayers = areaLayers;
    var areaLayer = areaLayers[0];
    var adl = {
      id: areaDataLayerId
    }; // need id for automation lookup
    ...
    var pdl = pointDataLayers[0]; // New top level properties that should be mapped to the areaLayer

    var props = ['labelDisplay', 'labelType'];

    this._mapOptionHelper(options, props, [areaLayer]); // New top level properties that should be mapped to the area/pointDataLayers


    props = ['animationOnDataChange', 'focusRenderer', 'hoverRenderer', 'renderer', 'selectionMode', 'selectionRenderer'];

    this._mapOptionHelper(options, props, [adl, pdl]); // New top level properties that should be mapped to the areaDataLayer


    props = ['areas', 'isolatedItem'];

    this._mapOptionHelper(options, props, [adl]); // We need to split up selection ids to their data layers
    // so we can keep track of events from different data layers.


    if (!this._selectionDataLayerMap) {
      this._selectionDataLayerMap = {
        adl1: [],
        pdl1: []
      };
   ...
   * @memberof oj.ojThematicMap
   */
  _mapOptionHelper: function _mapOptionHelper(options, props, optionObjs) {
   ...
   * @private
   */
  _getDataLayerId: function _getDataLayerId(dataLayer, index, dataItemType) {
    if (this._IsCustomElement()) {
      switch (dataItemType) {
        case 'area':
          return 'adl1';
  ...
  _WrapInlineTemplateRenderer: function _WrapInlineTemplateRenderer(origRenderer, templateName, option) {
    var getDefaultWrapperFunction = function getDefaultWrapperFunction(defaultFunc) {
 ...
 * @property {any=}          startLocation.id The marker or area id to be used as the start point.

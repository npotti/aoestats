...
oj.__registerWidget('oj.editableValue', $.oj.baseComponent, {
  ...
  _initAsyncValidatorMessagingHint: function _initAsyncValidatorMessagingHint() {
    ...
    if (allAsyncValidatorsWithHint.length > 0) {
      ...
      syncValidatorHintMC = self._getMessagingContent(self._MESSAGING_CONTENT_UPDATE_TYPE.VALIDATOR_HINTS); // we use a counter to keep track of the busycontext
  ...
  _updateValidatorMessagingHint: function _updateValidatorMessagingHint() {
    ...
    var syncValidatorHintMC = this._getMessagingContent(this._MESSAGING_CONTENT_UPDATE_TYPE.VALIDATOR_HINTS);
  ...
  _initComponentMessaging: function _initComponentMessaging() {
    ...
    var messagingContent = this._getMessagingContent(this._MESSAGING_CONTENT_UPDATE_TYPE.ALL); // if default placeholder is currently set then it needs to be cleared here. This is needed for
  ...
  _getMessagingContent: function _getMessagingContent(updateType) {
    ...
    var converterHint = '';
    var messages;
    var validatorHints = []; // eslint-disable-next-line no-param-reassign
    ...
    if (updateType === this._MESSAGING_CONTENT_UPDATE_TYPE.ALL || updateType === this._MESSAGING_CONTENT_UPDATE_TYPE.VALIDITY_STATE) {
      ...
      messages = this._getMessages(); // update validityState before packaging it

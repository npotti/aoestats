  ...
  oj.__registerWidget('oj.ojToolbar', $.oj.baseComponent, {
    ...
    _InitOptions: function _InitOptions(originalDefaults, constructorOptions) {
      ...
      if ('disabled' in constructorOptions) {
        Logger.warn("Caller attempted to set the 'disabled' option on Toolbar, but Toolbar does not support the 'disabled' option.  See API doc.");
      }
    },
    ...
    _setOption: function _setOption(key, value) {
      ...
      if (key === 'disabled') {
        Logger.warn("Caller attempted to set the 'disabled' option on Toolbar, but Toolbar does not support the 'disabled' option.  See API doc.  Ignoring the call.");
        return;
    ...
    _setup: function _setup() {
      ...
      var self = this;
      var elem = this.element[0];
      this.isRtl = this._GetReadingDirection() === 'rtl'; // initialize as an empty JQuery object so we don't fall over if focus hasn't been introduced before a click is made

      this.$enabledButtons = $(); // When toolbar is binding listeners to buttons, use the Toolbar's eventNamespace, not the Button's
      ...
      if (this._IsCustomElement()) {
        ...
        this.topLevelChildren = elem.querySelectorAll('oj-button, oj-menu-button, oj-buttonset-one, oj-buttonset-many'); // refresh the top-level buttons, and refresh the buttonsets to make them refresh their buttons, so that all toolbar buttons are refreshed.
        // Reason: to make them re-fetch their chroming option, in case it's still set to the default dynamic getter,
        // which takes its value from the containing buttonset or toolbar if present.
        // TBD: Consider only calling refresh() on children that haven't had their chroming option set, i.e. those still using the dynamic getter.
    ...
    _handleFocus: function _handleFocus($button) {
    ...
    _handleInitialFocus: function _handleInitialFocus() {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
      var self = this;
      var elem = this.element[0]; // remove this event handling as we only want to run this setup logic on the first focusin event
    ...
    _getButtonFocusElem: function _getButtonFocusElem(button) {
      // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    ...
    _initTabindexes: function _initTabindexes(isCreate) {
      ...
      if (this._IsCustomElement()) {
        for (var i = 0; i < this.$buttons.length; i++) {
          // TODO: this depends on having solution for bind and unbind
    ...
    // No return value.
    _setTabStop: function _setTabStop(_button) {
      var $button = _button; // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    ...
    _destroy: function _destroy() {
      ...
      elem.removeAttribute('role'); // Since we're not destroying the buttons themselves, the restoreAttrs code will NOT take care of this.
...
(function () {
  ...
  oj.CustomElementBridge.register('oj-toolbar', {
    metadata: __oj_toolbar_metadata
  });
})();

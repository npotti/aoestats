...
(function () {
  oj.__registerWidget('oj.ojTrain', $.oj.baseComponent, {
     ...
     * @private
     */
    _setupTrain: function _setupTrain() {
      // Constrain initial value
      var options = this.options;
      var steps = options.steps;
      this._stepNum = steps.length; // Train Wrapper
      ...
      this._stepList.addClass('oj-train-step-list'); // Initialize the background progress bar object that will be updated to have the correct width based on the current step.
      ...
      for (var i = 0; i < this._stepNum; i++) {
        ...
        var stepTag = $('<li>').addClass('oj-train-step-list-item').attr({
          id: this._stepArray[i][1]
      ...
      this._connectorFill.css({
        width: connectorFillWidth + '%'
      });
     ...
     * @private
     */
    _setupArray: function _setupArray() {
      var options = this.options;
      this._stepArray = [];
     ...
     * @private
     */
    _drawButton: function _drawButton(index) {
      var button = $('<div></div>').addClass('oj-train-button');
      ...
      if (this._stepArray[index]) {
        ...
        var stepBackground = this._stepList.children().eq(index).find('.oj-train-button-connector'); // Check that there are at least 3 items in the list item indicating that a button has already been created for this step.
     ...
     * @private
     */
    _drawMessageType: function _drawMessageType(index) {
      if (this._stepArray[index] && this._stepArray[index][4]) {
        var icon = $('<div></div>').addClass('oj-train-icon oj-component-icon').attr('aria-hidden', 'true');
        var scrnRead = $('<span></span>');
        var desc = '';
        var self = this;
     ...
     * @private
     */
    _fireOptionChange: function _fireOptionChange(previousValue, value, originalEvent) {
      var eventData = {
        ...
        optionMetadata: {
          writeback: originalEvent ? 'shouldWrite' : 'shouldNotWrite'
        }
      };
     ...
     * @private
     */
    _drawStepNumberLabel: function _drawStepNumberLabel(index) {
      var stepNumberLabel = $('<div></div>').addClass('oj-train-button-text');
     ...
     * @private
     */
    _drawIndividualConnectorLine: function _drawIndividualConnectorLine(index) {
      if (index !== this._stepNum - 1) {
        var stepConnector = $('<div></div>').addClass('oj-train-step-individual-connector');
     ...
     * @private
     */
    _drawStepFill: function _drawStepFill(index) {
     ...
     * @private
     */
    _drawLabel: function _drawLabel(index) {
      var self = this;

      if (this._stepArray[index]) {
        var labelWrapper = $('<div></div>').addClass('oj-train-label-wrapper');
        var label = $('<a>');
     ...
     * @private
     */
    _getStepIndex: function _getStepIndex(id) {
      for (var i = 0; i < this._stepNum; i++) {
     ...
     * @memberof oj.ojTrain
     */
    nextSelectableStep: function nextSelectableStep() {
     ...
     * @memberof oj.ojTrain
     */
    previousSelectableStep: function previousSelectableStep() {
     ...
     * @memberof oj.ojTrain
     */
    getNextSelectableStep: function getNextSelectableStep() {
     ...
     * @memberof oj.ojTrain
     */
    getPreviousSelectableStep: function getPreviousSelectableStep() {
      var selectedIndex = this._getStepIndex(this.options.selectedStep || this.options.selected);
     ...
     * @memberof oj.ojTrain
     */
    setStep: function setStep(stepProperties) {
     ...
     * @return {void}
     */
    updateStep: function updateStep(id, stepProperties) {
      if (id) {
    ...
    _setOption: function _setOption(key, value, flags) {
      if ((key === 'selectedStep' || key === 'selected') && this._stepArray && this._stepArray[this._selectedIndex]) {
    ...
    _destroy: function _destroy() {
      ...
      this.element.find('.oj-train-step-list').remove(); // Call super at the end for destroy.
     ...
     * @private
     */
    _fireSelectedStepOptionChange: function _fireSelectedStepOptionChange(newSelectedStep, event) {
      var prevSelected = this._stepArray[this._selectedIndex][1];
     ...
     * @private
     */
    _setFocus: function _setFocus(id) {
    ...
    getSubIdByNode: function getSubIdByNode(node) {
      ...
      for (var stepIndex = 0; stepIndex < stepNum; stepIndex++) {
        ...
        var subIdNode = this.getNodeBySubId(stepLocator); // Checking whether currentNode is a descendant of the button or label because
        ...
        if ($(currentNode).closest(subIdNode).length > 0 || $(currentNode).closest($(subIdNode.parentNode.parentNode).find('.oj-train-label')[0]).length > 0) {
          return stepLocator;
        }
      }
...
(function () {
  ...
  oj.CustomElementBridge.register('oj-train', {
    metadata: __oj_train_metadata
  });
})();

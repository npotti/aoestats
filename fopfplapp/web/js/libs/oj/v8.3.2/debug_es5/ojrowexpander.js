...
oj.FlattenedNodeSet.prototype.getCount = function () {
  ...
  if (this.m_count === undefined) {
    this.m_count = this._getCount(this.m_nodeSet, 0); // if explicit start is specified (subset), need to take that into
...
oj.FlattenedNodeSet.prototype._getCount = function (nodeSet, total) {
  ...
  resultTotal += count; // if there's child node set
...
oj.FlattenedNodeSet.prototype._getDataOrMetadata = function (nodeSet, index, current, func) {
  ...
  for (var i = 0; i < count; i++) {
    var currIndex = current.index; // found the index
    ...
    current.index = currIndex + 1; // if there's child node set
...
oj.NodeSetWrapper.prototype.getCount = function () {
  ...
  var nodeCount = this.m_nodeSet.getCount(); // if the requested start is a subset of the NodeSet, adjust
  // accordingly
...
oj.NodeSetWrapper.prototype.getMetadata = function (index) {
  ...
  var rowKey = metadata.key; // inject additional metadata
...
oj.FlattenedTreeDataSource.prototype.Init = function () {
  // super
  oj.FlattenedTreeDataSource.superclass.Init.call(this); // we have to react if the underlying TreeDataSource has changed

  this.m_wrapped.on('change', this._handleModelEvent.bind(this));
  this.m_busy = false; // retrieves the fetch size against the underlying data source
...
oj.FlattenedTreeDataSource.prototype.Destroy = function () {
  ...
  this.m_wrapped.off('change'); // delegate to underlying data source
...
oj.FlattenedTreeDataSource.prototype.fetchRows = function (range, callbacks) {
  this.m_busy = true; // check if we should fetch rows from descendants result set or walk the tree
...
oj.FlattenedTreeDataSource.prototype._fetchRowsFromChildren = function (range, callbacks) {
  var fetchRange; // this condition should always be true since in high-water mark scrolling we are
  // always asking for rows after the current last row

  if (range.start > this._getLastIndex()) {
    var maxFetchSize = this._getMaxFetchSize(); // initial fetch


    if (this._getLastIndex() < 0) {
      fetchRange = {};
      fetchRange.start = range.start; // adjust fetch count if neccessary
      ...
      var depth = lastEntry.depth; // see if we are fetching within the parent
      ...
      if (count === -1 || index < count - 1) {
        ...
        if (!processed) {
          ...
          var nodeSet = new oj.EmptyNodeSet(null, range.start); // invoke original success callback
  ...
  this.handleMaxCountReached(range, callbacks); // busy can't be set by max count reached because it can be overwritten by wrapped datasource
...
oj.FlattenedTreeDataSource.prototype._handleFetchSuccess = function (nodeSet, parent, depth, originalRange, requestedRange, count, callbacks, options) {
  var toExpand = []; // first process the node set to get the rows to expand with the indexes
  ...
  nodeSet = new oj.NodeSetWrapper(nodeSet, this.insertMetadata.bind(this), flattenedRange); // expand the fetched nodes to get the node set as full as possible from the original fetch

  if (toExpand.length !== 0) {
    // there are rows to expand, so we'll need to combine the nodeset after
    // we got the expanded nodeset
    var queue = [];
    queue.push(toExpand); // we'll reuse the syncExpandRows method, which is used to combine nested expanding
    ...
    prevNodeSetInfo.callbacks = {
      success: function (newNodeSet) {
        this._verifyFetchResults(newNodeSet, parent, depth, originalRange, requestedRange, count, callbacks, options);
      }.bind(this),
      error: function (status) {
        this._handleFetchError(status, callbacks);
      }.bind(this)
...
oj.FlattenedTreeDataSource.prototype._fetchFromAncestors = function (parent, depth, range, callbacks, maxFetchSize, options) {
  ...
  var count;
  var fetchRange;
  var currDepth; // adjusted for loop i = current-1 to i = current.
  ...
  for (var i = current; i >= 0; i--) {
    ...
    if (currDepth < depth) {
      ...
      if (countUnknown || index < count - 1) {
        ...
        if (countUnknown) {
          fetchRange.count = Math.min(maxFetchSize, Math.max(0, fetchSize)); // if count is unknown, we cannot do batch fetch

          batchFetchOptions = undefined; // stop going up parents
        } else {
        ...
        this.m_wrapped.fetchChildren(parent, fetchRange, {
          success: function (_parent, _currDepth, _fetchRange, _count, nodeSet) {
            this._handleFetchSuccess(nodeSet, _parent, _currDepth, range, _fetchRange, _count, callbacks, options);
  ...
  if (batchFetchOptions != null) {
    this.m_wrapped.fetchChildren(parent, {
      start: range.count,
      count: 0
    }, {
      success: function (nodeSet) {
        this._handleFetchSuccess(nodeSet, parent, currDepth, range, fetchRange, count, callbacks, options);
      }.bind(this),
      error: function (status) {
        this._handleFetchError(status, callbacks);
      }.bind(this)
...
oj.FlattenedTreeDataSource.prototype._processNodeSet = function (nodeSet, parent, depth, toExpand) {
  ...
  var nodeCount = nodeSet.getCount(); // walk the node set and populate the internal cache
...
oj.FlattenedTreeDataSource.prototype._handleFetchDescendantsSuccess = function (nodeSet, range, callbacks) {
  var options;
  var actualStart;
  var _nodeSet = nodeSet; // this condition should always be true since in high-water mark scrolling we are
  // always asking for rows after the current last row

  if (range.start > this._getLastIndex()) {
    var maxFetchSize = this._getMaxFetchSize();

    var count = Math.min(maxFetchSize, range.count); // wrap it to inject additional metadata

    _nodeSet = new oj.NodeSetWrapper(_nodeSet, this.insertMetadata.bind(this), null, this.m_collapsedKeys);
  ...
  this.m_busy = false; // process any outstanding operations
...
oj.FlattenedTreeDataSource.prototype._processDescendantsNodeSet = function (nodeSet, parent, depth, lastEntry, maxCount, options) {
  ...
  for (var i = 0; i < nodeCount; i++) {
    ...
    var metadata = nodeSet.getMetadata(nodeStart + i);
    var key = metadata.key; // see if we need to check depth

    if (options.checkDepth) {
      if (lastEntry.depth >= depth) {
        // eslint-disable-next-line no-param-reassign
        options.found = true; // eslint-disable-next-line no-param-reassign
    ...
    if (lastEntry == null || options.found) {
      this._addEntry(key, depth, nodeStart + i, parent); // eslint-disable-next-line no-param-reassign


      options.count += 1; // include state metadata for row expander
...
oj.FlattenedTreeDataSource.prototype._expand = function (rowKey, options) {
  ...
  var maxCount = this.getMaxCount(); // if cache is full, check if the rowKey is the last row, if it's
  ...
  this.m_wrapped.fetchChildren(rowKey, {
    start: 0,
    count: fetchSize
  }, {
    success: function (nodeSet) {
...
oj.FlattenedTreeDataSource.prototype.collapse = function (rowKey) {
  ...
  var parent = this._getEntry(rowIndex - 1); // could happen if parent node has already collapsed
  ...
  for (var i = 0; i < count; i++) {
    keys.push({
      key: this._getEntry(rowIndex + i).key,
      index: rowIndex + i
    });
  ...
  this._removeEntry(rowIndex, count); // (firing of event to view)


  this.removeRows(keys); // fire datasource event
...
oj.FlattenedTreeDataSource.prototype._removeExpanded = function (rowKey) {
  var index = this._getExpandedKeyIndex(rowKey); // index found, remove from array
...
oj.FlattenedTreeDataSource.prototype._removeCollapsed = function (rowKey) {
  var index = this._getCollapsedKeyIndex(rowKey); // index found, remove from array
...
oj.FlattenedTreeDataSource.prototype.handleExpandSuccess = function (rowKey, nodeSet, childCount, options) {
  var queue;
  var prevNodeSetInfo; // wrap it to inject additional metadata
  // eslint-disable-next-line no-param-reassign
  ...
  var toExpand = []; // go through the node set and insert an entry with info about the row into internal cache
  ...
  if (done) {
    ...
    if (prevNodeSetInfo != null) {
      ...
      if (callbacks != null) {
        ...
        callbacks.success.call(null, nodeSet); // we are done at this point, we don't fire insert events
    ...
    if (prevNodeSetInfo != null) {
      ...
      for (var j = 0; j < prevNodeSetInfo.keys.length; j++) {
        this.handleEvent('expand', {
          rowKey: prevNodeSetInfo.keys[j]
        });
      }
    }

    this.m_busy = false; // fire event
    ...
    if (prevNodeSetInfo === undefined) {
      prevNodeSetInfo = {}; // populate the initial insertion index and key, this is needed when we are actually firing
      // the insert event

      prevNodeSetInfo.firstIndex = rangeStart;
      prevNodeSetInfo.firstKey = rowKey; // cache of row keys for firing expand event when everything is done

      prevNodeSetInfo.keys = [];
    } // update the previous node set


    prevNodeSetInfo.nodeSet = nodeSet; // update keys array for fire expand events later

    prevNodeSetInfo.keys.push(rowKey); // expand any child rows that should be expanded
...
oj.FlattenedTreeDataSource.prototype._syncExpandRows = function (queue, prevNodeSetInfo) {
  ...
  var last = queue[queue.length - 1]; // then take the first row key from the set

  var key = last.shift(); // if this is the last item in the set, we can remove the set from queue
  ...
  this._expand(key, {
    prevNodeSetInfo: prevNodeSetInfo,
    queue: queue
  });
};
/**
...
oj.FlattenedTreeDataSource.prototype._deleteAllRowsBelow = function (index, count) {
  ...
  for (var i = 0; i < actualCount; i++) {
    keys.push({
      key: this._getEntry(index + i).key,
      index: index + i
    });
  } // update internal cache


  this._removeEntry(index, actualCount); // fire event to remove rows from view
...
oj.FlattenedTreeDataSource.prototype._handleInsertEvent = function (parentKey, index, nodeSet) {
  ...
  var depth = parent.depth + 1;
  var insertIndex = parentIndex + index + 1; // there should only be one row in the set

  var metadata = nodeSet.getMetadata(nodeSet.getStart()); // insert into cache
...
oj.FlattenedTreeDataSource.prototype._handleDeleteEvent = function (parentKey, index) {
  ...
  var start = this._getEntry(startIndex); // make sure the child data is valid


  oj.Assert.assert(start.parent === parent && start.depth === parent.depth + 1); // remove the entry and all of its children from cache
  ...
  while (currentIndex <= lastIndex) {
    var current = this._getEntry(currentIndex); // check if we have reached the last child of the deleted node
...
oj.FlattenedTreeDataSource.prototype.refresh = function () {
  // clear the cache
  this._clearAll(); // todo: more work here to force fetch (remove then insert)
...
oj.__registerWidget('oj.ojRowExpander', $.oj.baseComponent, {
  ...
  _initContent: function _initContent() {
    ...
    this.depth = oj.DataProviderFeatureChecker.isDataProvider(this.datasource) ? context.treeDepth + 1 : context.depth;
    ...
    this.index = oj.DataProviderFeatureChecker.isDataProvider(this.datasource) ? context.indexFromParent : context.index;
    ...
    if (this.iconState === 'expanded' || this.iconState === 'collapsed') {
      ...
      $(this.element).on('keypress', function (event) {
        ...
        if (code === $.ui.keyCode.ENTER || code === $.ui.keyCode.SPACE) {
          ...
          self._fireExpandCollapse(); // stop browser from for example scrolling the page


          event.preventDefault(); // ensure focus stays
      ...
      this.component.element.get(0).addEventListener('keydown', this.handleKeyDownCallback, true); // listens for expand and collapse event from flattened datasource
   ...
   * @private
   */
  _initExpanded: function _initExpanded() {
    if (!oj.DataProviderFeatureChecker.isDataProvider(this.datasource)) {
   ...
   * @private
   */
  _getDataProviderExpanded: function _getDataProviderExpanded() {
   ...
   * @private
   */
  _getFlattenedDataProvider: function _getFlattenedDataProvider() {
   ...
   * @private
   */
  _getChildCount: function _getChildCount(parentKey) {
    if (oj.DataProviderFeatureChecker.isDataProvider(this.datasource)) {
      var dataprovider;

      if (parentKey != null) {
        dataprovider = this._getFlattenedDataProvider().dataProvider.getChildDataProvider(this.parentKey);
      } else {
  ...
  _destroy: function _destroy() {
    ...
    $(this.component.element).off('ojbeforecurrentcell', this.handleActiveKeyChangeCallback); // unregister expand/collapse events
  ...
  _subscribeToDataProvider: function _subscribeToDataProvider() {
    var self = this;
      ...
      if (shouldExpand || shouldCollapse) {
        ...
        if (completionPromise) {
          completionPromise.then(function () {
            if (shouldExpand) {
              self._handleExpandEvent({
                rowKey: self.rowKey
              });
            }

            if (shouldCollapse) {
              self._handleCollapseEvent({
                rowKey: self.rowKey
              });
            }
          });
  ...
  _expand: function _expand() {
    if (this.iconState === 'collapsed') {
      ...
      if (!oj.DataProviderFeatureChecker.isDataProvider(this.datasource)) {
        ...
        this._getFlattenedDataProvider().setExpanded(this._getDataProviderExpanded().add([this.rowKey]));
      }
  ...
  _collapse: function _collapse() {
    if (this.iconState === 'expanded') {
      ...
      if (!oj.DataProviderFeatureChecker.isDataProvider(this.datasource)) {
        ...
        this._getFlattenedDataProvider().setExpanded(this._getDataProviderExpanded().delete([this.rowKey]));
      }
  ...
  _setOption: function _setOption(key, value, flags) {
    ...
    this._super(key, value, flags); // refresh if context is updated
   ...
   * @private
   */
  _addIndentation: function _addIndentation() {
   ...
   * @private
   */
  _appendSpacer: function _appendSpacer(depth) {
  ...
  _addIcon: function _addIcon() {
    ...
    this.toucharea = $(document.createElement('div')).addClass(this.classNames.toucharea); // if icon is a leaf do not add # because that will trigger navigation if entered in JAWS

    this.icon = $(document.createElement('a')).attr('href', this.iconState === 'leaf' ? '' : '#').attr('aria-labelledby', this._getLabelledBy()).addClass(this.classNames.icon).addClass(this.classNames.clickable).attr('aria-label', this.getTranslatedString('accessibleLevelDescription', {
      level: this.depth
    }));
   ...
   */
  _addIconClass: function _addIconClass(classKey) {
   ...
   * @param {string} classKey the key of the appropriate icon class expand/collapse/leaf
   */
  _removeIconClass: function _removeIconClass(classKey) {
   ...
   * @private
   */
  _setIconStateClass: function _setIconStateClass() {
   ...
   * @private
   */
  _removeIconStateClass: function _removeIconStateClass() {
    switch (this.iconState) {
      case 'leaf':
   ...
   * @private
   */
  _handleActiveKeyChangeEvent: function _handleActiveKeyChangeEvent(event, ui) {
    var previousRowKey;
  ...
  _handleKeyDownEvent: function _handleKeyDownEvent(event) {
    var targetContext = Components.__GetWidgetConstructor(this.component.element.get(0))('getContextByNode', event.target);
    ...
    if (this.rowKey === rowKey) {
      var code = event.keyCode || event.which; // ctrl (or equivalent) is pressed
   ...
   * @private
   */
  _handleExpandEvent: function _handleExpandEvent(event) {
    ...
    if (rowKey === this.rowKey) {
      ...
      this._updateContextState('expanded'); // if the event is triggered by initial setting of expanded, we should not
   ...
   * @private
   */
  _handleCollapseEvent: function _handleCollapseEvent(event) {
    var rowKey = event.rowKey ? event.rowKey : event.detail.key;
    ...
    if (rowKey === this.rowKey) {
      ...
      this._updateContextState('collapsed'); // if the event is triggered by initial setting of expanded, we should not
      // fire expand or option change event
   ...
   * @private
   */
  _updateExpandedState: function _updateExpandedState(expanded) {
   ...
   * @private
   */
  _updateContextState: function _updateContextState(newState) {
    var context = this.options.context;
    context.state = newState; // need to reuse the same object so mark it as changed ourselves
   ...
   * @private
   */
  _fireExpandCollapse: function _fireExpandCollapse() {
    var state = this.iconState; // show loading icon, note this changes the icon state to 'loading'
   ...
   * @private
   */
  _ariaExpanded: function _ariaExpanded(bool) {
  ...
  getSubIdByNode: function getSubIdByNode(node) {
    if (node === this.icon.get(0)) {
      return {
        subId: 'oj-rowexpander-disclosure'
      };
    }
   ...
   * @private
   */
  _getLabelledBy: function _getLabelledBy() {
...
(function () {
  ...
  oj.CustomElementBridge.register('oj-row-expander', {
    metadata: __oj_row_expander_metadata
  });
})();

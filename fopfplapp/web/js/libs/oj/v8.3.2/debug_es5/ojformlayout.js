...
Object.freeze(__oj_form_layout_metadata); // counter for the generation of unique ids.
...
function ojFormLayout(context) {
  ...
  var LABEL_ELEMENT_ID = '-labelled-by'; // We need to mark every element we add so that we can remove it during a refresh/re-render.
  ...
  var updatePending = false; // update labelEdge and colspanWrap based on context

  _updateDefaultFromTheme(context); // Our version of GCC has a bug where the second param of MutationObserver.observe must be of
  ...
  self._rootElementMutationObserver = new MutationObserver(function (mutations) {
    ...
    if (document.body.contains(element)) {
      ...
      _moveNewDirectChildrenToOjFormDiv(mutations); // Ignore mutations that aren't direct children of bonus dom div elements as those
  ...
  this.createDOM = function () {
    element.classList.add('oj-form-layout'); // Create wrapper div
  ...
  this.updateDOM = function () {
    function doUpdate() {
      ...
      labelWidth = element.labelEdge === 'start' ? element.labelWidth : '100%'; // Wait until child elements have finished upgrading before performing DOM
  ...
  function _notReady() {
    if (!readyResolveFunc) {
      ...
      var options = {
        description: "The oj-form-layout component with id = '" + element.id + "' is being rendered."
      };
  ...
  function _ojFormNotReady() {
    if (!ojFormReadyResolveFunc) {
      var busyContext = Context.getContext(ojForm).getBusyContext();
      var options = {
        description: "The oj-form div for oj-form-layout component with id = '" + element.id + "' is being rendered."
      };
  ...
  function _removeAllBonusDomElements() {
    ...
    for (var i = 0; i < length; ++i) {
      var bonusDomElem = bonusDomElems[i]; // don't remove bonus dom elems owned by child oj-form-layouts
      ...
      if (_isNodeOfThisFormLayout(bonusDomElem)) {
        if (bonusDomElem.tagName === 'OJ-LABEL') {
          ...
          bonusDomElem.for = ''; // For the oj-label elements we create, we can just remove them safely
  ...
  function _addLabelsFromHints() {
    ...
    var childCnt = 0;
    var directionIsColumn = element.direction === 'column'; // for all of the direct children, add a label for any child element supports "labelHint"

    while (child) {
      var tagName = child.tagName.toLowerCase(); // check for custom element
      ...
      if (tagName.indexOf('-') !== -1) {
        if (tagName === 'oj-label') {
          ...
          if (!child) {
            ...
            _ensureUniqueId(label); // make the component ready before throwing an error so the page won't hang
            ...
            throw new Error("oj-form-layout component with id='" + element.id + "' has an oj-label child element with id='" + label.id + "' but has no next sibling element that it is associated with.");
          }
  ...
  function _addLabelFromHint(child) {
    ...
    if (child instanceof Element && 'labelHint' in child && child.labelHint !== '') {
      ...
      ojLabel = _createOjLabelAndInitialize(child); // the label should preceed the input element it is associated with
  ...
  function _createOjLabelAndInitialize(editableElem) {
    var evLabelledBy = editableElem.labelledBy;

    if (evLabelledBy) {
      // catch the condition where the app dev removed
      // oj-label in favor of label-hint but forgot to remove labelled-by.
      var labelledByLabel = document.getElementById(evLabelledBy);

      if (labelledByLabel) {
        Logger.error('The oj-form-layout descendent component with id ' + editableElem.id + ' has both label-hint and labelled-by. Remove labelled-by="' + evLabelledBy + '" since no matching element was found in the document, ' + 'and a label will be created using the label-hint.');
      }
    }
    ...
    ojLabel.setAttribute('data-oj-internal', ''); // programmatically created elements not managed by a binding stratagy like knockout
    // needs this attribute to signal the component should be created.

    ojLabel.setAttribute('data-oj-binding-provider', 'none');
    ojLabel.setAttribute('data-oj-context', ''); // Note: the hint might be null, but that is fine, we still want a label for this case to hang the required and help icons off of
  ...
  function _childLabelHintChanged(event) {
    var editable = event.target; // Get the hint span and update its innerText
    ...
    for (var i = 0; i < mutationsLength; i++) {
      ...
      if (mutation.type === 'childList') {
        ...
        for (var j = 0; j < addedNodesLength; j++) {
          var child = mutation.addedNodes[j]; // check to see if it was added as a direct child
  ...
  function _getOjLabelForChildElement(child) {
    var ojLabel; // for editable value components that have a labelled-by attribute, we need to
  ...
  function _addAllFlexDivs() {
    ...
    var needsFullWidthClass = false; // each iteration should process a label/input pair

    while (j < arrayLength) {
      var label = childArray[j];
      colspan = 1; // if the current child element is on the unresolvedChildren list, skip it
      ...
      if (unresolvedChildren.indexOf(label) === -1) {
        ...
        if (tagName === 'oj-label') {
          ...
          var elem = childArray[j]; // for direction === "row", we only render the oj-flex div once per row.

          currentRow = _addFlexDivs(label, elem, directionIsColumn || pairCnt % maxCols === 0);
          ...
          currentRow = _addFlexDivs(label, null, directionIsColumn || pairCnt % maxCols === 0);
        } else {
          ...
          if ('colspan' in label && label.getAttribute('colspan')) {
            if (directionIsColumn) {
              ...
              var availableCols = maxCols - curCol; // force integer colspan values
          ...
          currentRow = _addFlexDivs(label, null, directionIsColumn || pairCnt % maxCols === 0, colspan);
        }
      }
  ...
  function _getLabelFlexItemWidth() {
    ...
    if (!labelFlexItemWidth) {
      labelFlexItemWidth = labelWidth; // no need to adjust labelEdge == 'top', it will always be '100%'
      ...
      if (element.labelEdge === 'start' && element.direction === 'row') {
        ...
        var parts = element.labelWidth.match(CssUnitsRegex); // only adjust for units that are (or can be) relative to horizontal screen size.
  ...
  function _addEmptyFlexItem(flexContainer, width) {
    var flexItem = _createDivElement('oj-flex-item'); //  - IE11 doesn't allow calc() expressions in 'style.flex' so we have to specify
  ...
  function _addFlexDivs(label, elem, createOjFlex, colspan) {
    ...
    if (elem) {
      ...
      slotWidth = _getFullFlexItemWidth(label, colspan); // this is actually the element flex item in this case
      //  - IE11 doesn't allow calc() expressions in 'style.flex' so we have to specify
      // the longhand versions just in case the slotWidth is specified as a calc() expression

      labelOjFlexItem.style.flexGrow = '1';
      labelOjFlexItem.style.flexShrink = '1';
      labelOjFlexItem.style.flexBasis = slotWidth; // We need this for both row and column direction so that labels will
      // truncate correctly.

      labelOjFlexItem.style.maxWidth = slotWidth;
      labelOjFlexItem.style.width = slotWidth; // for the case where there is an empty label flex item with 0px width, we need to add this class
  ...
  function _ignoreMutations(mutations) {
    ...
    for (var i = 0; i < mutationsLength; i++) {
      ...
      if (mutation.type === 'childList' && _isBonusDomDivOrSelf(mutation.target) && _isNodeOfThisFormLayout(mutation.target)) {
        ignore = false;
        break;
  ...
  function _isBonusDomDivOrSelf(node) {
    return node === element || node && node.tagName === 'DIV' && node.hasAttribute(BONUS_DOM_ATTR);
  }
  /**
  ...
  function _isNodeOfThisFormLayout(_node) {
    ...
    while (node !== element) {
      ...
      node = node.parentElement; // must be a deleted node so ignore it
...
(function () {
  ...
  oj.CustomElementBridge.register('oj-form-layout', {
    metadata: __oj_form_layout_metadata
  });
})();

...
(function () {
  ...
  oj.__registerWidget('oj.ojInputNumber', $.oj.editableValue, {
    ...
    _InitOptions: function _InitOptions(originalDefaults, constructorOptions) {
      ...
      var self = this; // call super class with arguments: originalDefaults and constructorOptions. It will set
      // the this.options.
      ...
      if (!this._IsCustomElement()) {
        ...
        var props = [{
          ...
          attribute: 'value'
        }, // don't coerce here. I do it myself
        {
          attribute: 'readonly',
          option: 'readOnly',
          validateOption: true
        ...
        oj.EditableValueUtils.initializeOptionsFromDom(props, constructorOptions, this, // post-process callback
      ...
      this.initialValue = opts.value; // now make sure min < max, else throw an Error
    ...
    _AfterCreate: function _AfterCreate() {
      this._super(); // Initialize transientValue to be the same as value upon component creation
      ...
      this.option({
        transientValue: this.options.value
      }, {
        _context: context
      });

      this._refreshAriaMinMax(); // buttons and aria-valuenow and aria-valuetext are updated in _Refresh for value which
    ...
    */
    ...
    _GetConverter: function _GetConverter() {
      return this.options.converter ? this._getConverter() : $.oj.ojInputNumber.prototype.options.converter;
    },
    ...
    */
    ...
    _destroy: function _destroy() {
      var ret = this._super(); // destroy the buttonset
      ...
      this.element.off('blur keydown keyup compositionstart compositionend input'); //  - DomUtils.unwrap() will avoid unwrapping if the node is being destroyed by Knockout
    ...
    _Refresh: function _Refresh(name, value, forceDisplayValueRefresh) {
      ...
      switch (name) {
        case 'value':
          valuenow = this.options.value || 0; // this gets called after value option changes, so no need to convert
          // the display value.
        ...
        case 'min':
          ...
          valuenow = this._getDisplayValueParsed(); // disables or enables both the up and down buttons depending upon what the value
    ...
    _SetLoading: function _SetLoading() {
      ...
      this._refreshStateTheming('readOnly', readOnly); // removes role spinbutton while loading
     ...
     * @memberof oj.ojInputNumber
     */
    _getImplicitNumberRangeValidator: function _getImplicitNumberRangeValidator() {
    ...
    _setup: function _setup() {
      ...
      if (needsButtonset) {
        ...
        this.upButton.ojButton({
          label: incrementString
        });
        this.downButton.ojButton({
     ...
     * @memberof oj.ojInputNumber
     */
    _markInternalComponents: function _markInternalComponents() {
     ...
     * @memberof oj.ojInputNumber
     */
    _createOjButtonset: function _createOjButtonset() {
      ...
      this.upButton = $(buttonSetObj.upButton).ojButton({
        ...
        icons: {
          start: 'oj-component-icon oj-inputnumber-up-icon'
        }
      });
      this.downButton = $(buttonSetObj.downButton).ojButton({
        ...
        icons: {
          start: 'oj-component-icon oj-inputnumber-down-icon'
        }
      });
     ...
     * @memberof oj.ojInputNumber
     */
    _destroyOjButtonset: function _destroyOjButtonset() {
     ...
     * @memberof oj.ojInputNumber
     */
    _createOrDestroyOjButtonset: function _createOrDestroyOjButtonset() {
      var needsButtonset = this._needsButtonset(); // Create or destroy buttonset
    ...
    _draw: function _draw() {
      ...
      containerElem.setAttribute('role', 'presentation'); // only custom element will have an OuterWrapper
      ...
      this._SetInputType(this._ALLOWED_TYPES); // Won't create buttons if readonly or if step is 0.
     ...
     * @memberof oj.ojInputNumber
     */
    _needsButtonset: function _needsButtonset() {
     ...
     * @memberof oj.ojInputNumber
     */
    _createButtonset: function _createButtonset() {
     ...
     * @memberof oj.ojInputNumber
     */
    _repeat: function _repeat(i, direction, event, spinning) {
      var stopRepeat = false;
      var domElem;
      var busyContext;
      var self = this;
      var stepOpt = this.options.step; // if direction is > 0, it is going up, else it is going down.
      ...
      i = i || 500;
      clearTimeout(this.timer); // this.timer will be cleared elsewhere, like when the user stops holding down the up/down
      ...
      }, i); // I need two paths. When I am 'spinning' and when I click click click the up
     ...
     */
    _spin: function _spin(step, event, transientOnly) {
      var adjustedValue;
      ...
      var value = this._ParseValueShowErrors(displayValue); // parsing is synchronous, but _SetValue could be async if we have async-validators.
     ...
     * @memberof oj.ojInputNumber
     */
    _adjustParsedValueOnSpinAndUpdateDisplay: function _adjustParsedValueOnSpinAndUpdateDisplay(value, step) {
      ...
      var precision;
      var adjustedValue;
      var formatReturn; // get the max precision. e.g., min=2.4, initialValue=3.444, maxPrecision is 3.

      precision = this._precision(minOpt, stepOpt, initialValue);
      adjustedValue = this._adjustValue(value, step, minOpt, maxOpt, stepOpt, precision, initialValue); // Show the user what is going to be validated. We are making it so that clicking on the
     ...
     * @memberof oj.ojInputNumber
     */
    _precision: function _precision(minOpt, stepOpt, value) {
     ...
     */
    _adjustValue: function _adjustValue(value, step, minOpt, maxOpt, stepOpt, precision, initialValue) {
      var newValue;
      var stepBase;
      var aboveMin;
      ...
      if (precision > 0) {
        valueWithFraction = this._adjustValueForFractions(value, step, minOpt, maxOpt, stepOpt, precision, initialValue);
      ...
      try {
        ...
        if (e instanceof TypeError) {
          ...
          Logger.warn("inputNumber's value after conversion is not a number. \n" + 'The converter must convert the value to a Number. coercing using +'); // eslint-disable-next-line no-param-reassign
      ...
      if (maxOpt != null && newValue > maxOpt) {
        var validMax = Math.floor((maxOpt - stepBase) / stepOpt) * stepOpt + stepBase; // fix precision from bad JS floating point math
     ...
     * @returns {number}
     */
    _adjustValueForFractions: function _adjustValueForFractions(value, step, minOpt, maxOpt, stepOpt, precision, initialValue) {
      // don't call this function if precision is 0
      oj.Assert.assert(precision > 0);
      var power = Math.pow(10, precision); // if minOpt, maxOpt, stepOpt are undefined, keep them that way
      ...
      var adjustValuePower = this._adjustValue(Math.round(value * power), Math.round(step * power), minOptPower, maxOptPower, stepOptPower, 0, Math.round(initialValue * power));
    ...
    _stop: function _stop(event) {
      ...
      if (this._valuePending) {
        ...
        this.option({
          value: this.options.transientValue
        }, {
          _context: context
        });
     ...
     * @memberof oj.ojInputNumber
     */
    _updateButtons: function _updateButtons(valuenow) {
      var options = this.options;
      var maxOpt = options.max;
      var minOpt = options.min;
      ...
      if (options.disabled || valuenow === undefined || isMaxOptNonNull && isMinOptNonNull && maxOpt === minOpt && valuenow === maxOpt) {
     ...
     * @memberof oj.ojInputNumber
     */
    _getDisplayValueParsed: function _getDisplayValueParsed() {
      var value;
      var displayValue;

      try {
        displayValue = this._GetDisplayValue() || 0; // if displayValue is not parseable, say it is 'abc',
     ...
     * @memberof oj.ojInputNumber
     */
    _blurEnterSetValue: function _blurEnterSetValue(event) {
      ...
      this._stop(event); // clear step queue


      this.stepQueue = []; // used to ignore if we get newer blurs

      this._blurEnterSetValueCounter += 1;
      currentCounter = this._blurEnterSetValueCounter;
      this._valuePending = false;
      setValueReturn = this._SetValue(val, event, {
        targetOptions: ['transientValue', 'value']
      });
      ...
      if (setValueReturn instanceof Promise) {
        ...
        setValueReturn.then(function (setValueResolved) {
          ...
          if (currentCounter === self._blurEnterSetValueCounter) {
            if (!setValueResolved) {
              ...
              valuenow = self._getDisplayValueParsed(); // we get here if parse is synchronous but validate is async, and validate
              // failed.
        ...
        valuenow = setValueReturn ? this.options.value : this._getDisplayValueParsed(); // disables or enables both the up and down buttons depending upon what the value
        // is on the screen after conversion plus what min/max are set to.
     ...
     * @memberof oj.ojInputNumber
     */
    _updateButtonsAria: function _updateButtonsAria(valuenow) {
     ...
     * @memberof oj.ojInputNumber
     */
    _createRangeValidatorOptions: function _createRangeValidatorOptions() {
      var options = this.options;
      var minOpt = options.min;
      var maxOpt = options.max;
      ...
      var numberRangeValidatorOptions; // First check if the translations hint/messageDetail/messageSummary options are set, and if
      // so, use them.
     ...
     * @memberof oj.ojInputNumber
     */
    _parse: function _parse(option, val) {
      var returnValue;
     ...
     * @memberof oj.ojInputNumber
     */
    _parseStep: function _parseStep(val) {
      var defaultStep = 1;
      var parsedStep;
     ...
     * @memberof oj.ojInputNumber
     */
    _refreshRoleSpinbutton: function _refreshRoleSpinbutton(needsRole) {
      if (!needsRole) {
     ...
     * @memberof oj.ojInputNumber
     */
    _refreshAriaMinMax: function _refreshAriaMinMax() {
     ...
     * @memberof oj.ojInputNumber
     */
    _refreshAriaValueNowText: function _refreshAriaValueNowText(valuenow) {
     ...
     * @memberof oj.ojInputNumber
     */
    _refreshAriaText: function _refreshAriaText(valuenow) {
     ...
     * @memberof oj.ojInputNumber
     */
    _setAttr: function _setAttr(attrName, value) {
      if (value !== undefined) {
        if (value === null) {
     ...
     * @param {Object=} event an optional event if this was a result of ui interaction.
     */
    _stepNoStartStop: function _stepNoStartStop(step, event) {
      var currentStepObject;
      var currentSpinStep;
      var currentEvent; // this.stepQueue = []; initialized in _AfterCreate and cleared if spinning or blurEnter
      ...
      if (spinPromise instanceof Promise) {
        ...
        spinPromise.then(function () {
          ...
          self.stepQueue.shift(); // continue stepping if stepQueue is not empty.
     ...
     * @param {Object=} event an optional event if this was a result of ui interaction.
     */
    _step: function _step(step, event) {
      this._start();
    ...
    _regularEventsAndListeners: {
      ...
      'touchend .oj-inputnumber-button': function touchendOjInputnumberButton(event) {
        this._stop(event);
      },
      'touchcancel .oj-inputnumber-button': function touchcancelOjInputnumberButton(event) {
        this._stop(event);
      },
      'mousedown .oj-inputnumber-button.oj-enabled': function mousedownOjInputnumberButtonOjEnabled(event) {
      ...
      'mouseup .oj-inputnumber-button': function mouseupOjInputnumberButton(event) {
        if (this._isRealMouseEvent(event)) {
          this._stop(event);
        }
      },
      'mouseenter .oj-inputnumber-button.oj-enabled': function mouseenterOjInputnumberButtonOjEnabled(event) {
      ...
      'mouseleave .oj-inputnumber-button': function mouseleaveOjInputnumberButton(event) {
        if (this._isRealMouseEvent(event)) {
          this._stop(event);
        }
      }
    ...
    _passiveEventsAndListeners: {
      'touchstart .oj-inputnumber-button.oj-enabled': function touchstartOjInputnumberButtonOjEnabled(event) {
        this._start();
    ...
    */
    _registerEvents: function _registerEvents() {
      ...
      this._on(this._regularEventsAndListeners); // will register these with the passive option in 8.1.0
...
(function () {
  ...
  __oj_input_number_metadata.extension._ALIASED_PROPS = {
    readonly: 'readOnly'
  };
  ...
  oj.CustomElementBridge.register('oj-input-number', {
    metadata: __oj_input_number_metadata
  });
})();
